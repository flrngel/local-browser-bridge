...
fn macos_v0_12_27_result_and_aggregate_schemas_are_aligned_end_to_end() {
    ...
    let producer = fs::read_to_string("evidence/v0.12.41/computer/helper-evidence-rig.mjs")
        .unwrap()
        .replace("\r\n", "\n");
    let documentation = fs::read_to_string("evidence/v0.12.41/computer/README.md")
        .unwrap()
        .replace("\r\n", "\n");
    ...
    assert!(finalizer.contains("const PRODUCT_VERSION = \"0.12.41\";"));
    ...
    for required in [
        ...
        assert!(
            ...
            "release evidence verifier is missing the v0.12.41 schema binding: {required}"
        );
    }
}
