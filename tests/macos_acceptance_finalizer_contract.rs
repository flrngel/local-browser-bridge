...
#[test]
fn macos_v0_12_26_result_and_aggregate_schemas_are_aligned_end_to_end() {
    let finalizer = finalizer_source().replace("\r\n", "\n");
    let producer = fs::read_to_string("evidence/v0.12.26/computer/helper-evidence-rig.mjs")
        .unwrap()
        .replace("\r\n", "\n");
    let documentation = fs::read_to_string("evidence/v0.12.26/computer/README.md")
        .unwrap()
        .replace("\r\n", "\n");
    ...
    assert!(finalizer.contains("const PRODUCT_VERSION = \"0.12.26\";"));
    ...
    for required in [
        ...
        assert!(
            ...
            "release evidence verifier is missing the v0.12.26 schema binding: {required}"
        );
    }
}

#[test]
fn macos_v0_12_26_release_verifier_recomputes_every_tagged_harness_hash() {
    let verifier = fs::read_to_string("scripts/verify-release-acceptance-evidence.sh")
        .unwrap()
        .replace("\r\n", "\n");
...
#[test]
fn macos_v0_12_26_dual_lane_finalizer_is_dependency_free_and_fail_closed() {
...
#[test]
fn macos_v0_12_26_finalizer_enforces_exact_app_share_and_quiet_pointer_invariants() {
    let source = finalizer_source().replace("\r\n", "\n");
    for required in [
...
#[test]
fn macos_v0_12_26_dual_lane_finalizer_self_test_passes() {
