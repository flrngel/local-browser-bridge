...
fn macos_finalizer_wrapper_owns_dependent_aggregate_path_creation() {
    ...
    let documentation = fs::read_to_string("evidence/v0.12.66/computer/README.md")
        .unwrap()
        .replace("\r\n", "\n");
...
fn macos_v0_12_27_result_and_aggregate_schemas_are_aligned_end_to_end() {
    ...
    let producer = fs::read_to_string("evidence/v0.12.66/computer/helper-evidence-rig.mjs")
        .unwrap()
        .replace("\r\n", "\n");
    let documentation = fs::read_to_string("evidence/v0.12.66/computer/README.md")
        .unwrap()
    ...
    assert!(finalizer.contains("const PRODUCT_VERSION = \"0.12.66\";"));
    ...
    for required in [
        ...
        assert!(
            ...
            "release evidence verifier is missing the v0.12.66 schema binding: {required}"
        );
    }
}
