...
fn candidate_binder_is_exact_external_and_immutable() {
    ...
    for required in [
        ...
        "TrustedGitExecutable must be an absolute path for v0.12.30.",
...
fn v0122_browser_protocol_is_byte_exact_while_v01229_uses_schema_three() {
    ...
    let entries = fs::read_dir("evidence/v0.12.30/browser")
        .unwrap()
        .map(Result::unwrap)
        .map(|entry| {
    ...
    assert_eq!(template["extension"]["version"], "0.12.30");
    assert_eq!(
        schema["$schema"],
...
fn v01229_finalizer_enforces_surface_consent_restoration_and_retention_relations() {
    ...
    let readme = source("evidence/v0.12.30/browser/README.md");
    let operator_schema = source("evidence/v0.12.30/browser/operator-results.schema.json");
    ...
    for required in [
        ...
        "Evidence finalizer complete v0.12.30 self-test failed.",
    ] {
        assert!(
            script.contains(required),
            "v0.12.30 finalizer is missing {required}"
        );
    }
    ...
    for required in [
        ...
        assert!(
            ...
            "v0.12.30 operator protocol is missing {required}"
        );
    }
    for forbidden in [
        "REPLACE_WITH_",
...
fn screenshot_tool_strips_metadata_but_never_claims_unknown_pixel_redaction() {
    ...
    let finalizer = source("scripts/write-browser-evidence-record.ps1");
    let v01229_readme = source("evidence/v0.12.30/browser/README.md");
    let coordinator = source("scripts/test-windows-stock-chrome.ps1");
    for required in [
        "LegacyV0122ReviewConfirmed is mandatory for v0.12.2 compatibility.",
        "LegacyV0122ReviewConfirmed is forbidden for the v0.12.30 independent-review protocol.",
        ...
        "BindReview is available only for the v0.12.30 independent-review protocol.",
...
fn v01229_powershell_entrypoints_cannot_fall_through_legacy_gates() {
    ...
    assert!(browser_runner.contains("$Version -ceq \"0.12.30\""));
    for required in [
    ...
    assert!(computer_runner.contains("-ProductVersion \"0.12.30\""));
    assert!(computer_runner.contains("productVersion -cne \"0.12.30\""));

    let watcher = source("scripts/wait-windows-foreground-arm-handoff.ps1");
    assert!(watcher.contains("$script:ProductVersion = \"0.12.30\""));
    assert!(watcher.contains("$script:MarkerSchemaVersion = 2"));
    assert!(watcher.contains("productVersion = \"0.12.30\""));
}
...
fn new_browser_evidence_files_are_english_only() {
    for path in [
        ...
        "evidence/v0.12.30/browser/README.md",
        "evidence/v0.12.30/browser/operator-results.template.json",
        "evidence/v0.12.30/browser/operator-results.schema.json",
        "evidence/v0.12.30/browser/computer-helper-chain.schema.json",
        "evidence/v0.12.30/browser/scoped-action-approval.schema.json",
