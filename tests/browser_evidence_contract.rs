...
fn candidate_binder_is_exact_external_and_immutable() {
    ...
    for required in [
        ...
        "TrustedGitExecutable must be an absolute path for v0.12.16.",
...
fn withdrawn_v0128_browser_protocol_is_byte_exact_unexecuted_and_not_current() {
    ...
    assert!(finalizer.contains("$script:OperatorV2Version = \"0.12.16\""));
...
#[test]
fn v0122_browser_protocol_is_byte_exact_while_v01216_uses_schema_three() {
    ...
    let entries = fs::read_dir("evidence/v0.12.16/browser")
        .unwrap()
        .map(Result::unwrap)
        .map(|entry| {
    ...
    assert_eq!(template["extension"]["version"], "0.12.16");
    assert_eq!(
        schema["$schema"],
...
#[test]
fn v01216_finalizer_enforces_surface_consent_restoration_and_retention_relations() {
    let script = source("scripts/write-browser-evidence-record.ps1");
    let coordinator = source("scripts/test-windows-stock-chrome.ps1");
    let readme = source("evidence/v0.12.16/browser/README.md");
    let operator_schema = source("evidence/v0.12.16/browser/operator-results.schema.json");
    ...
    for required in [
        ...
        "Evidence finalizer complete v0.12.16 self-test failed.",
    ] {
        assert!(
            script.contains(required),
            "v0.12.16 finalizer is missing {required}"
        );
    }
    ...
    for required in [
        ...
        assert!(
            ...
            "v0.12.16 operator protocol is missing {required}"
        );
    }
    for forbidden in [
        "REPLACE_WITH_",
...
fn screenshot_tool_strips_metadata_but_never_claims_unknown_pixel_redaction() {
    ...
    let finalizer = source("scripts/write-browser-evidence-record.ps1");
    let v01216_readme = source("evidence/v0.12.16/browser/README.md");
    let coordinator = source("scripts/test-windows-stock-chrome.ps1");
    for required in [
        "LegacyV0122ReviewConfirmed is mandatory for v0.12.2 compatibility.",
        "LegacyV0122ReviewConfirmed is forbidden for the v0.12.16 independent-review protocol.",
        ...
        "BindReview is available only for the v0.12.16 independent-review protocol.",
    ...
    assert!(v01216_readme.contains("six digest-bound finalized screenshot sidecars"));
}
...
fn token_cleanup_proof_is_bound_to_the_trusted_popup_and_reduced_state() {
    ...
    let normalized_v01216 = v01216_readme
        .split_whitespace()
        .collect::<Vec<_>>()
        .join(" ");
    assert!(normalized_v01216.contains("accept the popup confirmation under the scoped approval"));
    assert!(v01216_readme.contains("Chrome's token-clear confirmation"));
    assert!(!v01216_readme.contains("confirmationAcceptedByHuman"));
}

#[test]
fn v01216_computer_helper_chain_is_live_exact_window_and_six_capture_bound() {
...
#[test]
fn v01216_powershell_entrypoints_cannot_fall_through_legacy_gates() {
    let paths = [
    ...
    assert!(sanitizer.contains("@(\"0.12.2\", \"0.12.16\")"));
    assert!(sanitizer.contains("$script:CandidateVersionFromPreflight -ceq \"0.12.16\""));

    let finalizer = source("scripts/write-browser-evidence-record.ps1");
    assert!(finalizer.contains("$script:OperatorV2Version = \"0.12.16\""));
    assert!(finalizer.contains("@(\"0.12.2\", $script:OperatorV2Version)"));
    assert!(finalizer.contains("\"evidence\", \"v0.12.16\", \"browser\""));

    let recorder = source("scripts/record-computer-helper-chain.ps1");
    assert!(recorder.contains("$script:Version = \"0.12.16\""));

    let browser_runner = source("scripts/test-windows-browser-api.ps1");
    assert!(browser_runner.contains("$Version -ceq \"0.12.16\""));
    for required in [
    ...
    assert!(computer_runner.contains("-ProductVersion \"0.12.16\""));
    assert!(computer_runner.contains("productVersion -cne \"0.12.16\""));

    let watcher = source("scripts/wait-windows-foreground-arm-handoff.ps1");
    assert!(watcher.contains("$script:ProductVersion = \"0.12.16\""));
    assert!(watcher.contains("$script:MarkerSchemaVersion = 2"));
    assert!(watcher.contains("productVersion = \"0.12.16\""));
}

#[test]
fn v01216_powershell_json_readers_preserve_iso_timestamp_strings() {
    for path in [
        "scripts/browser-evidence-candidate.ps1",
        "scripts/record-computer-helper-chain.ps1",
        "scripts/sanitize-browser-evidence-screenshot.ps1",
...
fn new_browser_evidence_files_are_english_only() {
    for path in [
        ...
        "evidence/v0.12.9/browser/operator-results.schema.json",
        "evidence/v0.12.9/browser/computer-helper-chain.schema.json",
        "evidence/v0.12.16/browser/README.md",
        "evidence/v0.12.16/browser/operator-results.template.json",
        "evidence/v0.12.16/browser/operator-results.schema.json",
        "evidence/v0.12.16/browser/computer-helper-chain.schema.json",
        "evidence/v0.12.16/browser/scoped-action-approval.schema.json",
