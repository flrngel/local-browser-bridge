...
fn candidate_binder_is_exact_external_and_immutable() {
    ...
    for required in [
        ...
        "TrustedGitExecutable must be an absolute path for v0.12.5.",
...
#[test]
fn v0122_browser_protocol_is_byte_exact_while_v0125_uses_schema_two() {
    ...
    let template: Value = serde_json::from_str(&source(
        "evidence/v0.12.5/browser/operator-results.template.json",
    ))
    .unwrap();
    let schema: Value = serde_json::from_str(&source(
        "evidence/v0.12.5/browser/operator-results.schema.json",
    ))
    .unwrap();
    assert_eq!(template["schemaVersion"], 2);
    assert_eq!(template["extension"]["version"], "0.12.5");
    assert_eq!(
        schema["$schema"],
...
#[test]
fn v0125_finalizer_enforces_surface_consent_restoration_and_retention_relations() {
    let script = source("scripts/write-browser-evidence-record.ps1");
    let readme = source("evidence/v0.12.5/browser/README.md");
    for required in [
        ...
        "v0.12.5 screenshot identity, helper surface, or visible-state criterion is invalid.",
        ...
        "Evidence finalizer complete v0.12.5 self-test failed.",
    ] {
        assert!(
            script.contains(required),
            "v0.12.5 finalizer is missing {required}"
        );
    }
    for required in [
        ...
        assert!(
            ...
            "v0.12.5 protocol is missing {required}"
        );
    }
...
fn screenshot_tool_strips_metadata_but_never_claims_unknown_pixel_redaction() {
    ...
    let v015_readme = source("evidence/v0.12.5/browser/README.md");
    for required in [
        ...
        "AttestReview is available only for the v0.12.5 two-phase screenshot protocol.",
    ...
    let sanitize = v015_readme.find("-Mode Sanitize").unwrap();
    let human_pause = v015_readme.find("Automation must then pause.").unwrap();
    let review_receipt = v015_readme
        .find("Exact purpose-and-image-digest-bound human review receipt")
        .unwrap();
    let attest = v015_readme.find("-Mode AttestReview").unwrap();
...
fn token_cleanup_proof_is_bound_to_the_trusted_popup_and_reduced_state() {
    ...
    assert!(v015_readme.contains("wait for the popup's native `confirm()` dialog"));
    assert!(v015_readme.contains("confirmationAcceptedByHuman:true"));
}

#[test]
fn v0125_computer_helper_chain_is_live_exact_window_and_three_capture_bound() {
    let recorder = source("scripts/record-computer-helper-chain.ps1");
    let finalizer = source("scripts/write-browser-evidence-record.ps1");
    let schema: Value = serde_json::from_str(&source(
        "evidence/v0.12.5/browser/computer-helper-chain.schema.json",
    ))
    .unwrap();
...
fn new_browser_evidence_files_are_english_only() {
    for path in [
        ...
        "evidence/v0.12.2/browser/operator-results.template.json",
        "evidence/v0.12.5/browser/README.md",
        "evidence/v0.12.5/browser/operator-results.template.json",
        "evidence/v0.12.5/browser/operator-results.schema.json",
        "evidence/v0.12.5/browser/computer-helper-chain.schema.json",
    ] {
