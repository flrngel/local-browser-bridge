...
fn candidate_binder_is_exact_external_and_immutable() {
    ...
    for required in [
        ...
        "TrustedGitExecutable must be an absolute path for v0.12.67.",
...
fn withdrawn_v0128_browser_protocol_is_byte_exact_unexecuted_and_not_current() {
    ...
    assert!(finalizer.contains("$script:OperatorV2Version = \"0.12.67\""));
...
fn v0122_browser_protocol_is_byte_exact_while_v01229_uses_schema_three() {
    ...
    let entries = fs::read_dir("evidence/v0.12.67/browser")
        .unwrap()
        .map(Result::unwrap)
        .map(|entry| {
    ...
    let template: Value = serde_json::from_str(&source(
        "evidence/v0.12.67/browser/operator-results.template.json",
    ))
    .unwrap();
    let schema: Value = serde_json::from_str(&source(
        "evidence/v0.12.67/browser/operator-results.schema.json",
    ))
    .unwrap();
    let helper_schema: Value = serde_json::from_str(&source(
        "evidence/v0.12.67/browser/computer-helper-chain.schema.json",
    ))
    .unwrap();
    let approval_schema: Value = serde_json::from_str(&source(
        "evidence/v0.12.67/browser/scoped-action-approval.schema.json",
    ))
    .unwrap();
    let review_schema: Value = serde_json::from_str(&source(
        "evidence/v0.12.67/browser/independent-visual-review.schema.json",
    ))
    .unwrap();
    let external_schema: Value = serde_json::from_str(&source(
        "evidence/v0.12.67/browser/external-surface-attestation.schema.json",
    ))
    .unwrap();
    assert_eq!(template["schemaVersion"], 3);
    assert_eq!(template["extension"]["version"], "0.12.67");
    assert_eq!(
        schema["$schema"],
...
fn v01229_finalizer_enforces_surface_consent_restoration_and_retention_relations() {
    ...
    let readme = source("evidence/v0.12.67/browser/README.md");
    let operator_schema = source("evidence/v0.12.67/browser/operator-results.schema.json");
    ...
    for required in [
        ...
        "Evidence finalizer complete v0.12.67 self-test failed.",
    ] {
        assert!(
            script.contains(required),
            "v0.12.67 finalizer is missing {required}"
        );
    }
    ...
    for required in [
        ...
        assert!(
            ...
            "v0.12.67 operator protocol is missing {required}"
        );
    }
    for forbidden in [
        "REPLACE_WITH_",
...
fn screenshot_tool_strips_metadata_but_never_claims_unknown_pixel_redaction() {
    ...
    let finalizer = source("scripts/write-browser-evidence-record.ps1");
    let v01229_readme = source("evidence/v0.12.67/browser/README.md");
    let coordinator = source("scripts/test-windows-stock-chrome.ps1");
    for required in [
        "LegacyV0122ReviewConfirmed is mandatory for v0.12.2 compatibility.",
        "LegacyV0122ReviewConfirmed is forbidden for the v0.12.67 independent-review protocol.",
        ...
        "BindReview is available only for the v0.12.67 independent-review protocol.",
...
fn v01229_powershell_entrypoints_cannot_fall_through_legacy_gates() {
    ...
    assert!(sanitizer.contains("@(\"0.12.2\", \"0.12.67\")"));
    assert!(sanitizer.contains("$script:CandidateVersionFromPreflight -ceq \"0.12.67\""));
    ...
    assert!(finalizer.contains("\"evidence\", \"v0.12.67\", \"browser\""));
    ...
    let recorder = source("scripts/record-computer-helper-chain.ps1");
    assert!(recorder.contains("$script:Version = \"0.12.67\""));

    let browser_runner = source("scripts/test-windows-browser-api.ps1");
    assert!(browser_runner.contains("$Version -ceq \"0.12.67\""));
    for required in [
    ...
    assert!(computer_runner.contains("-ProductVersion \"0.12.67\""));
    assert!(computer_runner.contains("productVersion -cne \"0.12.67\""));

    let watcher = source("scripts/wait-windows-foreground-arm-handoff.ps1");
    assert!(watcher.contains("$script:ProductVersion = \"0.12.67\""));
    assert!(watcher.contains("$script:MarkerSchemaVersion = 2"));
    assert!(watcher.contains("productVersion = \"0.12.67\""));
}
...
fn new_browser_evidence_files_are_english_only() {
    for path in [
        ...
        "evidence/v0.12.67/browser/README.md",
        "evidence/v0.12.67/browser/operator-results.template.json",
        "evidence/v0.12.67/browser/operator-results.schema.json",
        "evidence/v0.12.67/browser/computer-helper-chain.schema.json",
        "evidence/v0.12.67/browser/scoped-action-approval.schema.json",
