...
fn candidate_binder_is_exact_external_and_immutable() {
    ...
    for required in [
        ...
        "TrustedGitExecutable must be an absolute path for v0.12.6.",
...
#[test]
fn v0122_browser_protocol_is_byte_exact_while_v0126_uses_schema_two() {
    ...
    let template: Value = serde_json::from_str(&source(
        "evidence/v0.12.6/browser/operator-results.template.json",
    ))
    .unwrap();
    let schema: Value = serde_json::from_str(&source(
        "evidence/v0.12.6/browser/operator-results.schema.json",
    ))
    .unwrap();
    assert_eq!(template["schemaVersion"], 2);
    assert_eq!(template["extension"]["version"], "0.12.6");
    assert_eq!(
        schema["$schema"],
...
#[test]
fn v0126_finalizer_enforces_surface_consent_restoration_and_retention_relations() {
    let script = source("scripts/write-browser-evidence-record.ps1");
    let readme = source("evidence/v0.12.6/browser/README.md");
    for required in [
        ...
        "v0.12.6 screenshot identity, helper surface, or visible-state criterion is invalid.",
        ...
        "Evidence finalizer complete v0.12.6 self-test failed.",
    ] {
        assert!(
            script.contains(required),
            "v0.12.6 finalizer is missing {required}"
        );
    }
    for required in [
        ...
        "--hostname github.com",
        "try {\n$Repository = New-PrivateEmptyDirectory",
        ...
        assert!(
            ...
            "v0.12.6 protocol is missing {required}"
        );
    }
    ...
    let directory_creation = readme
        .find("$Repository = New-PrivateEmptyDirectory")
        .unwrap();
...
fn screenshot_tool_strips_metadata_but_never_claims_unknown_pixel_redaction() {
    ...
    let v016_readme = source("evidence/v0.12.6/browser/README.md");
    for required in [
        ...
        "AttestReview is available only for the v0.12.6 two-phase screenshot protocol.",
    ...
    let sanitize = v016_readme.find("-Mode Sanitize").unwrap();
    let human_pause = v016_readme.find("Automation must then pause.").unwrap();
    let review_receipt = v016_readme
        .find("Exact purpose-and-image-digest-bound human review receipt")
        .unwrap();
    let attest = v016_readme.find("-Mode AttestReview").unwrap();
...
fn token_cleanup_proof_is_bound_to_the_trusted_popup_and_reduced_state() {
    ...
    assert!(v016_readme.contains("wait for the popup's native `confirm()` dialog"));
    assert!(v016_readme.contains("confirmationAcceptedByHuman:true"));
}

#[test]
fn v0126_computer_helper_chain_is_live_exact_window_and_three_capture_bound() {
    let recorder = source("scripts/record-computer-helper-chain.ps1");
    let finalizer = source("scripts/write-browser-evidence-record.ps1");
    let schema: Value = serde_json::from_str(&source(
        "evidence/v0.12.6/browser/computer-helper-chain.schema.json",
    ))
    .unwrap();
...
fn new_browser_evidence_files_are_english_only() {
    for path in [
        ...
        "evidence/v0.12.2/browser/operator-results.template.json",
        "evidence/v0.12.6/browser/README.md",
        "evidence/v0.12.6/browser/operator-results.template.json",
        "evidence/v0.12.6/browser/operator-results.schema.json",
        "evidence/v0.12.6/browser/computer-helper-chain.schema.json",
    ] {
