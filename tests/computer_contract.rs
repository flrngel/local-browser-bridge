...
fn macos_v0_12_27_pointer_evidence_is_bounded_corroboration_not_causal_attribution() {
    let rig = fs::read_to_string("evidence/v0.12.39/computer/helper-evidence-rig.mjs").unwrap();
    let probe = fs::read_to_string("evidence/v0.12.39/computer/SystemProbe.swift").unwrap();
    ...
    for required in [
        ...
        assert!(
            ...
            "missing v0.12.39 pointer contract: {required}"
        );
    }
...
fn macos_v0_12_27_action_pointer_classifier_matches_the_sealed_action_schema() {
    let rig = fs::read_to_string("evidence/v0.12.39/computer/helper-evidence-rig.mjs")
        .unwrap()
        .replace("\r\n", "\n");
...
fn macos_v0_12_14_quiet_lane_stabilizes_the_native_seat_before_candidate_execution() {
    let rig = fs::read_to_string("evidence/v0.12.39/computer/helper-evidence-rig.mjs")
        .unwrap()
        .replace("\r\n", "\n");
    let probe = fs::read_to_string("evidence/v0.12.39/computer/SystemProbe.swift")
        .unwrap()
        .replace("\r\n", "\n");
...
fn macos_v0_12_27_app_share_handoff_is_exact_non_authoritative_and_fail_closed() {
    let app = fs::read_to_string("evidence/v0.12.39/computer/AppShareHandoff.swift")
        .unwrap()
        .replace("\r\n", "\n");
    let probe = fs::read_to_string("evidence/v0.12.39/computer/SystemProbe.swift")
        .unwrap()
    ...
    let rig = fs::read_to_string("evidence/v0.12.39/computer/helper-evidence-rig.mjs")
        .unwrap()
        .replace("\r\n", "\n");
    let readme = fs::read_to_string("evidence/v0.12.39/computer/README.md")
        .unwrap()
        .replace("\r\n", "\n");
...
fn macos_pointer_arm_state_machine_execution_regressions_pass() {
    let output = match Command::new("node")
        .args([
            "evidence/v0.12.39/computer/helper-evidence-rig.mjs",
            "--self-test",
        ])
        .output()
    {
...
fn macos_candidate_evidence_targets_current_version_and_only_reduced_outputs() {
    let entries = fs::read_dir("evidence/v0.12.39/computer")
        .unwrap()
        .map(Result::unwrap)
        .map(|entry| {
            let file_type = entry.file_type().unwrap();
    ...
    let rig = fs::read_to_string("evidence/v0.12.39/computer/helper-evidence-rig.mjs")
        ...
        .replace("\r\n", "\n");
    let fixture =
        fs::read_to_string("evidence/v0.12.39/computer/HelperEvidenceFixture.swift").unwrap();
    let readme = fs::read_to_string("evidence/v0.12.39/computer/README.md").unwrap();
    ...
    assert!(!rig.replace("v0.12.39", "").contains("v0.12.1"));
    assert!(!fixture.replace("v0.12.39", "").contains("v0.12.1"));
    assert!(!rig.replace("v0.12.39", "").contains("v0.12.2"));
    assert!(!fixture.replace("v0.12.39", "").contains("v0.12.2"));
...
fn macos_package_preparer_accepts_only_the_canonical_bounded_ustar_package() {
    ...
    let rig = repository.join("evidence/v0.12.39/computer/helper-evidence-rig.mjs");
...
"#,
    ...
    for (scenario, expected_failure) in [
        ...
        let archive = case_root.join("local-browser-bridge-v0.12.39-macos-universal.tar.gz");
        ...
        let manifest_text = format!(
            "{zero_hash}  local-browser-bridge-v0.12.39-windows-x86_64.exe\n\
             {zero_hash}  local-computer-helper-v0.12.39-windows-x86_64.exe\n\
             {archive_sha256}  local-browser-bridge-v0.12.39-macos-universal.tar.gz\n\
             {zero_hash}  local-browser-bridge-extension-v0.12.39.zip\n"
        );
...
fn macos_packaged_evidence_acts_types_and_explicitly_cancels_fail_closed() {
    ...
    let fixture = fs::read_to_string("evidence/v0.12.39/computer/HelperEvidenceFixture.swift")
        .unwrap()
        .replace("\r\n", "\n");
...
fn windows_foreground_arm_handoff_watcher_is_strict_read_only_and_non_authoritative() {
    ...
    assert!(runner.contains("-ProductVersion \"0.12.39\""));
