...
fn release_workflow_and_local_builder_package_both_processes() {
    ...
    assert!(
        source(".github/workflows/ci.yml")
            .contains("verify-macos-app-share-handoff-self-test.sh 0.12.51\n")
    );
...
fn current_source_is_unblocked_and_package_versions_are_aligned() {
    ...
    for (path, required) in [
        ("Cargo.toml", "version = \"0.12.51\""),
        (
            "Cargo.lock",
            "name = \"local-browser-bridge\"\nversion = \"0.12.51\"",
        ),
        ("extension/manifest.json", "\"version\": \"0.12.51\""),
        ("extension/lib.js", "export const VERSION = \"0.12.51\";"),
        (
            "scripts/run-windows-computer-use-acceptance.ps1",
        ...
        (
            ...
            "readonly EVIDENCE_PRODUCT_VERSION=\"0.12.51\"",
        ),
        (
            "scripts/verify-windows-release-candidate.ps1",
    ...
    assert!(
        source("evidence/v0.12.51/computer/AppShareHandoff.swift")
            .contains("private let productVersion = \"0.12.51\"")
    );

    assert!(std::path::Path::new("evidence/v0.12.51/browser").is_dir());
    assert!(std::path::Path::new("evidence/v0.12.51/computer").is_dir());
...
fn macos_app_share_handoff_is_release_gated_and_pointer_watcher_is_adversarial_only() {
    ...
    let producer = source("evidence/v0.12.51/computer/helper-evidence-rig.mjs");
    let playbook = source("evidence/v0.12.51/computer/README.md");
    ...
    assert!(adversarial_watcher.contains("const PRODUCT_VERSION = \"0.12.51\";"));
    assert!(
    ...
    for integration in [&ci, &local] {
        assert!(
            integration.contains("node --check evidence/v0.12.51/computer/helper-evidence-rig.mjs"),
            "CI/local validation does not syntax-check the exact v0.12.51 macOS evidence rig"
        );
        assert!(
            integration
                .contains("node evidence/v0.12.51/computer/helper-evidence-rig.mjs --self-test")
        );
        assert!(
        ...
        for source in ["HelperEvidenceFixture.swift", "SystemProbe.swift"] {
            assert!(
                integration.contains(&format!(
                    "xcrun swiftc -typecheck evidence/v0.12.51/computer/{source}"
                )),
    ...
    assert!(ci.contains(
        "xcrun swiftc -typecheck evidence/v0.12.51/computer/PhysicalPointerHandoff.swift"
    ));
    for release_path in [&candidate, &local] {
        assert!(
