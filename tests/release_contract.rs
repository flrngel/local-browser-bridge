...
fn macos_pointer_concurrency_handoff_watcher_is_read_only_and_release_gated() {
    ...
    let producer = source("evidence/v0.12.16/computer/helper-evidence-rig.mjs");
    ...
    for integration in [&ci, &release, &local] {
        assert!(
            integration.contains("node --check evidence/v0.12.16/computer/helper-evidence-rig.mjs"),
            "release path does not syntax-check the exact macOS evidence rig"
        );
        assert!(
            integration
                .contains("node evidence/v0.12.16/computer/helper-evidence-rig.mjs --self-test")
        );
    }
        ...
        for source in [
            ...
            assert!(
                integration.contains(&format!(
                    "xcrun swiftc -typecheck evidence/v0.12.16/computer/{source}"
                )),
