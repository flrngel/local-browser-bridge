...
fn v01238_source_is_unblocked_and_release_versions_are_aligned() {
    match fs::symlink_metadata("RELEASE_BLOCKED") {
        ...
        Ok(_) => {
            panic!("the reviewed v0.12.38 source must not retain a release-blocker file or symlink")
        }
    }

    for (path, required) in [
        ("Cargo.toml", "version = \"0.12.38\""),
        (
            "Cargo.lock",
            "name = \"local-browser-bridge\"\nversion = \"0.12.38\"",
        ),
        ("extension/manifest.json", "\"version\": \"0.12.38\""),
        ("extension/lib.js", "export const VERSION = \"0.12.38\";"),
        (
            "scripts/run-windows-computer-use-acceptance.ps1",
        ...
        (
            ...
            "readonly EVIDENCE_PRODUCT_VERSION=\"0.12.38\"",
        ),
        (
            "scripts/verify-windows-release-candidate.ps1",
            "$ProductVersion = \"0.12.38\"",
        ),
        (
            "scripts/write-browser-evidence-record.ps1",
        ...
        assert!(
            ...
            "v0.12.38 version alignment is missing from {path}: {required}"
        );
    }

    assert!(std::path::Path::new("evidence/v0.12.38/browser").is_dir());
    assert!(std::path::Path::new("evidence/v0.12.38/computer").is_dir());
}
...
fn macos_app_share_handoff_is_release_gated_and_pointer_watcher_is_adversarial_only() {
    ...
    let producer = source("evidence/v0.12.38/computer/helper-evidence-rig.mjs");
    let playbook = source("evidence/v0.12.38/computer/README.md");
    ...
    assert!(adversarial_watcher.contains("const PRODUCT_VERSION = \"0.12.38\";"));
    assert!(
    ...
    for integration in [&ci, &local] {
        assert!(
            integration.contains("node --check evidence/v0.12.38/computer/helper-evidence-rig.mjs"),
            "CI/local validation does not syntax-check the exact v0.12.38 macOS evidence rig"
        );
        assert!(
            integration
                .contains("node evidence/v0.12.38/computer/helper-evidence-rig.mjs --self-test")
        );
        assert!(
        ...
        for source in ["HelperEvidenceFixture.swift", "SystemProbe.swift"] {
            assert!(
                integration.contains(&format!(
                    "xcrun swiftc -typecheck evidence/v0.12.38/computer/{source}"
                )),
    ...
    assert!(ci.contains(
        "xcrun swiftc -typecheck evidence/v0.12.38/computer/PhysicalPointerHandoff.swift"
    ));
    for release_path in [&candidate, &local] {
        assert!(
