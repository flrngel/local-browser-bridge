...
Version 0.12.22 introduced the create-once request/start/complete chain around one acceptance-only app-share button action, v0.12.23 separated sealed action-pointer evidence from keyboard-aware independent-system evidence, and v0.12.24 added a strictly newer same-share/same-target/same-geometry frame after the app-share `ACTION` receipt. Version 0.12.25 retains those contracts and makes the Windows read-only handoff watcher portable to exact system PowerShell 5.1 by passing its marker paths explicitly instead of resolving a script function from a closure-created dynamic module. The chain is orchestration evidence, not a notification-only signal and not authority to control the product target.
