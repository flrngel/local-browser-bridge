...
A live run against the packaged 0.11.0 build proved the in-page Stop button end to end with a genuine dispatched mouse event: the lease was released, the global pause latched, remote start and observe were both refused with `HUMAN_CONTROL_PAUSED`, the page overlay was removed, and the bridge refused to click its own Stop button (`CONTROL_UI_OCCLUSION`) — 11 of 11 checks in [evidence/v0.11.0/README.md](evidence/v0.11.0/README.md). A separate stock-Chrome 151 run against the exact published 0.11.1 server and extension proved the browser-owned path with a PID-scoped native click: the warning and page pill were simultaneously visible, the unique **Cancel** button produced `canceled_by_user`, the lease and overlay disappeared, `humanPaused` latched, and remote restart was refused with HTTP 423 `HUMAN_CONTROL_PAUSED` — 11 of 11 checks in [evidence/v0.11.1/README.md](evidence/v0.11.1/README.md). The extension-popup **Release control** path and persistence across a full Chrome restart remain separate live-evidence boundaries.
...
### 0.11.2: recursive cross-origin frame discovery

A boundary run against the exact published 0.11.1 artifacts exposed a real depth-two defect: Chrome rendered a cross-origin iframe nested inside another cross-origin iframe, but the bridge observed only the first level. `Target.setAutoAttach` covers direct targets only, while 0.11.1 armed only the page target.

Version 0.11.2 recursively arms each child session only after its routing probe returns that child's own frame ID. Nested attach and detach events now enter the same lease, iframe-only, 16-session, five-level, and shared four-second observation bounds as direct children. A local packaged candidate passed all 22 live checks in Chrome for Testing 152: both OOPIF levels merged with accumulated offsets, a depth-two `page.click` landed as `isTrusted`, a same-process frame was reported, frame fill failed with `FRAME_ACTION_UNSUPPORTED`, and the session-routing fault injection stayed fail-closed. The published 0.11.1 failure and the explicitly non-release 0.11.2 candidate result are preserved together in [evidence/v0.11.1/README.md](evidence/v0.11.1/README.md).
...
- Open shadow roots are included in browser observations, and cross-origin iframe elements are merged into the same observation with top-level coordinates and `<generation>.f2.e5` refs that `page.click` and `page.hover` accept. The published 0.11.0 artifacts proved one depth-one OOPIF end to end. The exact published 0.11.1 boundary run then proved a top-level same-process skip and a live `FRAME_ACTION_UNSUPPORTED` refusal, but also exposed that depth-two OOPIF discovery was missing. Version 0.11.2 repairs that non-recursive attach path; a local packaged candidate merged both levels and landed a depth-two trusted click in Chrome for Testing 152 ([evidence/v0.11.1/README.md](evidence/v0.11.1/README.md)). Same-process frames remain unmerged, frame-scoped fill/select/key/scroll/typeText remain deferred, and the candidate run is not immutable release proof (see [docs/PROTOCOL.md](docs/PROTOCOL.md)).
- A pending JavaScript dialog blocks renderer-touching commands but no longer revokes the lease: version 0.10 shipped this gate and a live run proved it was still defeated by an in-flight observation (see [evidence/v0.10.0/README.md](evidence/v0.10.0/README.md)), and version 0.11 closes that path — the same scenario was re-run green on the packaged 0.11.0 build, with the dialog visible to the client, `page.observe` answering `BLOCKED_BY_DIALOG`, and the lease still active through `page.handleDialog` (see [evidence/v0.11.0/README.md](evidence/v0.11.0/README.md)). An explicit stop, a human pause, the lease TTL, and any failure seen with no dialog pending still revoke.
