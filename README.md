...
- Open shadow roots are included in browser observations, and cross-origin iframe elements are merged into the same observation with top-level coordinates and `<generation>.f2.e5` refs that `page.click` and `page.hover` accept; every unmerged frame is reported in `frameSummary.skipped`. A live run against the published 0.11.0 artifacts proved that path end to end on one genuine out-of-process iframe at depth 1 — the frame discovered and merged, its elements carrying their own origin, coordinates translated into the top-level viewport, and a click the child document saw as `isTrusted` ([evidence/v0.11.0/README.md](evidence/v0.11.0/README.md), 21 of 21 checks). It did not cover nested frames, a same-process iframe skip, or a Chrome that fails the session-routing probe; those stay harness-only. Same-process iframes and frame-scoped fill/select/key/scroll/typeText remain deferred (see `docs/PROTOCOL.md`).
