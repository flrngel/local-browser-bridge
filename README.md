...
The v0.12.22 release harness uses a create-once request/start/complete chain around one acceptance-only app-share button action. That chain is orchestration evidence, not a notification-only signal and not authority to control the product target.
...
> Version 0.12.22 is the current source and release target. Install only from an actual published Release after its macOS, Windows, stock-Chrome, evidence, and immutable-publication gates pass. Version 0.12.20 was withdrawn without a Release after its then-mandatory physical-pointer test timed out and its Windows action window was not discoverable. Version 0.12.21 was then withdrawn before candidate execution or publication when review found a pre-gate macOS verifier execution path and Windows CI found a nonportable filesystem assertion. The pointer lane is historical; its tools remain optional adversarial coverage and cannot satisfy v0.12.22. The current target instead uses an exact-app-share orchestration check, a non-executing pre-gate verifier, portable fail-closed marker checks, and one stable Windows sentinel title.
