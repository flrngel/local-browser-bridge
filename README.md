...
Version 0.12.22 introduced the create-once request/start/complete chain around one acceptance-only app-share button action, v0.12.23 separated sealed action-pointer evidence from keyboard-aware independent-system evidence, and v0.12.24 added a strictly newer same-share/same-target/same-geometry frame after the app-share `ACTION` receipt. Version 0.12.25 made the Windows read-only handoff watcher portable to exact system PowerShell 5.1 by passing its marker paths explicitly instead of resolving a script function from a closure-created dynamic module, but its PowerShell-hosted sentinel was not exposed by the Windows app-share surface. Version 0.12.26 retained the protocol contracts and launched a source-bound, ephemeral dedicated Windows GUI process so the acceptance surface had an application identity without turning that identity into product authority. The chain is orchestration evidence, not a notification-only signal and not authority to control the product target.
...
> Version 0.12.27 is the current source and release target. Install only from an actual published Release after its macOS, Windows, stock-Chrome, evidence, and immutable-publication gates pass. Version 0.12.20 was withdrawn without a Release after its then-mandatory physical-pointer test timed out and its Windows action window was not discoverable. Version 0.12.21 was withdrawn before candidate execution or publication when review found a pre-gate macOS verifier execution path and Windows CI found a nonportable filesystem assertion. The exact v0.12.22 candidate then passed its quiet-seat preflight and returned a Confirmed semantic action with safe sealed route, focus, pointer, and Space invariants, but the harness incorrectly sent that pointer-only action record through its keyboard-aware independent-system classifier and failed closed after 55 of 56 checks. Version 0.12.23 separated those two fail-closed classifiers without weakening either schema. Its quiet packaged lane passed 208/208 checks; its deliberate lane accepted the exact app-share start receipt and completed 89/89 recorded assertions, then reused a pre-handoff stream frame after 43.807 seconds and correctly received HTTP 409 `COMPUTER_STALE_FRAME` before product dispatch. No completion receipt, Windows, Chrome, publication, or Release followed. The exact negative record is retained on the immutable [v0.12.23 evidence commit](https://github.com/flrngel/local-browser-bridge/tree/4e4db75a4ede915d982d139a82dacac8a6c4772a/evidence/v0.12.23/computer/attempts/withdrawn-9e50811-macos-app-share-stale-frame). The v0.12.24 candidate reached the interactive Windows handoff, then its read-only watcher failed closed before any operator action because exact system PowerShell 5.1 could not resolve the atomic marker reader from a closure-created dynamic module; Chrome and publication did not follow. Version 0.12.25 established the explicit-argument watcher callback, passed its packaged macOS lanes, then timed out before Windows product actions because the app-share could not discover its terminal-hosted sentinel; Chrome and publication did not follow, and its waiting release job was canceled. Version 0.12.26 attempt 1 bound successfully and ran only the macOS quiet lane; externally observed shared-seat HID pointer activity during `computer.typeText` failed that lane, so the evidence was [preserved](https://github.com/flrngel/local-browser-bridge/tree/b1ba6a3bb77bc467e352716f099febb7c17fe767/evidence/v0.12.26/computer/attempts/withdrawn-0430e29-macos-quiet-pointer-contamination) and the attempt was canceled. Attempt 2 rebuilt the candidate but stopped before execution: the byte-identical extension legitimately returned one valid attestation from each attempt, while the verifier incorrectly required every returned statement to name the current attempt. That failure was also [preserved](https://github.com/flrngel/local-browser-bridge/tree/596e257d5d49845d7ca4f40e4f8282c99aba5687/evidence/v0.12.26/computer/attempts/withdrawn-0430e29-candidate-attestation-selection-mismatch) and canceled. Neither attempt reached Windows, stock Chrome, or a public Release. Version 0.12.27 changes only release tooling: every returned attestation must still be well formed and bind the same run, source, tag, workflow, GitHub-hosted runner, and exact subject, while exactly one statement must bind the current attempt. Zero, duplicate, malformed, or wrong-subject results fail closed. Local deployment also verifies an exact five-file staging directory and atomically replaces only a proven generated-artifact `dist/`, retaining rollback state on failure. Product, protocol, and platform behavior are unchanged. The historical physical-pointer tools remain optional adversarial coverage and cannot satisfy v0.12.27.
