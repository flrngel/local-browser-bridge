...
Version 0.12.22 introduced the create-once request/start/complete chain around one acceptance-only app-share button action, and v0.12.23 retained that surface while separating sealed action-pointer evidence from keyboard-aware independent-system evidence. Version 0.12.24 retains both contracts and, after the app-share `ACTION` receipt, requires a strictly newer frame from the same share, target, and geometry within the reserved deadline before it derives product click authority. The chain is orchestration evidence, not a notification-only signal and not authority to control the product target.
