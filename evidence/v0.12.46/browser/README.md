# Windows v0.12.46 stock-Chrome acceptance

This directory defines the release gate for the v0.12.46 browser extension and
...
0.12.46 aligns the bounded acceptance inventory and tests with the shipped
...
approval, or public Release. Version 0.12.46 retains the exact-attempt trust
...
0.12.46 retains that Job topology and moves the schema-2 reservation to the
...
and cannot satisfy any v0.12.46 browser gate.
...
and cannot satisfy or be reused by any v0.12.46 browser gate.
...
No v0.12.33 byte, result, screenshot, or marker can satisfy v0.12.46.
...
messages, and evidence cannot be reused for v0.12.46. An interrupted or failed
v0.12.46 attempt is preserved only as sanitized negative metadata and is never
...
1. the installed v0.12.46 extension and Chrome's debugger-use indicator;
...
3. perform only required conditional toggles and load the verified directory
   ...
   is changed;
4. verify exactly one new test-owned card, version 0.12.46, no load errors, and
