# Windows v0.12.14 stock-Chrome acceptance

This directory defines the release gate for the v0.12.14 browser extension and
...
Version v0.12.12 was withdrawn after three macOS deliberate-concurrency
attempts stopped before product dispatch: one at the shared arm deadline and
two on pre-dispatch pointer/input contamination. Windows and stock-Chrome
acceptance never completed, publication remained blocked, and no v0.12.12
Release exists.

Version v0.12.13 was withdrawn after its quiet macOS lane completed 192 of 193
assertions. The unchanged whole-run boundary detected unrelated shared-seat
`mouseMoved`/cursor activity, so that lane remained negative and was not
attributed to the helper. Deliberate macOS, Windows helper, and stock-Chrome
acceptance never started; publication was canceled and no v0.12.13 Release
exists. Its negative record remains only on branch
`evidence/v0.12.13-macos-quiet-pointer-contamination-32695400912` at commit
`bdcc3620e28260e31a3a78bf7e584adf1f0db44e`, path
...
or generated evidence byte may be reused for v0.12.14. A failed or interrupted
...
```powershell
$trustDestination = Join-Path ([IO.Path]::GetTempPath()) ("lbb-1214-" + [Guid]::NewGuid().ToString("N"))
...
From the exact clean v0.12.14 checkout, invoke:
