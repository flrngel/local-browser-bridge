...
const EXPECTED_VERSION = "0.12.14";
...
const FIXTURE_TITLE = "LBB v0.12.14 Persistent SCStream Evidence";
const SIBLING_FIXTURE_TITLE = "LBB v0.12.14 Same-PID Sibling Receiver";
const SEMANTIC_VALUE = "v0.12.14-semantic-value";
...
function quietSeatTransitionDisposition(before, after) {
  ...
  const pointerState = pointerLaneState(invariants);
...
async function main() {
  ...
  scratchDir = await mkdtemp(join(scratchParent, "lbb-v0.12.14-scstream-"));
  ...
  const result = {
    ...
    candidateNotice: "This becomes release evidence only after the supplied checksum manifest and archive are published immutably for v0.12.14.",
    ...
    limitations: [
      "This run proves the supplied v0.12.14 macOS release-candidate archive, not an immutable GitHub release until those exact checksums are published.",
      "Each lane starts candidate execution only after a 30-second, 60-transition native SystemProbe epoch with cumulative pointer counters and sampled foreground, focus, cursor, and active-Space equality. The gate reduces pre-existing shared-seat contamination but cannot reserve the active login seat; all unchanged per-action and whole-run proofs still fail closed on later activity.",
...
if (preparePackageMode) {
  ...
  try {
    ...
    if (scratchDir?.startsWith(`${scratchParent}/lbb-v0.12.14-scstream-`)) {
