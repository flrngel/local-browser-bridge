...
const EXPECTED_VERSION = "0.12.11";
...
const FIXTURE_TITLE = "LBB v0.12.11 Persistent SCStream Evidence";
const SIBLING_FIXTURE_TITLE = "LBB v0.12.11 Same-PID Sibling Receiver";
const SEMANTIC_VALUE = "v0.12.11-semantic-value";
...
if (
  ...
  console.error(
    "Usage: node helper-evidence-rig.mjs <server> <helper> <output-dir> <scratch-parent> <macos-archive> <SHA256SUMS.txt> <expected-SHA256SUMS-sha256> [quiet|deliberate-concurrency]",
  );
  process.exit(2);
}
if (!POINTER_EVIDENCE_LANES.has(pointerEvidenceLane)) {
...
const pointerHandoffSource = resolve(outputDir, "PointerHandoff.swift");
...
async function main() {
  ...
  scratchDir = await mkdtemp(join(scratchParent, "lbb-v0.12.11-scstream-"));
  ...
  const result = {
    ...
    candidateNotice: "This becomes release evidence only after the supplied checksum manifest and archive are published immutably for v0.12.11.",
    ...
    limitations: [
      "This run proves the supplied v0.12.11 macOS release-candidate archive, not an immutable GitHub release until those exact checksums are published.",
...
try {
  ...
  if (scratchDir?.startsWith(`${scratchParent}/lbb-v0.12.11-scstream-`)) {
