# macOS v0.12.11 persistent-share evidence candidate

This directory defines a fresh deterministic evidence harness for the packaged
macOS v0.12.11 server and helper candidate. It is protocol infrastructure, not
passing evidence; no v0.12.11 packaged run exists yet.
...
exists. Version 0.12.11 does not reuse or relabel any prior binary, screenshot,
result, marker, notification, log, or other generated evidence byte.
...
- The server and helper report exactly v0.12.11, are universal
...
- The supplied executables are byte-for-byte identical to the copies inside
  `local-browser-bridge-v0.12.11-macos-universal.tar.gz`. The archive must be
  bound by a canonical `SHA256SUMS.txt` containing exactly the four v0.12.11
...
counter advances spanning at least 500 ms. Any button, drag, scroll, or tablet
counter advance invalidates the gate. It then changes the panel to `ACTION
RUNNING`, takes a new independent baseline, and immediately starts one bounded
two-second target-routed `computer.click`. Keep moving without clicking until
the panel turns green. Entering `ACTION` starts a separate 10-second hard grace,
capped by a 310-second total prompt lifetime; the prompt exits at the earlier
deadline even after completion.
...
has Screen Recording and Accessibility permission. In the `quiet` lane, do not
interact with the foreground app, pointer, or Spaces. In the
`deliberate-concurrency` lane, the only permitted interaction is the continuous,
click-free pointer motion requested by the orange `MOVE` panel; do not touch the
foreground app or Spaces. An invariant failure is a real negative result and
must be retained. The runner refuses to overwrite any existing result, log,
operator marker, or expected screenshot. Move a completed or failed attempt to
a separately named evidence directory before starting another one.
...
PACKAGE_ROOT="$(mktemp -d "$SCRATCH_PARENT/lbb-v0.12.11-package.XXXXXX")"
trap 'rm -rf -- "$PACKAGE_ROOT"' EXIT
tar -xzf "$RELEASE_CANDIDATE_DIR/local-browser-bridge-v0.12.11-macos-universal.tar.gz" \
  -C "$PACKAGE_ROOT"

EVIDENCE_DIR="$(pwd)/evidence/v0.12.11/computer"
node evidence/v0.12.11/computer/helper-evidence-rig.mjs \
  "$PACKAGE_ROOT/local-browser-bridge" \
  "$PACKAGE_ROOT/Local Computer Helper.app/Contents/MacOS/local-computer-helper" \
  "$EVIDENCE_DIR" \
  "$SCRATCH_PARENT" \
  "$RELEASE_CANDIDATE_DIR/local-browser-bridge-v0.12.11-macos-universal.tar.gz" \
  "$RELEASE_CANDIDATE_DIR/SHA256SUMS.txt" \
  "$EXPECTED_SHA256SUMS_SHA256" \
  deliberate-concurrency &
RUNNER_PID=$!

# This watcher is a read-only notification relay, never acceptance authority.
# Keep this coordinator shell open; interact only when the orange panel asks
# for continuous click-free pointer motion.
WATCHER_STATUS=0
...
created. These records are diagnostic history, not v0.12.11 evidence:
...
The v0.12.2 candidate run is preserved byte-for-byte and is not v0.12.11
evidence:
...
They are linked for diagnostic history only and are not v0.12.11 results:
