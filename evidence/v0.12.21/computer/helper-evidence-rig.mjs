...
const EXPECTED_VERSION = "0.12.21";
...
const FIXTURE_TITLE = "LBB v0.12.21 Persistent SCStream Evidence";
const SIBLING_FIXTURE_TITLE = "LBB v0.12.21 Same-PID Sibling Receiver";
const SEMANTIC_VALUE = "v0.12.21-semantic-value";
...
const APP_SHARE_HANDOFF_COMPLETION_GRACE_MS = 10_000;
const APP_SHARE_HANDOFF_MINIMUM_ACTION_BUDGET_MS = 5_000;
...
async function publishAtomicMarkerOnce(path, marker, timeoutMs = MARKER_PUBLISH_TIMEOUT_MS) {
  ...
  try {
    ...
    if (
      ...
      descriptorState.size !== Buffer.byteLength(serialized, "utf8") ||
    ...
    if (
      linkedTemporaryState.nlink !== 2 || destinationState.nlink !== 2 ||
    ...
    if (publishedState.nlink !== 1 || !sameCoreFileIdentity(destinationState, publishedState)) {
...
async function readBoundAppShareReceipt(path, expectedKeys, validate, description) {
  ...
  try {
    ...
    if (
      ...
      descriptorBefore.size < 1 || descriptorBefore.size > 16 * 1024 ||
      (descriptorBefore.mode & 0o077) !== 0 ||
    ...
    if (
      ...
      (descriptorAfter.mode & 0o077) !== 0 ||
      (typeof process.getuid === "function" && descriptorAfter.uid !== process.getuid()) ||
...
function pointerLaneAccepted(invariants) {
  return recordPointerLane(invariants) === "quiet";
}
...
async function startPointerHandoff(presentationBaseline) {
  ...
  requireCheck(
    ...
    pointerLaneState(readyPresentationInvariants) === "quiet",
...
async function completePointerHandoff(pointerHandoffCompletionBaseline) {
  ...
  requireCheck(
    ...
    pointerLaneState(completionInvariants) === "quiet",
  ...
  requireCheck(
    ...
    pointerLaneState(completionPresentationInvariants) === "quiet",
...
async function main() {
  ...
  scratchDir = await mkdtemp(join(scratchParent, "lbb-v0.12.21-scstream-"));
  ...
  // an independently quiet shared-seat boundary, and the ACTION presentation.
  ...
  if (pointerEvidenceLane === "deliberate-concurrency") {
    pointerHandoffProductBoundaryQuiet = pointerLaneState(postResizeClick.invariants) === "quiet";
    pointerHandoffIndependentBoundaryQuiet =
      pointerLaneState(postResizeActionInvariants) === "quiet";
    requireCheck(
  ...
  const result = {
    ...
    candidateNotice: "This becomes release evidence only after the supplied checksum manifest and archive are published immutably for v0.12.21.",
    ...
    limitations: [
      "This run proves the supplied v0.12.21 macOS release-candidate archive, not an immutable GitHub release until those exact checksums are published.",
...
if (preparePackageMode) {
  ...
  try {
    ...
    if (scratchDir?.startsWith(`${scratchParent}/lbb-v0.12.21-scstream-`)) {
