# macOS v0.12.21 persistent-share evidence candidate

This directory defines a fresh deterministic evidence harness for the packaged
macOS v0.12.21 server and helper candidate. It is protocol infrastructure, not
passing evidence; no v0.12.21 packaged run exists yet.
...
canceled. Version 0.12.21 does not reuse or relabel any prior binary,
...
tag remains protected historical metadata. Version 0.12.21 does not reuse any
...
- The server and helper report exactly v0.12.21, are universal
...
- The supplied executables are byte-for-byte identical to the copies inside
  `local-browser-bridge-v0.12.21-macos-universal.tar.gz`. The archive must be
  bound by a canonical `SHA256SUMS.txt` containing exactly the four v0.12.21
...
open a ten-second `ACTION` interval and send the real two-second target-routed
`computer.click`. The exact fixture postcondition must advance while both the
product and independent shared-seat boundaries remain quiet. The runner then
requests `COMPLETE`; the app emits a completion receipt bound to the request
hash, start-receipt hash, and product timestamps. Its
...
Computer Use provider. The schema-7 result records only the bound receipt chain
plus sampled native SystemProbe boundaries and sets
`orchestrationNotProductControl: true`. The request/start/complete chain is
...
`v0.12.21` tag. The source checkout and every candidate/evidence directory
...
ATTEMPT_ROOT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.21-macos.XXXXXX")"
SCRATCH_PARENT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.21-scratch.XXXXXX")"
PACKAGE_ROOT="$PRIVATE_PARENT/lbb-v0.12.21-package-$RUN_NONCE"
...
ARCHIVE="$RELEASE_CANDIDATE_DIR/local-browser-bridge-v0.12.21-macos-universal.tar.gz"
MANIFEST="$RELEASE_CANDIDATE_DIR/SHA256SUMS.txt"
[[ ! -e "$PACKAGE_ROOT" && ! -L "$PACKAGE_ROOT" ]]
node evidence/v0.12.21/computer/helper-evidence-rig.mjs \
  --prepare-package \
...
pixels. In order, verify: ready with empty/zero counters; the v0.12.21 semantic
...
the v0.12.21 semantic value; semantic action 1/complete; persistent stream;
...
AGGREGATE_DIR="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.21-aggregate.XXXXXX")"
...
jq -e \
  ...
  --arg deliberate_sha256 "$DELIBERATE_REVIEWED_RESULT_SHA256" '
    ...
    .aggregateChecks.passingResultSchemaVersion == 7 and
...
`evidence/v0.12.21/release/run-RUN_ID-attempt-ATTEMPT/macos/` on
`evidence/v0.12.21-release-run-RUN_ID-attempt-ATTEMPT`; its single commit parent
...
created. These records are diagnostic history, not v0.12.21 evidence:
...
The v0.12.2 candidate run is preserved byte-for-byte and is not v0.12.21
evidence:
...
They are linked for diagnostic history only and are not v0.12.21 results:
