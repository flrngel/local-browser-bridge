# macOS v0.12.19 persistent-share evidence candidate

This directory defines a fresh deterministic evidence harness for the packaged
macOS v0.12.19 server and helper candidate. It is protocol infrastructure, not
passing evidence; no v0.12.19 packaged run exists yet.
...
canceled. Version 0.12.19 does not reuse or relabel any prior binary,
...
tag remains protected historical metadata. Version 0.12.19 does not reuse any
...
- The server and helper report exactly v0.12.19, are universal
...
- The supplied executables are byte-for-byte identical to the copies inside
  `local-browser-bridge-v0.12.19-macos-universal.tar.gz`. The archive must be
  bound by a canonical `SHA256SUMS.txt` containing exactly the four v0.12.19
...
The retained schema-v6 result contains only bounded booleans, counts, and state
...
stage `waitDeliberatePointerActivity` and `actionDispatched: false`.
Schema v6 uses a tri-state dispatch field: `false` before the command request, `null`
...
`v0.12.19` tag. The source checkout and every candidate/evidence directory
...
ATTEMPT_ROOT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.19-macos.XXXXXX")"
SCRATCH_PARENT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.19-scratch.XXXXXX")"
PACKAGE_ROOT="$PRIVATE_PARENT/lbb-v0.12.19-package-$RUN_NONCE"
...
ARCHIVE="$RELEASE_CANDIDATE_DIR/local-browser-bridge-v0.12.19-macos-universal.tar.gz"
MANIFEST="$RELEASE_CANDIDATE_DIR/SHA256SUMS.txt"
[[ ! -e "$PACKAGE_ROOT" && ! -L "$PACKAGE_ROOT" ]]
node evidence/v0.12.19/computer/helper-evidence-rig.mjs \
  --prepare-package \
...
jq -e '
  ...
  .operatorHandoff.requested == false and
  .operatorHandoff.requestPublicationAcknowledged == false and
  .operatorHandoff.completePublicationAcknowledged == false and
  .operatorHandoff.promptClosed == false and
  .operatorHandoff.clickFreeMotionObserved == false and
  .operatorHandoff.actionDispatched == false and
  .operatorHandoff.productBoundaryContaminated == false and
  .operatorHandoff.independentBoundaryContaminated == false and
  .operatorHandoff.sustainedMotionSamples == 0 and
  .operatorHandoff.sustainedMotionSpanMilliseconds == 0 and
  .operatorHandoff.markerNotificationOnly == true and
  .operatorHandoff.markerAcceptedAsAuthority == false and
  .operatorHandoff.externalAcknowledgementConsumed == false and
  .operatorHandoff.rawPromptIdentityRetainedInResult == false and
  .operatorHandoff.rawPointerDataRetained == false and
  ([.screenshots[].file] == [
...
pixels. In order, verify: ready with empty/zero counters; the v0.12.19 semantic
...
jq -e '
  ...
  .quietSeatStabilization.monitoringUnknown == false and
  .pointerEvidence.quietObserved == true and
  .pointerEvidence.concurrentSharedSeatActivityObserved == true and
  .pointerEvidence.unknownObserved == false and
  .operatorHandoff.requested == true and
  .operatorHandoff.requestPublicationAcknowledged == true and
  .operatorHandoff.completePublicationAcknowledged == true and
  .operatorHandoff.promptClosed == true and
  .operatorHandoff.clickFreeMotionObserved == true and
  .operatorHandoff.actionDispatched == true and
  .operatorHandoff.productBoundaryContaminated == true and
  .operatorHandoff.independentBoundaryContaminated == true and
  .operatorHandoff.sustainedMotionSamples >= 3 and
  .operatorHandoff.sustainedMotionSpanMilliseconds >= 500 and
  .operatorHandoff.sustainedMotionSpanMilliseconds <= 300000 and
  .operatorHandoff.markerNotificationOnly == true and
  .operatorHandoff.markerAcceptedAsAuthority == false and
  .operatorHandoff.externalAcknowledgementConsumed == false and
  .operatorHandoff.rawPromptIdentityRetainedInResult == false and
...
the v0.12.19 semantic value; semantic action 1/complete; persistent stream;
...
AGGREGATE_DIR="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.19-aggregate.XXXXXX")"
...
jq -e \
  ...
  --arg deliberate_sha256 "$DELIBERATE_REVIEWED_RESULT_SHA256" '
    ...
    .aggregateChecks.passingResultSchemaVersion == 6 and
    .aggregateChecks.inventoryFileCount == 18 and
...
The retained macOS subtree is exactly 19 files: one aggregate, eight quiet-lane
files, and ten deliberate-lane files. Store it at
`evidence/v0.12.19/release/run-RUN_ID-attempt-ATTEMPT/macos/` on
`evidence/v0.12.19-release-run-RUN_ID-attempt-ATTEMPT`; its single commit parent
must be the tagged source and every retained file must be an ordinary `100644`
blob. The aggregate's `inventoryFileCount: 18` counts both lane inventories and
...
created. These records are diagnostic history, not v0.12.19 evidence:
...
The v0.12.2 candidate run is preserved byte-for-byte and is not v0.12.19
evidence:
...
They are linked for diagnostic history only and are not v0.12.19 results:
