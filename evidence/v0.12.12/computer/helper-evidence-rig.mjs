...
const EXPECTED_VERSION = "0.12.12";
...
const FIXTURE_TITLE = "LBB v0.12.12 Persistent SCStream Evidence";
const SIBLING_FIXTURE_TITLE = "LBB v0.12.12 Same-PID Sibling Receiver";
const SEMANTIC_VALUE = "v0.12.12-semantic-value";
...
const POINTER_HANDOFF_MINIMUM_ACTION_BUDGET_MS = 5_000;
const POINTER_HANDOFF_COMPLETION_RESERVE_MS = 3_000;
...
const POINTER_HANDOFF_MARKER_SCHEMA = 1;
...
if (!preparePackageMode && (
  ...
  !expectedTagObjectSha || !expectedWorkflowRunId || !expectedWorkflowRunAttempt ||
  !expectedArtifactId || !expectedArtifactZipSha256
)) {
  console.error(
    "Usage: node helper-evidence-rig.mjs <server> <helper> <fresh-output-dir> <scratch-parent> <macos-archive> <SHA256SUMS.txt> <expected-SHA256SUMS-sha256> <expected-source-sha> <expected-tag-object-sha> <expected-workflow-run-id> <expected-workflow-run-attempt> <expected-artifact-id> <expected-artifact-zip-sha256> [quiet|deliberate-concurrency]",
  );
  process.exit(2);
}
for (const [label, value, pattern] of preparePackageMode ? [] : [
  ...
  ["tag object SHA", expectedTagObjectSha, /^[0-9a-f]{40}$/],
...
const pointerHandoffSource = resolve(rigSourceDirectory, "PointerHandoff.swift");
...
const pointerHandoffRequestPath = join(operatorDirectory, POINTER_HANDOFF_REQUEST_FILE);
const pointerHandoffCompletePath = join(operatorDirectory, POINTER_HANDOFF_COMPLETE_FILE);
...
let pointerHandoffSustainedMotionSamples = 0;
let pointerHandoffSustainedMotionSpanMilliseconds = 0;
let pointerHandoffClickFreeArmObserved = false;
let pointerHandoffClickFreeActionObserved = false;
let pointerHandoffClickFreeCompletionObserved = false;
let pointerHandoffActionDispatched = false;
let pointerHandoffProductBoundaryContaminated = false;
let pointerHandoffIndependentBoundaryContaminated = false;
...
function assertNoRetainedPointerRawData(text, label) {
  for (const rawField of ["\"cursorX\":", "\"cursorY\":", "\"hidPointerCounters\":"]) {
...
function run(commandName, args, timeoutMs = SUBPROCESS_TIMEOUT_MS) {
  ...
  if (result.error?.code === "ETIMEDOUT" || result.signal === "SIGKILL") {
    throw new Error(`${basename(commandName)} exceeded its bounded execution deadline`);
  }
  if (result.error) {
...
function verifyHarnessSourceBinding(stage) {
  ...
  const tag = `v${EXPECTED_VERSION}`;
  const tagObjectSha = run("git", ["-C", sourceRoot, "rev-parse", tag]);
  const peeledTagSha = run("git", ["-C", sourceRoot, "rev-parse", `${tag}^{}`]);
  const tagObjectType = run("git", ["-C", sourceRoot, "cat-file", "-t", tagObjectSha]);
  ...
  requireCheck(`${stage} harness annotated tag object matches coordinator binding`,
    tagObjectType === "tag" && tagObjectSha === expectedTagObjectSha && peeledTagSha === sourceSha,
    tagObjectType === "tag" && tagObjectSha === expectedTagObjectSha && peeledTagSha === sourceSha
      ? "annotated tag object and peel matched"
      : "annotated tag object or peel mismatch");
  ...
  harnessSourceBinding.annotatedTagObjectSha = tagObjectSha;
...
async function publishAtomicMarkerOnce(path, marker, timeoutMs = MARKER_PUBLISH_TIMEOUT_MS) {
  ...
  try {
    ...
    await handle.close();
    handle = null;
    // macOS renameatx_np(RENAME_EXCL), exposed by the already-bound runner
    // prompt utility, publishes one complete singly linked file atomically and
    // refuses an existing destination without a check/rename race.
    run(
...
async function writePointerHandoffState(state) {
  if (
    ![
    ...
    throw new Error("refusing an invalid pointer handoff prompt state");
  }
...
function pointerHandoffSummary() {
  return {
    ...
    sustainedMotionSamples: pointerHandoffSustainedMotionSamples,
    sustainedMotionSpanMilliseconds: pointerHandoffSustainedMotionSpanMilliseconds,
    clickFreeMotionObserved:
      pointerHandoffClickFreeArmObserved &&
      pointerHandoffClickFreeActionObserved &&
      pointerHandoffClickFreeCompletionObserved,
    actionDispatched: pointerHandoffActionDispatched,
    productBoundaryContaminated: pointerHandoffProductBoundaryContaminated,
    independentBoundaryContaminated: pointerHandoffIndependentBoundaryContaminated,
...
function pointerHandoffRequestMarker(promptPid) {
  ...
  return {
    schemaVersion: POINTER_HANDOFF_MARKER_SCHEMA,
...
async function bindCanonicalChecksumManifest(expectedSha256) {
  ...
  try {
    ...
    const descriptorBefore = await manifestHandle.stat();
    const pathBefore = await lstat(sumsPath);
    requireCheck(
      ...
      descriptorBefore.isFile() && pathBefore.isFile() && !pathBefore.isSymbolicLink() &&
        descriptorBefore.nlink === 1 && sameOrdinaryFileIdentity(descriptorBefore, pathBefore) &&
        descriptorBefore.size > 0 && descriptorBefore.size <= MAX_CHECKSUM_MANIFEST_BYTES &&
        (descriptorBefore.mode & 0o022) === 0 &&
        (typeof process.getuid !== "function" || descriptorBefore.uid === process.getuid()),
      "ordinary, singly linked, owner-controlled, and within the manifest byte bound",
    );
    bytes = await manifestHandle.readFile();
    const descriptorAfter = await manifestHandle.stat();
    const pathAfter = await lstat(sumsPath);
    requireCheck(
      "checksum manifest stayed stable while it was bound",
      bytes.length === descriptorBefore.size &&
...
function pointerPromptDeliveryObserved(sample) {
  return (
    sample?.pointerPromptProbeRequested === true &&
    sample?.pointerPromptOwnerMatched === true &&
    sample?.pointerPromptTitleMatched === true &&
    sample?.pointerPromptOnScreen === true &&
    sample?.pointerPromptNonactivating === true
  );
}
...
function systemInvariants(before, after) {
  ...
  return {
    ...
    ...classifySharedPointerBoundary(before, after),
...
function actionBoundSystemInvariants(before, after) {
  return {
    ...systemInvariants(before, after),
    ...classifyActionBoundSharedPointerBoundary(before, after),
  };
}
...
function pointerLaneState(invariants) {
  ...
  if (state === "contaminated" && invariants.sharedPointerActivityObserved === true) {
    return "concurrent-shared-seat-activity";
  }
  return "unknown";
}

function recordPointerLane(invariants) {
  const state = pointerLaneState(invariants);
...
function pointerLaneAccepted(invariants) {
  const state = recordPointerLane(invariants);
  return pointerEvidenceLane === "quiet"
    ? state === "quiet"
    : state === "quiet" || state === "concurrent-shared-seat-activity";
}
...
function allIndependentInvariantsHeld(invariants) {
  return [
    ...
    pointerLaneAccepted(invariants),
...
async function startPointerHandoff(presentationBaseline) {
  ...
  if (pointerHandoffProcess || pointerHandoffRequestPublicationAcknowledged) {
    throw new Error("the one-shot pointer handoff was already started");
  }
  await mkdir(operatorDirectory, { recursive: false, mode: 0o700 });
  pointerHandoffRequestId = randomBytes(16).toString("hex");
  pointerHandoffArmDeadlineMilliseconds = Date.now() + DELIBERATE_CONCURRENCY_WAIT_MS;
  pointerHandoffHardDeadlineMilliseconds =
    pointerHandoffArmDeadlineMilliseconds + POINTER_HANDOFF_COMPLETION_GRACE_MS;
  ...
  if (
    ...
    throw new Error("the pointer handoff prompt did not receive a distinct process identity");
  }

  const waitingPromptSample = await waitForPointerPromptState(POINTER_HANDOFF_WAITING_STATE);
  const waitingPresentationInvariants = systemInvariants(
    presentationBaseline,
    waitingPromptSample,
  );
  requireIndependentInvariants("pointer handoff WAITING presentation", waitingPresentationInvariants);
  requireCheck(
    "pointer handoff WAITING presentation remained quiet",
    pointerLaneState(waitingPresentationInvariants) === "quiet",
    "the runner-owned notification appeared without shared-pointer activity",
  );

  await writePointerHandoffState(POINTER_HANDOFF_MOVE_STATE);
  const movePromptSample = await waitForPointerPromptState(POINTER_HANDOFF_MOVE_STATE);
  const movePresentationInvariants = systemInvariants(waitingPromptSample, movePromptSample);
  requireCheck(
    "pointer handoff MOVE transition preserved foreground, focus, and Space",
    movePresentationInvariants.foregroundUnchanged === true &&
      movePresentationInvariants.userFocusUnchanged === true &&
      movePresentationInvariants.spaceUnchanged === true,
    "immediate post-orange pointer motion is tolerated without weakening non-interruption",
  );

  const requestMarker = pointerHandoffRequestMarker(pointerHandoffProcess.pid);
  await publishAtomicMarkerOnce(pointerHandoffRequestPath, requestMarker);
  pointerHandoffRequestPublicationAcknowledged = true;
  if (
    pointerHandoffProcess.exitCode !== null ||
    pointerHandoffProcess.signalCode !== null
  ) {
    throw new Error("the pointer handoff prompt exited before the operator notification");
  }
  log(
    "ACTION REQUIRED: the orange macOS pointer handoff panel is visible; move continuously without clicking until it turns green.",
  );
  return movePromptSample;
}
...
async function waitForDeliberatePointerActivity(baseline) {
  ...
  if (!pointerHandoffRequestPublicationAcknowledged || !pointerHandoffProcess) {
    throw new Error("the deliberate pointer wait requires a published one-shot handoff request");
  }
  ...
  while (Date.now() < pointerHandoffArmDeadlineMilliseconds) {
    ...
    remainingPointerHandoffTime(pointerHandoffArmDeadlineMilliseconds, "arm probe");
    ...
    if (progress === "disallowed") {
      throw new Error("click, drag, scroll, or tablet activity invalidated the pointer-motion arm");
    }
    ...
    if (progress === "advanced") {
      ...
      if (
        ...
        if (actionTransitionProgress === "unknown") {
          throw new Error("the independent HID pointer monitor became unreadable during the ACTION transition");
        }
        if (actionTransitionProgress === "disallowed") {
          throw new Error("click, drag, scroll, or tablet activity invalidated the ACTION transition");
        }
...
async function completePointerHandoff(pointerHandoffCompletionBaseline) {
  ...
  if (
    ...
    !pointerHandoffProductBoundaryContaminated ||
    !pointerHandoffIndependentBoundaryContaminated ||
    !pointerHandoffClickFreeArmObserved ||
    !pointerHandoffClickFreeActionObserved ||
    pointerHandoffSustainedMotionSamples < DELIBERATE_MOTION_REQUIRED_SAMPLES ||
    pointerHandoffSustainedMotionSpanMilliseconds < DELIBERATE_MOTION_MINIMUM_SPAN_MS
  ) {
    throw new Error("the pointer handoff completion preconditions were not satisfied");
  }
  ...
  const completionProgress = clickFreePointerMotionProgress(
    pointerHandoffCompletionBaseline,
    completionBoundary,
  );
  if (completionProgress === "unknown") {
    throw new Error("the independent HID pointer monitor became unreadable before completion");
  }
  if (completionProgress === "disallowed") {
    throw new Error("click, drag, scroll, or tablet activity invalidated the completion boundary");
  }
  if (completionProgress !== "advanced") {
    throw new Error("click-free mouse movement stopped before the panel completed");
  }
  ...
  if (completionPresentationProgress === "unknown") {
    throw new Error("the independent HID pointer monitor became unreadable before the green completion state");
  }
  if (completionPresentationProgress === "disallowed") {
    throw new Error("click, drag, scroll, or tablet activity invalidated the green completion transition");
  }
  if (completionPresentationProgress !== "advanced") {
    throw new Error("click-free mouse movement stopped before the panel turned green");
  }
  ...
  const completeMarker = pointerHandoffCompleteMarker(pointerHandoffProcess.pid);
  if (completeMarker.createdAt < pointerHandoffRequestCreatedAt) {
    throw new Error("the pointer handoff completion predates its request");
  }
  await publishAtomicMarkerOnce(
    pointerHandoffCompletePath,
    completeMarker,
    Math.min(MARKER_PUBLISH_TIMEOUT_MS, remainingCompletionMs),
  );
  ...
  const promptTeardown = await terminate(pointerHandoffProcess, "pointer handoff prompt");
  ...
  if (!pointerHandoffPromptClosed) {
    throw new Error("the pointer handoff prompt was not closed by the runner");
  }
}
...
async function main() {
  ...
  scratchDir = await mkdtemp(join(scratchParent, "lbb-v0.12.12-scstream-"));
  const fixtureBinary = join(scratchDir, "helper-evidence-fixture");
  systemProbeBinary = join(scratchDir, "system-probe");
  pointerHandoffBinary = join(scratchDir, "pointer-handoff");
  ...
  const harnessSha256 = {
    ...
    pointerHandoff: await sha256(pointerHandoffSource),
  ...
  requireCheck(
    "pointer handoff prompt self-test",
    run(pointerHandoffBinary, ["--self-test"]) ===
      "macOS pointer handoff prompt self-test passed",
    "fixed WAITING/MOVE/COMPLETE vocabulary and titles",
  );
  ...
  const postResizeActionDurationMs = pointerEvidenceLane === "deliberate-concurrency"
    ? DELIBERATE_CONCURRENCY_ACTION_MS
  ...
  if (pointerEvidenceLane === "deliberate-concurrency") {
    if (
      ...
      actionResponseTimeoutMs < POINTER_HANDOFF_MINIMUM_ACTION_BUDGET_MS
    ) {
      throw new Error("insufficient bounded action time remained after the pointer-motion arm");
    }
  ...
  if (
    ...
    throw new Error("the pointer handoff panel was not independently visible immediately after dispatch");
  }
  if (pointerEvidenceLane === "deliberate-concurrency") {
    const clickFreeActionProgress = clickFreePointerMotionProgress(
      postResizeSystemBefore,
      postResizeSystemAfter,
    );
    if (clickFreeActionProgress === "unknown") {
      throw new Error("the independent HID pointer monitor became unreadable across the action boundary");
    }
    if (clickFreeActionProgress === "disallowed") {
      throw new Error("click, drag, scroll, or tablet activity invalidated the action boundary");
    }
    pointerHandoffClickFreeActionObserved = clickFreeActionProgress === "advanced";
    requireCheck(
      "click-free mouse movement crossed the independent action boundary",
      pointerHandoffClickFreeActionObserved,
      "mouseMoved advanced while button, drag, scroll, and tablet counters stayed stable",
    );
  }
  ...
  if (pointerEvidenceLane === "deliberate-concurrency") {
    ...
    requireCheck(
      ...
      pointerHandoffProductBoundaryContaminated &&
        pointerHandoffIndependentBoundaryContaminated,
      "both post-resize boundaries reported bounded shared-pointer contamination",
    );
  ...
  if (pointerEvidenceLane === "deliberate-concurrency") {
    requireCheck(
      "operator handoff markers were published and the prompt was closed before success",
      pointerHandoffRequestPublicationAcknowledged &&
        pointerHandoffCompletePublicationAcknowledged &&
        pointerHandoffPromptClosed &&
        pointerHandoffProcess === null &&
      ...
      "the runner published both create-once notifications and completed prompt teardown",
    );
  }
  ...
  const result = {
    ...
    candidateNotice: "This becomes release evidence only after the supplied checksum manifest and archive are published immutably for v0.12.12.",
    ...
    pointerEvidence,
    operatorHandoff: pointerHandoffSummary(),
    environment: {
    ...
    harness: {
      ...
      pointerHandoffSha256: harnessSha256.pointerHandoff,
    ...
    limitations: [
      "This run proves the supplied v0.12.12 macOS release-candidate archive, not an immutable GitHub release until those exact checksums are published.",
...
if (preparePackageMode) {
  ...
  try {
    ...
    if (scratchDir?.startsWith(`${scratchParent}/lbb-v0.12.12-scstream-`)) {
