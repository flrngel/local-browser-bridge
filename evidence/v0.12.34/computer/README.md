# macOS v0.12.34 persistent-share evidence candidate

This directory defines a fresh deterministic evidence harness for the packaged
macOS v0.12.34 server and helper candidate. It is protocol infrastructure, not
passing evidence; no v0.12.34 packaged run exists yet.
...
Release. Version 0.12.34 retains the exact-attempt trust rules and completes the
...
This v0.12.34 harness reuses none of those evidence bytes.
...
Version 0.12.34 retains the 1,000 ms authority-age limit, exact share, target,
and geometry checks, ban on one-shot observation, and larger abortable wait. It
adds explicit breakaway to both coordinator Jobs, atomic private Job-list child
creation, and a guard-plus-lifetime runner self-test that compiles and executes
the real fixture. It reuses no earlier candidate or evidence byte.
...
canceled. Version 0.12.34 does not reuse or relabel any prior binary,
...
tag remains protected historical metadata. Version 0.12.34 does not reuse any
...
- The server and helper report exactly v0.12.34, are universal
...
- The supplied executables are byte-for-byte identical to the copies inside
  `local-browser-bridge-v0.12.34-macos-universal.tar.gz`. The archive must be
  bound by a canonical `SHA256SUMS.txt` containing exactly the four v0.12.34
...
ATTEMPT_ROOT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.34-macos.XXXXXX")"
SCRATCH_PARENT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.34-scratch.XXXXXX")"
PACKAGE_ROOT="$PRIVATE_PARENT/lbb-v0.12.34-package-$RUN_NONCE"
...
ARCHIVE="$RELEASE_CANDIDATE_DIR/local-browser-bridge-v0.12.34-macos-universal.tar.gz"
MANIFEST="$RELEASE_CANDIDATE_DIR/SHA256SUMS.txt"
[[ ! -e "$PACKAGE_ROOT" && ! -L "$PACKAGE_ROOT" ]]
node evidence/v0.12.34/computer/helper-evidence-rig.mjs \
  --prepare-package \
...
pixels. In order, verify: ready with empty/zero counters; the v0.12.34 semantic
...
the v0.12.34 semantic value; semantic action 1/complete; persistent stream;
...
AGGREGATE_DIR="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.34-aggregate.XXXXXX")"
...
`evidence/v0.12.34/release/run-RUN_ID-attempt-ATTEMPT/macos/` on
`evidence/v0.12.34-release-run-RUN_ID-attempt-ATTEMPT`; its single commit parent
...
created. These records are diagnostic history, not v0.12.34 evidence:
...
The v0.12.2 candidate run is preserved byte-for-byte and is not v0.12.34
evidence:
...
They are linked for diagnostic history only and are not v0.12.34 results:
