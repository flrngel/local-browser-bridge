# Windows v0.12.34 stock-Chrome acceptance

This directory defines the release gate for the v0.12.34 browser extension and
...
approval, or public Release. Version 0.12.34 retains the exact-attempt trust
rules and completes the checked-in coordinator's source-only nested-Job
...
retried. The worker is bound to an atomic kill-on-close guard Job and then the
named lifetime Job before Worker or Intent state is published. Both permit only
explicit child breakaway, and each runner-owned child is atomically placed in
...
and cannot satisfy any v0.12.34 browser gate.
...
and cannot satisfy or be reused by any v0.12.34 browser gate.
...
No v0.12.33 byte, result, screenshot, or marker can satisfy v0.12.34.
...
messages, and evidence cannot be reused for v0.12.34. An interrupted or failed
v0.12.34 attempt is preserved only as sanitized negative metadata and is never
...
1. the installed v0.12.34 extension and Chrome's debugger-use indicator;
...
3. perform only required conditional toggles and load the verified directory
   ...
   is changed;
4. verify exactly one new test-owned card, version 0.12.34, no load errors, and
