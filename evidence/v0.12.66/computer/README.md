# macOS v0.12.66 persistent-share evidence candidate
...
0.12.66 requires fresh source, candidate bytes, reservation, and evidence in
...
not run. Version 0.12.66 aligns every completion-receipt consumer with the
existing 18-second app and runner grace and regression-tests a valid action
that takes more than ten seconds.
...
Version 0.12.66 retains that scheduling fix without extending the three-second
...
Version 0.12.66 collects those read-only baselines first, obtains a strictly
...
Version 0.12.66 commits the requested AppKit receiver with the bounded
...
Version 0.12.66 re-proves the requested-active receiver immediately before
...
stable for 265 polls. Version 0.12.66 binds the hello's controller PID to the
...
macOS v0.12.66 server and helper candidate. It is protocol infrastructure, not
...
Version 0.12.66 expands the exact bounded inventory and its adversarial tests
...
Release. Version 0.12.66 retains the exact-attempt trust rules and completes the
...
This v0.12.66 harness reuses none of those evidence bytes.
...
withdrawn without a retry, tag, or Release. Version 0.12.66 retains the exact
...
Version 0.12.66 keeps the same thresholds and refusal behavior, adds only a
...
Version 0.12.66 creates the destination and protected owner-only DACL in one
...
canceled. Version 0.12.66 does not reuse or relabel any prior binary,
...
tag remains protected historical metadata. Version 0.12.66 does not reuse any
...
```bash
node evidence/v0.12.66/computer/helper-evidence-rig.mjs --quiet-readiness
```
...
- The server and helper report exactly v0.12.66, are universal
...
- The supplied executables are byte-for-byte identical to the copies inside
  `local-browser-bridge-v0.12.66-macos-universal.tar.gz`. The archive must be
  bound by a canonical `SHA256SUMS.txt` containing exactly the four v0.12.66
...
ATTEMPT_ROOT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.66-macos.XXXXXX")"
SCRATCH_PARENT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.66-scratch.XXXXXX")"
PACKAGE_ROOT="$PRIVATE_PARENT/lbb-v0.12.66-package-$RUN_NONCE"
...
ARCHIVE="$RELEASE_CANDIDATE_DIR/local-browser-bridge-v0.12.66-macos-universal.tar.gz"
MANIFEST="$RELEASE_CANDIDATE_DIR/SHA256SUMS.txt"
[[ ! -e "$PACKAGE_ROOT" && ! -L "$PACKAGE_ROOT" ]]
node evidence/v0.12.66/computer/helper-evidence-rig.mjs \
  --prepare-package \
...
pixels. In order, verify: ready with empty/zero counters; the v0.12.66 semantic
...
the v0.12.66 semantic value; semantic action 1/complete; persistent stream;
...
`evidence/v0.12.66/release/run-RUN_ID-attempt-ATTEMPT/macos/` on
`evidence/v0.12.66-release-run-RUN_ID-attempt-ATTEMPT`; its single commit parent
...
created. These records are diagnostic history, not v0.12.66 evidence:
...
The v0.12.2 candidate run is preserved byte-for-byte and is not v0.12.66
evidence:
...
They are linked for diagnostic history only and are not v0.12.66 results:
