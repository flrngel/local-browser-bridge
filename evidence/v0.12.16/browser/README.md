# Windows v0.12.16 stock-Chrome acceptance

This directory defines the release gate for the v0.12.16 browser extension and
...
messages, and evidence cannot be reused for v0.12.16. An interrupted or failed
v0.12.16 attempt is preserved only as sanitized negative metadata and is never
...
1. the installed v0.12.16 extension and Chrome's debugger-use indicator;
...
3. perform only required conditional toggles and load the verified directory
   ...
   is changed;
4. verify exactly one new test-owned card, version 0.12.16, no load errors, and
