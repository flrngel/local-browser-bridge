# macOS v0.12.16 persistent-share evidence candidate

This directory defines a fresh deterministic evidence harness for the packaged
macOS v0.12.16 server and helper candidate. It is protocol infrastructure, not
passing evidence; no v0.12.16 packaged run exists yet.
...
canceled. Version 0.12.16 does not reuse or relabel any prior binary,
...
- The server and helper report exactly v0.12.16, are universal
...
- The supplied executables are byte-for-byte identical to the copies inside
  `local-browser-bridge-v0.12.16-macos-universal.tar.gz`. The archive must be
  bound by a canonical `SHA256SUMS.txt` containing exactly the four v0.12.16
...
`v0.12.16` tag. The source checkout and every candidate/evidence directory
...
ATTEMPT_ROOT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.16-macos.XXXXXX")"
SCRATCH_PARENT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.16-scratch.XXXXXX")"
PACKAGE_ROOT="$PRIVATE_PARENT/lbb-v0.12.16-package-$RUN_NONCE"
...
ARCHIVE="$RELEASE_CANDIDATE_DIR/local-browser-bridge-v0.12.16-macos-universal.tar.gz"
MANIFEST="$RELEASE_CANDIDATE_DIR/SHA256SUMS.txt"
[[ ! -e "$PACKAGE_ROOT" && ! -L "$PACKAGE_ROOT" ]]
node evidence/v0.12.16/computer/helper-evidence-rig.mjs \
  --prepare-package \
...
AGGREGATE_DIR="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.16-aggregate.XXXXXX")"
...
created. These records are diagnostic history, not v0.12.16 evidence:
...
The v0.12.2 candidate run is preserved byte-for-byte and is not v0.12.16
evidence:
...
They are linked for diagnostic history only and are not v0.12.16 results:
