...
const EXPECTED_VERSION = "0.12.42";
...
const FIXTURE_TITLE = "LBB v0.12.42 Persistent SCStream Evidence";
const SIBLING_FIXTURE_TITLE = "LBB v0.12.42 Same-PID Sibling Receiver";
const SEMANTIC_VALUE = "v0.12.42-semantic-value";
...
async function main() {
  ...
  scratchDir = await mkdtemp(join(scratchParent, "lbb-v0.12.42-scstream-"));
  ...
  const result = {
    ...
    candidateNotice: "This becomes release evidence only after the supplied checksum manifest and archive are published immutably for v0.12.42.",
    ...
    limitations: [
      "This run proves the supplied v0.12.42 macOS release-candidate archive, not an immutable GitHub release until those exact checksums are published.",
...
if (preparePackageMode) {
  ...
  try {
    ...
    if (scratchDir?.startsWith(`${scratchParent}/lbb-v0.12.42-scstream-`)) {
