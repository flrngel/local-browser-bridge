...
const EXPECTED_VERSION = "0.12.58";
...
const FIXTURE_TITLE = "LBB v0.12.58 Persistent SCStream Evidence";
const SIBLING_FIXTURE_TITLE = "LBB v0.12.58 Same-PID Sibling Receiver";
const SEMANTIC_VALUE = "v0.12.58-semantic-value";
...
async function main() {
  ...
  scratchDir = await mkdtemp(join(scratchParent, "lbb-v0.12.58-scstream-"));
  ...
  const result = {
    ...
    candidateNotice: "This becomes release evidence only after the supplied checksum manifest and archive are published immutably for v0.12.58.",
    ...
    limitations: [
      "This run proves the supplied v0.12.58 macOS release-candidate archive, not an immutable GitHub release until those exact checksums are published.",
...
if (preparePackageMode) {
  ...
  try {
    ...
    if (scratchDir?.startsWith(`${scratchParent}/lbb-v0.12.58-scstream-`)) {
