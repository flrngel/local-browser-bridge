# macOS v0.12.65 persistent-share evidence candidate
...
not run. Version 0.12.65 aligns every completion-receipt consumer with the
...
Version 0.12.65 retains that scheduling fix without extending the three-second
...
Version 0.12.65 collects those read-only baselines first, obtains a strictly
...
Version 0.12.65 commits the requested AppKit receiver with the bounded
...
Version 0.12.65 re-proves the requested-active receiver immediately before
...
stable for 265 polls. Version 0.12.65 binds the hello's controller PID to the
...
macOS v0.12.65 server and helper candidate. It is protocol infrastructure, not
passing evidence. Any passing packaged result must be read from the immutable
[v0.12.65 GitHub Release](https://github.com/flrngel/local-browser-bridge/releases/tag/v0.12.65)
...
Version 0.12.65 expands the exact bounded inventory and its adversarial tests
...
Release. Version 0.12.65 retains the exact-attempt trust rules and completes the
...
This v0.12.65 harness reuses none of those evidence bytes.
...
withdrawn without a retry, tag, or Release. Version 0.12.65 retains the exact
...
Version 0.12.65 keeps the same thresholds and refusal behavior, adds only a
...
Version 0.12.65 creates the destination and protected owner-only DACL in one
...
canceled. Version 0.12.65 does not reuse or relabel any prior binary,
...
tag remains protected historical metadata. Version 0.12.65 does not reuse any
...
```bash
node evidence/v0.12.65/computer/helper-evidence-rig.mjs --quiet-readiness
```
...
- The server and helper report exactly v0.12.65, are universal
...
- The supplied executables are byte-for-byte identical to the copies inside
  `local-browser-bridge-v0.12.65-macos-universal.tar.gz`. The archive must be
  bound by a canonical `SHA256SUMS.txt` containing exactly the four v0.12.65
...
ATTEMPT_ROOT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.65-macos.XXXXXX")"
SCRATCH_PARENT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.65-scratch.XXXXXX")"
PACKAGE_ROOT="$PRIVATE_PARENT/lbb-v0.12.65-package-$RUN_NONCE"
...
ARCHIVE="$RELEASE_CANDIDATE_DIR/local-browser-bridge-v0.12.65-macos-universal.tar.gz"
MANIFEST="$RELEASE_CANDIDATE_DIR/SHA256SUMS.txt"
[[ ! -e "$PACKAGE_ROOT" && ! -L "$PACKAGE_ROOT" ]]
node evidence/v0.12.65/computer/helper-evidence-rig.mjs \
  --prepare-package \
...
pixels. In order, verify: ready with empty/zero counters; the v0.12.65 semantic
...
the v0.12.65 semantic value; semantic action 1/complete; persistent stream;
...
`evidence/v0.12.65/release/run-RUN_ID-attempt-ATTEMPT/macos/` on
`evidence/v0.12.65-release-run-RUN_ID-attempt-ATTEMPT`; its single commit parent
...
created. These records are diagnostic history, not v0.12.65 evidence:
...
The v0.12.2 candidate run is preserved byte-for-byte and is not v0.12.65
evidence:
...
They are linked for diagnostic history only and are not v0.12.65 results:
