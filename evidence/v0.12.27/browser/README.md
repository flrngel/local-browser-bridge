# Windows v0.12.27 stock-Chrome acceptance

This directory defines the release gate for the v0.12.27 browser extension and
...
workflow attempt. Version 0.12.27 validates every returned statement's exact
source, tag, workflow, GitHub-hosted runner, same-run, and subject identity,
then requires exactly one current-attempt statement; zero, duplicates,
malformed entries, wrong subjects, and other-run entries fail closed. No
v0.12.26 Windows, stock-Chrome, publication, or Release evidence is reused.
...
messages, and evidence cannot be reused for v0.12.27. An interrupted or failed
v0.12.27 attempt is preserved only as sanitized negative metadata and is never
...
1. the installed v0.12.27 extension and Chrome's debugger-use indicator;
...
```powershell
$trustDestination = Join-Path ([IO.Path]::GetTempPath()) ("lbb-1215-" + [Guid]::NewGuid().ToString("N"))
...
$privateParent = Join-Path ([IO.Path]::GetTempPath()) ("lbb-1215-acceptance-" + [Guid]::NewGuid().ToString("N"))
...
3. perform only required conditional toggles and load the verified directory
   ...
   is changed;
4. verify exactly one new test-owned card, version 0.12.27, no load errors, and
