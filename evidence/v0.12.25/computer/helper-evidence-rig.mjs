...
const EXPECTED_VERSION = "0.12.25";
...
const FIXTURE_TITLE = "LBB v0.12.25 Persistent SCStream Evidence";
const SIBLING_FIXTURE_TITLE = "LBB v0.12.25 Same-PID Sibling Receiver";
const SEMANTIC_VALUE = "v0.12.25-semantic-value";
...
async function runRigSelfTest() {
  ...
  const refreshDecisionCases = [
    ...
    ["stale publication", shareAuthoritySuccessor, successorPublishedMilliseconds + 981, false],
  ...
  const dispatchDecisionCases = [
    ...
    ["stale dispatch authority", shareAuthoritySuccessor, shareAuthoritySuccessor.frameId, successorPublishedMilliseconds + 981, false],
  ...
  if (
    decideShareActionAuthorityRefresh(
      shareAuthorityBaseline,
      shareAuthoritySuccessor,
      successorPublishedMilliseconds + 980,
    ).estimatedAgeMs !== APP_SHARE_ACTION_FRAME_MAX_ESTIMATED_AGE_MS ||
    freshShareActionAuthorityTimeout(10_000) !== 2_000 ||
    freshShareActionAuthorityTimeout(8_001) !== 1 ||
    freshShareActionAuthorityTimeout(8_000) !== null ||
...
function freshShareActionAuthorityTimeout(remainingMilliseconds) {
  ...
  const budget = remainingMilliseconds - APP_SHARE_HANDOFF_COMPLETION_RESERVE_MS -
    APP_SHARE_HANDOFF_MINIMUM_ACTION_BUDGET_MS;
  return budget < 1 ? null : Math.min(2_000, budget);
}
...
async function main() {
  ...
  scratchDir = await mkdtemp(join(scratchParent, "lbb-v0.12.25-scstream-"));
  ...
  if (pointerEvidenceLane === "deliberate-concurrency") {
    ...
    current = await waitForNextShareState(
      ...
      (sample, candidateObservation) => {
        const candidateAuthority = shareActionAuthority(candidateObservation, sample);
        return decideShareActionAuthorityRefresh(
          armedShareAuthority,
          candidateAuthority,
          Date.now(),
        ).accepted;
    ...
    const freshShareAuthority = shareActionAuthority(
      current.snapshot.computerObservation,
      current.sample,
    );
    const refreshedAuthorityDecision = decideShareActionAuthorityRefresh(
      armedShareAuthority,
      freshShareAuthority,
      Date.now(),
    );
  ...
  const result = {
    ...
    candidateNotice: "This becomes release evidence only after the supplied checksum manifest and archive are published immutably for v0.12.25.",
    ...
    limitations: [
      "This run proves the supplied v0.12.25 macOS release-candidate archive, not an immutable GitHub release until those exact checksums are published.",
...
if (preparePackageMode) {
  ...
  try {
    ...
    if (scratchDir?.startsWith(`${scratchParent}/lbb-v0.12.25-scstream-`)) {
