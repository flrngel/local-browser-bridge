...
const EXPECTED_VERSION = "0.12.67";
...
const EXPECTED_MACOS_ARCHIVE_ENTRIES = [
  ...
  { name: "Local Browser Bridge.app", kind: "directory", mode: 0o755, maximumBytes: 0 },
  { name: "Local Browser Bridge.app/Contents", kind: "directory", mode: 0o755, maximumBytes: 0 },
  { name: "Local Browser Bridge.app/Contents/Info.plist", kind: "file", mode: 0o644, maximumBytes: 1024 * 1024 },
  { name: "Local Browser Bridge.app/Contents/MacOS", kind: "directory", mode: 0o755, maximumBytes: 0 },
  { name: "Local Browser Bridge.app/Contents/MacOS/local-browser-bridge-desktop", kind: "file", mode: 0o755, maximumBytes: 200 * 1024 * 1024 },
  { name: "Local Browser Bridge.app/Contents/_CodeSignature", kind: "directory", mode: 0o755, maximumBytes: 0 },
  { name: "Local Browser Bridge.app/Contents/_CodeSignature/CodeResources", kind: "file", mode: 0o644, maximumBytes: 16 * 1024 * 1024 },
...
const FIXTURE_TITLE = "LBB v0.12.67 Persistent SCStream Evidence";
const SIBLING_FIXTURE_TITLE = "LBB v0.12.67 Same-PID Sibling Receiver";
const SEMANTIC_VALUE = "v0.12.67-semantic-value";
...
const ACTIVE_TARGET_RECEIVER_PROBE_TIMEOUT_MS = 16_000;
...
// Keep a 500 ms dispatch margin inside the product's three-second stale-frame
// refusal while allowing normal SCStream publication and API observation
// latency on a loaded release-validation host.
...
const QUIET_SEAT_UNKNOWN_CAUSES = [
  "pointer-monitor-unhealthy",
  "cursor-unreadable",
  "pointer-counter-unreadable",
  "keyboard-counter-unreadable",
  "active-space-probe-unhealthy",
  "active-space-unreadable",
  "foreground-probe-unhealthy",
  "foreground-transition-unreadable",
  "foreground-identity-unreadable",
  "foreground-ax-probe-unhealthy",
  "foreground-ax-identity-unreadable",
  "pointer-boundary-uncorroborated",
  "keyboard-boundary-uncorroborated",
...
function emptyQuietSeatResetCauseCounts() {
  return Object.fromEntries(QUIET_SEAT_RESET_CAUSES.map((cause) => [cause, 0]));
}

let quietSeatStabilization = freshQuietSeatSummary();
...
function run(commandName, args, timeoutMs = SUBPROCESS_TIMEOUT_MS) {
  ...
  if (result.error) {
    const error = new Error(`${basename(commandName)} could not be executed`);
    error.code = "SUBPROCESS_LAUNCH_FAILURE";
    throw error;
  }
  if (result.status !== 0) {
    const detail = sanitizePathDetail((result.stderr || result.stdout || "no diagnostic output").trim());
    const error = new Error(`${basename(commandName)} failed: ${detail}`);
...
if len(expected) != 17 or len(expected) != len(expected_entries):
    raise SystemExit("internal macOS archive inventory is not exactly seventeen unique entries")
  ...
  return await readBoundAppShareReceipt(
    ...
    (record) => {
      ...
      return record.schemaVersion === APP_SHARE_HANDOFF_MARKER_SCHEMA &&
        ...
        productStarted >= startCreated &&
        productStarted - startCreated <= APP_SHARE_HANDOFF_COMPLETION_GRACE_MS &&
        productCompleted >= productStarted &&
        productCompleted - productStarted <= APP_SHARE_HANDOFF_COMPLETION_GRACE_MS &&
        created >= productCompleted &&
        created - startCreated <= APP_SHARE_HANDOFF_COMPLETION_GRACE_MS &&
...
function processProbe(
  ...
  const output = run(path, args, timeoutMs);
  try {
    return JSON.parse(output);
  } catch {
    const error = new Error("SystemProbe returned malformed output");
    error.code = "SUBPROCESS_MALFORMED_OUTPUT";
    throw error;
  }
}
...
function quietSeatSampleMonitoringState(sample) {
  ...
  if (sample?.pointerActivityMonitorHealthy !== true) {
    return { kind: "unknown", cause: "pointer-monitor-unhealthy" };
  }
  if (!Number.isFinite(sample?.cursorX) || !Number.isFinite(sample?.cursorY)) {
    return { kind: "unknown", cause: "cursor-unreadable" };
  }
  if (
    ...
    return { kind: "unknown", cause: "pointer-counter-unreadable" };
  }
  if (
    ...
    return { kind: "unknown", cause: "keyboard-counter-unreadable" };
  }
  if (sample?.activeSpaceProbeHealthy !== true) {
    return { kind: "unknown", cause: "active-space-probe-unhealthy" };
  }
  if (!Number.isSafeInteger(sample?.activeSpace) || sample.activeSpace < 1) {
    return { kind: "unknown", cause: "active-space-unreadable" };
  }
  if (sample?.foregroundProbeHealthy !== true) {
    return { kind: "unknown", cause: "foreground-probe-unhealthy" };
  }
...
function quietSeatTransitionDisposition(before, after) {
  ...
  if (beforeState.kind === "unknown") return { kind: "unknown", cause: beforeState.cause };
  if (afterState.kind === "unknown") return { kind: "unknown", cause: afterState.cause };
  if (beforeState.kind === "changed") return { kind: "reset", cause: beforeState.cause };
  if (afterState.kind === "changed") return { kind: "reset", cause: afterState.cause };

  const invariants = systemInvariants(before, after);
  const pointerState = independentPointerLaneState(invariants);
  if (pointerState === "unknown") {
    const cause =
  ...
  if (pointerState === "concurrent-shared-seat-activity") {
    return {
      kind: "reset",
      cause: invariants.sharedPointerActivityObserved === true
        ? "hid-pointer-activity"
        : "hid-keyboard-activity",
    };
  }
  if (invariants.foregroundUnchanged !== true) {
    return { kind: "reset", cause: "foreground-changed" };
  }
  if (invariants.userFocusUnchanged !== true) {
    return { kind: "reset", cause: "focus-changed" };
  }
  if (invariants.spaceUnchanged !== true) {
    return { kind: "reset", cause: "active-space-changed" };
  }
  return pointerState === "quiet"
    ? { kind: "stable", cause: null }
    : { kind: "unknown", cause: "transition-unclassified" };
}

function quietSeatProbeFailureCategory(error) {
  if (error?.code === "SUBPROCESS_TIMEOUT") return "timeout";
  if (error?.code === "SUBPROCESS_NONZERO_EXIT") return "nonzero-exit";
  if (error?.code === "SUBPROCESS_LAUNCH_FAILURE") return "launch-failure";
  if (error?.code === "SUBPROCESS_MALFORMED_OUTPUT") return "malformed-output";
...
function attachQuietSeatSummary(error, summary) {
  error.quietSeatStabilization = quietSeatSummarySnapshot(summary);
  return error;
}

function quietSeatSummarySnapshot(summary) {
  return {
    ...summary,
    resetCauseCounts: { ...summary.resetCauseCounts },
  };
}

function freshQuietSeatSummary({
  ...
  return {
    ...
    resetCauseCounts: emptyQuietSeatResetCauseCounts(),
...
function quietSeatReadinessRecord(status, summary) {
  return {
    ...
    requiredStableMilliseconds: summary.requiredStableMilliseconds,
    maximumWaitMilliseconds: summary.maximumWaitMilliseconds,
    sampleIntervalMilliseconds: summary.sampleIntervalMilliseconds,
    requiredStableTransitions: summary.requiredStableTransitions,
    completed: summary.completed,
    stableDurationMilliseconds: summary.stableDurationMilliseconds,
    observedSamples: summary.observedSamples,
    stableTransitions: summary.stableTransitions,
    resetCount: summary.resetCount,
    monitoringUnknown: summary.monitoringUnknown,
    diagnosticSchemaVersion: summary.diagnosticSchemaVersion,
    lastResetCause: summary.lastResetCause,
    lastUnknownCause: summary.lastUnknownCause,
    resetCauseCounts: { ...summary.resetCauseCounts },
    probeFailureCategory: summary.probeFailureCategory,
    rawProbeDataRetained: false,
  };
}

function quietSeatProbeFailureSummary(error, probeFailureCategory = quietSeatProbeFailureCategory(error)) {
  const summary = freshQuietSeatSummary();
  summary.monitoringUnknown = true;
  summary.lastUnknownCause = "probe-failure";
  summary.probeFailureCategory = probeFailureCategory;
  return summary;
}
...
async function executeQuietSeatReadiness({
  ...
  try {
    ...
    summary = error?.quietSeatStabilization == null
      ? quietSeatProbeFailureSummary(error)
      : quietSeatSummarySnapshot(error.quietSeatStabilization);
  }
  const ready =
    summary.completed === true &&
    summary.completedBeforeCandidateExecution === true &&
    summary.monitoringUnknown === false;
...
async function runQuietReadinessMode({ forceProbeNonzero = false } = {}) {
  ...
  let record;
  try {
    readinessRoot = await mkdtemp(join(tmpdir(), "lbb-v01237-quiet-readiness-"));
  ...
  if (readinessRoot !== null) {
    try {
      await rm(readinessRoot, { recursive: true, force: false });
    } catch {
      record = quietSeatReadinessRecord(
        "refused",
        quietSeatProbeFailureSummary(null, "cleanup-failure"),
      );
    }
  }
...
async function runNativeQuietSeatStabilization({
  ...
  const publishSummary = () => onSummary(quietSeatSummarySnapshot(summary));
  const failUnknown = (cause, probeFailureCategory = null) => {
    if (!QUIET_SEAT_UNKNOWN_CAUSES.includes(cause)) {
      throw new Error("the native quiet-seat unknown diagnostic is outside its fixed vocabulary");
    }
    if (
      probeFailureCategory !== null &&
      !QUIET_SEAT_PROBE_FAILURE_CATEGORIES.includes(probeFailureCategory)
    ) {
      throw new Error("the native quiet-seat probe diagnostic is outside its fixed vocabulary");
    }
    summary.monitoringUnknown = true;
    summary.lastUnknownCause = cause;
    summary.probeFailureCategory = probeFailureCategory;
    publishSummary();
    report(
      `Native quiet-seat monitoring failed closed (${cause}` +
        `${probeFailureCategory === null ? "" : `; ${probeFailureCategory}`}).`,
    );
    throw attachQuietSeatSummary(quietSeatUnknownError(), summary);
  };
  const recordReset = (cause) => {
    if (!QUIET_SEAT_RESET_CAUSES.includes(cause)) {
      failUnknown("transition-unclassified");
    }
    summary.resetCount += 1;
    summary.lastResetCause = cause;
    summary.resetCauseCounts[cause] += 1;
  };

  const baselineState = quietSeatSampleMonitoringState(baseline);
  if (baselineState.kind === "unknown") failUnknown(baselineState.cause);
  let previous = baseline;
  let stableEpochStartedAt = startedAtMilliseconds;
  if (baselineState.kind === "changed") {
    recordReset(baselineState.cause);
    report(
      `Observed pre-execution user-context activity (${baselineState.cause}); ` +
        "starting a fresh native quiet-seat epoch.",
    );
  }
  publishSummary();
  ...
  while (monotonicMilliseconds() < waitDeadlineMilliseconds) {
    ...
    if (disposition.kind === "reset") {
      ...
      report(
        `Observed ${disposition.cause}; reset the native quiet-seat epoch before candidate execution.`,
      );
    } else {
      ...
      if (
        ...
        summary.completedBeforeCandidateExecution = true;
        publishSummary();
        return { finalSample: sample, summary: quietSeatSummarySnapshot(summary) };
      }
    }
    previous = sample;
    publishSummary();
...
async function runQuietSeatExecutionSelfTests() {
  const runResetCause = async (expectedCause, changedSample) => {
    const clock = { value: 0 };
    const samples = [changedSample, changedSample, changedSample];
    const reports = [];
    ...
    rigSelfTestAssert(
      samples.length === 0 &&
        ...
        result.summary.lastResetCause === expectedCause &&
        result.summary.lastUnknownCause === null &&
        result.summary.resetCauseCounts[expectedCause] === 1 &&
        Object.values(result.summary.resetCauseCounts).reduce((sum, count) => sum + count, 0) === 1 &&
        result.summary.probeFailureCategory === null &&
        result.summary.stableDurationMilliseconds === 1_000 &&
        result.summary.stableTransitions === 2 &&
        reports.some((message) => message.includes(expectedCause)),
      `${expectedCause} did not reset and categorize the complete quiet-seat epoch`,
    );
  };

  const expectUnknown = async ({ sample = null, error = null, cause, probeCategory = null }) => {
    let failure = null;
    try {
    ...
    const summary = failure?.quietSeatStabilization;
    rigSelfTestAssert(
      failure?.message === quietSeatUnknownError().message &&
        summary?.monitoringUnknown === true &&
        summary?.completed === false &&
        summary?.diagnosticSchemaVersion === QUIET_SEAT_DIAGNOSTIC_SCHEMA_VERSION &&
        summary?.lastUnknownCause === cause &&
        summary?.probeFailureCategory === probeCategory,
      `${cause}/${probeCategory ?? "no-probe-failure"} did not fail closed with its bounded category`,
    );
  };

  {
    ...
    rigSelfTestAssert(
      result.summary.completed === true &&
        ...
        result.summary.diagnosticSchemaVersion === QUIET_SEAT_DIAGNOSTIC_SCHEMA_VERSION &&
        result.summary.lastResetCause === null &&
        result.summary.lastUnknownCause === null &&
        Object.values(result.summary.resetCauseCounts).every((count) => count === 0) &&
        result.summary.probeFailureCategory === null &&
        reports.length === 0,
  ...
  await runResetCause("active-space-changed", {
    ...rigSelfTestPointerSample(),
    activeSpace: 2,
  });

  await expectUnknown({
    sample: { ...rigSelfTestPointerSample(), foregroundAXProbeHealthy: false },
    cause: "foreground-ax-probe-unhealthy",
  });
  await expectUnknown({
    sample: { ...rigSelfTestPointerSample(), cursorX: 101 },
    cause: "pointer-boundary-uncorroborated",
  });
  for (const [code, probeCategory] of [
    ["SUBPROCESS_TIMEOUT", "timeout"],
    ["SUBPROCESS_NONZERO_EXIT", "nonzero-exit"],
    ["SUBPROCESS_MALFORMED_OUTPUT", "malformed-output"],
  ]) {
    await expectUnknown({
      error: Object.assign(new Error("self-test probe failure"), { code }),
      cause: "probe-failure",
      probeCategory,
    });
  }

  {
    ...
    const samples = [
      ...
      { ...rigSelfTestPointerSample({ mouseMoved: 1 }), cursorX: 101 },
    ];
    let failure = null;
    ...
    rigSelfTestAssert(
      candidateInvocations === 0 &&
        summary?.resetCount === 1 &&
        summary?.lastResetCause === "hid-pointer-activity" &&
        summary?.lastUnknownCause === "pointer-boundary-uncorroborated" &&
        summary?.monitoringUnknown === true &&
        summary?.completedBeforeCandidateExecution === false,
      "an alternating reset-to-unknown transition reached candidate execution or lost its categories",
    );
  }
...

  {
    ...
    rigSelfTestAssert(
      record.status === "ready" &&
        record.acceptanceEvidence === false &&
        record.candidateInvocations === 0 &&
        record.completed === true &&
        record.monitoringUnknown === false &&
        record.rawProbeDataRetained === false &&
        !/(foregroundPID|rawForegroundPSN|frontWindowID|cursorX|hidPointerCounters|error|path)/.test(serialized),
...

  {
    ...
    rigSelfTestAssert(
      record.status === "refused" &&
        ...
        record.candidateInvocations === 0 &&
        record.resetCount === 1 &&
        record.lastResetCause === "hid-pointer-activity" &&
        record.lastUnknownCause === "pointer-boundary-uncorroborated" &&
        record.probeFailureCategory === null &&
        record.rawProbeDataRetained === false,
...
async function runRigSelfTest() {
  ...
  if (
    quietSeatTransitionDisposition(quietBaseline, quietBaseline).kind !== "stable" ||
    quietSeatTransitionDisposition(
      quietBaseline,
      rigSelfTestPointerSample({ mouseMoved: 1 }),
    ).cause !== "hid-pointer-activity" ||
    quietSeatTransitionDisposition(
      quietBaseline,
      rigSelfTestPointerSample({ keyDown: 1 }),
    ).cause !== "hid-keyboard-activity" ||
    quietSeatTransitionDisposition(
      quietBaseline,
      rigSelfTestPointerSample({ identity: 2 }),
    ).cause !== "foreground-changed" ||
    quietSeatTransitionDisposition(quietBaseline, focusChanged).cause !== "focus-changed" ||
    quietSeatTransitionDisposition(
      quietBaseline,
      { ...quietBaseline, activeSpace: 2 },
    ).cause !== "active-space-changed" ||
    quietSeatTransitionDisposition(
      quietBaseline,
      { ...quietBaseline, pointerActivityMonitorHealthy: false },
    ).cause !== "pointer-monitor-unhealthy"
  ) {
  ...
  const refreshDecisionCases = [
    ...
    ["1250ms-age publication", shareAuthoritySuccessor, successorPublishedMilliseconds + 1_230, true],
    ["2501ms-age publication", shareAuthoritySuccessor, successorPublishedMilliseconds + 2_481, false],
  ...
  const dispatchDecisionCases = [
    ...
    ["1250ms-age dispatch authority", shareAuthoritySuccessor, shareAuthoritySuccessor.frameId, successorPublishedMilliseconds + 1_230, true],
    ["2501ms-age dispatch authority", shareAuthoritySuccessor, shareAuthoritySuccessor.frameId, successorPublishedMilliseconds + 2_481, false],
  ...
  const exactAgeBoundaryDecision = decideShareActionAuthorityRefresh(
    ...
    successorPublishedMilliseconds + 2_480,
  );
  if (
  ...
  const refusalCandidates = [
    () => ({
      ...shareAuthoritySuccessor,
      capturedAt: new Date(
        virtualRefreshStartedMilliseconds + refusalElapsedMilliseconds - 2_481,
      ).toISOString(),
    }),
...
async function main() {
  ...
  scratchDir = await mkdtemp(join(scratchParent, "lbb-v0.12.67-scstream-"));
  ...
  requireCheck(
    "system probe foreground-window self-test",
    runExactLine(
      systemProbeBinary,
      ["--self-test"],
  ...
  if (!postResizeClickResponse.ok) {
    ...
    const responseMessage = postResizeClickResponse.body.error?.message;
    if (pointerEvidenceLane === "deliberate-concurrency") {
  ...
  // Status and baseline probes are intentionally read-only, but the native
  // source can advance while they run and thereby retire the prior action
  // frame. Obtain a strictly newer share frame after those probes, then issue
  // the bound command without another asynchronous operation in between.
  current = await waitFor(
    "fresh cancellation action frame",
    async () => {
      const snapshot = await apiState();
      const candidateObservation = snapshot.computerObservation;
      const sample = shareSample(candidateObservation, "share-before-cancellation");
      return sample &&
        sample.shareId === firstShareId &&
  ...
  requireCheck("cancellation starts from a current resized share frame",
    observation.share?.active === true && observation.share?.id === firstShareId &&
      ...
      current.sample.sequence > preCancellationShareSample.sequence &&
      current.sample.sourceSequence > preCancellationShareSample.sourceSequence &&
      typeof canceledScreenshotUrl === "string" && canceledScreenshotUrl.startsWith("/api/computer/screenshot?"),
    `share=${observation.share?.id === firstShareId}, source=${preCancellationShareSample.sourceSequence}->${observation.sourceSequence}, transport=${preCancellationShareSample.sequence}->${current.sample.sequence}, screenshot-bound=${typeof canceledScreenshotUrl === "string"}`,
  );
  ...
  const cancellationDispatchedFixtureState = await Promise.race([
    waitFor(
    ...
    originalCanceledRequest.then((response) => {
      throw new Error(
        `computer.move completed before target delivery: HTTP ${response.status} ${response.body.error?.code || "unexpected-success"}`,
      );
    }),
  ]);
  ...
  const result = {
    ...
    candidateNotice: "This becomes release evidence only after the supplied checksum manifest and archive are published immutably for v0.12.67.",
    ...
    limitations: [
      "This run proves the supplied v0.12.67 macOS release-candidate archive, not an immutable GitHub release until those exact checksums are published.",
...
if (preparePackageMode) {
  ...
  try {
    ...
    if (scratchDir?.startsWith(`${scratchParent}/lbb-v0.12.67-scstream-`)) {
