# macOS v0.12.67 persistent-share evidence candidate
...
Version 0.12.67 bounds WGC startup and startup rollback below the outer helper
...
0.12.67 requires fresh source, candidate bytes, reservation, and evidence in
...
that takes more than ten seconds. Version 0.12.67 retains that repair.
...
Version 0.12.67 retains that scheduling fix without extending the three-second
...
Version 0.12.67 collects those read-only baselines first, obtains a strictly
...
Version 0.12.67 commits the requested AppKit receiver with the bounded
...
Version 0.12.67 re-proves the requested-active receiver immediately before
...
stable for 265 polls. Version 0.12.67 binds the hello's controller PID to the
...
macOS v0.12.67 server and helper candidate. It is protocol infrastructure, not
...
Version 0.12.67 expands the exact bounded inventory and its adversarial tests
...
Release. Version 0.12.67 retains the exact-attempt trust rules and completes the
...
v0.12.26, v0.12.27, nor v0.12.42 supplies candidate bytes or evidence for this fresh
...
This v0.12.67 harness reuses none of those evidence bytes.
...
withdrawn without a retry, tag, or Release. Version 0.12.67 retains the exact
...
Version 0.12.67 keeps the same thresholds and refusal behavior, adds only a
...
Version 0.12.67 creates the destination and protected owner-only DACL in one
...
canceled. Version 0.12.67 does not reuse or relabel any prior binary,
...
tag remains protected historical metadata. Version 0.12.67 does not reuse any
...
```bash
node evidence/v0.12.67/computer/helper-evidence-rig.mjs --quiet-readiness
```
...
- The server and helper report exactly v0.12.67, are universal
...
- The supplied executables are byte-for-byte identical to the copies inside
  `local-browser-bridge-v0.12.67-macos-universal.tar.gz`. The archive must be
  bound by a canonical `SHA256SUMS.txt` containing exactly the four v0.12.67
...
ATTEMPT_ROOT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.67-macos.XXXXXX")"
SCRATCH_PARENT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.67-scratch.XXXXXX")"
PACKAGE_ROOT="$PRIVATE_PARENT/lbb-v0.12.67-package-$RUN_NONCE"
...
ARCHIVE="$RELEASE_CANDIDATE_DIR/local-browser-bridge-v0.12.67-macos-universal.tar.gz"
MANIFEST="$RELEASE_CANDIDATE_DIR/SHA256SUMS.txt"
[[ ! -e "$PACKAGE_ROOT" && ! -L "$PACKAGE_ROOT" ]]
node evidence/v0.12.67/computer/helper-evidence-rig.mjs \
  --prepare-package \
...
pixels. In order, verify: ready with empty/zero counters; the v0.12.67 semantic
...
the v0.12.67 semantic value; semantic action 1/complete; persistent stream;
...
`evidence/v0.12.67/release/run-RUN_ID-attempt-ATTEMPT/macos/` on
`evidence/v0.12.67-release-run-RUN_ID-attempt-ATTEMPT`; its single commit parent
...
created. These records are diagnostic history, not v0.12.67 evidence:
...
The v0.12.2 candidate run is preserved byte-for-byte and is not v0.12.67
evidence:
...
They are linked for diagnostic history only and are not v0.12.67 results:
