# Windows v0.12.9 stock-Chrome acceptance protocol

This directory defines the release gate for the v0.12.9 browser extension and
...
No v0.12.9 stock-Chrome acceptance result exists yet. This protocol requires a
fresh v0.12.9 candidate and output directory; it cannot finalize the unexecuted
v0.12.8 browser gate or reuse any prior operator record, notification, or image.
...
coordinator independently supplies version `0.12.9`, lowercase `FINAL_SHA`,
...
if ($Version -cne "0.12.9" -or $FinalSha -cnotmatch '^[0-9a-f]{40}$' -or
...
4. Verify exactly one enabled v0.12.9 card with no load error. Open the
...
|---|---|---|
| `extension-loaded` | `browser-01-extension-loaded.png` | Stock `chrome://extensions` shows exactly one enabled unpacked Local Browser Bridge v0.12.9 card, no load error, and Chrome's native debugger-use indicator while the bridge lease is active. |
