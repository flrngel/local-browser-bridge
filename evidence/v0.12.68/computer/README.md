# macOS v0.12.68 persistent-share evidence candidate

The sole exact v0.12.67 candidate passed the Windows trust gate and
protocol-bound helper readiness, then its operator-dependent foreground-arm
gate expired with no mouse-down, mouse-up, or acknowledgement. It failed closed
before observation or any product action and is terminal. Version 0.12.68 uses
an automatic Windows readiness gate instead: consecutive native samples must
prove that the existing foreground and focus root in the runner's current interactive session remains stable and
outside the dedicated fixture process while the fixture stays nonactivating in
the background. The runner does not raise or activate the fixture, synthesize
input, or wait for an operator. It publishes matching schema-v3 request and
ready-proof records under the legacy foreground-arm filenames, and the
read-only watcher validates both automatically. Those records remain
notification-only and grant no consent or action authority. Fresh source,
candidate bytes, reservation, and evidence are mandatory; no v0.12.67 byte or
result is reusable.
...
asynchronous response, and classifies `COMPUTER_HELPER_WATCHDOG` as unavailable
with reconnect guidance. It uses fresh source, candidate bytes, reservations,
and evidence in every lane; no v0.12.66 byte or result is reusable.

The sole exact v0.12.65 candidate was withdrawn during its one-shot Windows
computer acceptance. Its recovery suite enqueued `computer.share.start`, then
the helper connection ended before the outcome could be established. The
result was the terminal, non-retriable `COMMAND_OUTCOME_UNKNOWN` record frozen
at
[`withdrawn-9a18c44-windows-share-start-outcome-unknown`](https://github.com/flrngel/local-browser-bridge/tree/bf8dbe595e7c7beb9a0817f0262fde46c1e49578/evidence/v0.12.65/computer/attempts/withdrawn-9a18c44-windows-share-start-outcome-unknown).
Stock-Chrome acceptance, tagging, and publication did not follow. Version
0.12.68 requires fresh source, candidate bytes, reservation, and evidence in
every lane; no v0.12.65 byte or result is reusable.

The sole exact v0.12.64 candidate passed its 207-check macOS quiet lane and
mandatory six-image review. Its deliberate lane then acknowledged the single
exact-app action, refreshed to a same-share frame with 986.503 milliseconds of
estimated dispatch age, dispatched the product action, observed the target
postcondition, and preserved both shared-seat boundaries. The app produced its
bound completion receipt after 11,765 milliseconds. That was inside the app
and runner's existing 18-second completion grace, but the watcher, receipt
reader, and finalizer still enforced an inconsistent 10-second interval and
rejected it. The exact terminal record is retained at
[`withdrawn-5cb7e8a-macos-app-share-completion-window`](../../v0.12.64/computer/attempts/withdrawn-5cb7e8a-macos-app-share-completion-window/README.md).
The candidate was not retried; Windows, Chrome, tagging, and publication did
not run. Version 0.12.66 aligned every completion-receipt consumer with the
existing 18-second app and runner grace and regression-tested a valid action
that takes more than ten seconds. Version 0.12.68 retains that repair.

The sole exact v0.12.63 candidate passed its 207-check macOS quiet lane and
mandatory six-image review. In the deliberate lane, the single exact-app-share
action and all independent shared-seat invariants passed. The harness then
bound a strictly newer same-share frame with an estimated dispatch age of
1,784.713 milliseconds, but the helper's next serialized share pump acquired
the controller before the queued click. That conversion consumed the remaining
three-second frame lease, so the helper correctly returned HTTP 409
`COMPUTER_STALE_FRAME` before any dispatch outcome could be proven. The exact
terminal record is retained at
[`withdrawn-7cfc4f0-macos-app-share-stale-frame`](../../v0.12.63/computer/attempts/withdrawn-7cfc4f0-macos-app-share-stale-frame/README.md).
The candidate was not retried; Windows, Chrome, tagging, and publication did
not run. Version 0.12.64 gave an aged published share frame one bounded action
admission interval before another share pump may acquire the controller.
Version 0.12.68 retains that scheduling fix without extending the three-second
frame lease, changing target/geometry binding, or permitting retrying stale
authority.

Version 0.12.57 never produced a GitHub candidate. Its clean source-built
package passed the deployment gate, then the first local quiet diagnostic
failed before native move dispatch. A diagnostic-only harness binding proved
that the server correctly returned HTTP 409 `COMPUTER_STALE_FRAME`: native
capture had advanced while the harness queried share status and collected its
baselines. The terminal local-only record is retained at
[`withdrawn-local-macos-cancellation-stale-frame-race`](../../v0.12.57/computer/attempts/withdrawn-local-macos-cancellation-stale-frame-race/README.md).
Version 0.12.68 collects those read-only baselines first, obtains a strictly
newer share frame immediately before the move, and races delivery against any
early command response. It requires entirely fresh candidate bytes and
evidence.

Version 0.12.56 never produced a GitHub candidate. Its clean source-built
package passed the deployment gate, but the first local quiet diagnostic
showed that even an exact-view `.activeAlways` tracking area could
intermittently miss the cancellation-bound move receipt. The unchanged package
later passed 207 of 207 checks, proving that the view callback was a
timing-dependent observation boundary. The terminal local-only record is
retained at
[`withdrawn-local-macos-window-move-observer-race`](../../v0.12.56/computer/attempts/withdrawn-local-macos-window-move-observer-race/README.md).
Version 0.12.57 records the exact window-number-bound `mouseMoved` event at
`NSWindow.sendEvent`, before view tracking-area dispatch, and requires entirely
fresh candidate bytes and evidence. No v0.12.56 byte is reusable here.

Version 0.12.55 never produced a GitHub candidate. Its clean source-built
package passed the deployment gate, but the local quiet diagnostic exposed an
observer defect: `FixtureView.mouseMoved` had no tracking area and therefore
depended on the current first-responder chain. The terminal local-only record
is retained at
[`withdrawn-local-macos-move-observer-first-responder-race`](../../v0.12.55/computer/attempts/withdrawn-local-macos-move-observer-first-responder-race/README.md).
Version 0.12.56 installed the exact-view tracking area whose remaining
observation race is recorded above. No v0.12.55 byte is reusable here.

Version 0.12.54 never produced a GitHub candidate. Its clean source-built
package passed the full local deployment gate but failed a repeated local
quiet diagnostic at the cancellation-bound `computer.move`: AX main/focused
re-proofs succeeded while the actual AppKit receiver still did not consume the
trajectory. The sanitized local-only record is retained at
[`withdrawn-local-macos-cancellation-move-key-window-race`](../../v0.12.54/computer/attempts/withdrawn-local-macos-cancellation-move-key-window-race/README.md).
Version 0.12.68 commits the requested AppKit receiver with the bounded
target-only make-key pair after no-raise activation, re-proves the phase, and
requires entirely fresh candidate bytes and evidence.

The sole exact v0.12.53 candidate passed its source, artifact, provenance,
package, architecture, signature, and version gates, then passed 158 quiet-lane
checks. Its cancellation-bound `computer.move` still did not advance the exact
target's mouse-move instrumentation within ten seconds. The candidate was
withdrawn without a retry or any later acceptance and its terminal record is
retained at
[`withdrawn-f5e7c85b-macos-cancellation-move-delivery-timeout`](../../v0.12.53/computer/attempts/withdrawn-f5e7c85b-macos-cancellation-move-delivery-timeout/README.md).
Version 0.12.68 re-proves the requested-active receiver immediately before
every trajectory event so dispatch cannot race the observed activation state.
It requires entirely fresh candidate bytes and evidence.

The sole exact v0.12.52 candidate passed 158 quiet-lane checks and produced six
visually reviewed exact-window screenshots. Its final cancellation proof then
timed out because a long `computer.move` addressed to the primary fixture did
not advance that inactive window's mouse-move instrumentation while a genuine
same-process sibling remained the application's main and focused receiver.
The harness failed closed, terminated every candidate process, and never
started the deliberate lane, Windows, stock Chrome, tagging, or publication.
The sanitized terminal record is retained at
[`withdrawn-e4461d4b-macos-cancellation-move-delivery-timeout`](../../v0.12.52/computer/attempts/withdrawn-e4461d4b-macos-cancellation-move-delivery-timeout/README.md).
Version 0.12.53 applied the same bounded, no-raise focus lease and exact prior
receiver restoration to mouse-move trajectories that already protect click,
drag, scroll, and native text delivery. It requires entirely fresh candidate
bytes and evidence.

The sole exact v0.12.50 candidate passed its fresh 207-check quiet lane, and
all six quiet screenshots passed mandatory visual review. The coordinator then
supplied a nonexistent owner-private scratch parent to the deliberate runner.
The runner stopped after seven source and output-boundary checks, before any
deliberate-lane candidate byte or app-share UI action. The terminal sanitized
record is retained at
[`withdrawn-e5530ce-macos-coordinator-scratch-path`](../../v0.12.50/computer/attempts/withdrawn-e5530ce-macos-coordinator-scratch-path/README.md).
Windows, stock Chrome, evidence finalization, tagging, and publication did not
complete. No v0.12.50 evidence is reusable by this fresh gate.

The sole exact v0.12.47 candidate passed both packaged macOS lanes and all
twelve screenshots passed mandatory visual review. The coordinator then
combined two documented sequential aggregate-path assignments into one shell
command under `set -u`; the second expansion saw no first assignment, and the
finalizer correctly rejected an empty output path. Its terminal nineteen-file
sanitized lane record is retained at
[`withdrawn-6f469e4-macos-finalizer-empty-path`](../../v0.12.47/computer/attempts/withdrawn-6f469e4-macos-finalizer-empty-path/README.md).
Windows, stock Chrome, tagging, and publication did not start. Version 0.12.48
adds a checked-in fail-closed wrapper that exclusively owns sequential
aggregate-directory creation, invokes the existing finalizer, and returns one
canonical path to the caller. The wrapper has a source self-test and is
mandatory in CI, local deployment verification, and this runbook.

The sole exact v0.12.48 candidate passed its fresh 207-check quiet lane and
mandatory review of all six quiet screenshots. In the deliberate lane, the
separately authorized exact app/window/button action was observed once with a
quiet shared input seat, but the post-receipt gate rejected six advancing
same-share frames because their minimum estimated age was 1,040.609619140625 ms
against a 1,000 ms evidence-only ceiling. It failed closed before product
dispatch. Its sanitized terminal record is retained at
[`withdrawn-8b98da5-macos-share-authority-age-bound`](../../v0.12.48/computer/attempts/withdrawn-8b98da5-macos-share-authority-age-bound/README.md).
Windows, stock Chrome, tagging, and publication did not start. Version 0.12.49
raised only the post-receipt evidence ceiling to 2,500 ms, leaving a 500 ms
dispatch margin inside the product's unchanged three-second stale-frame
refusal. Exact share, target, geometry, advancement, and dispatch-time age
checks remain mandatory. Its fresh macOS lanes passed, but its one-shot
Windows run stopped before UI use when a redundant Toolhelp basename filter
returned no exact-image child for the authenticated helper worker. Version
0.12.50 corrected that filter. The exact v0.12.58 candidate later passed both
macOS lanes and Windows trust, but its native runner still saw zero enumerated
exact-image children while the authenticated helper session and worker PID were
stable for 265 polls. Version 0.12.68 binds the hello's controller PID to the
exact launched supervisor, the worker PID to the exact live helper image and
interactive session, and uses enumeration only to reject a conflicting helper
child. It retains the macOS authority boundary with fresh bytes.

The sole exact v0.12.46 candidate passed 68 of 69 quiet-lane assertions, then
failed closed before product dispatch because the independent active-receiver
probe expired after eight seconds while the command still remained inside the
server's fifteen-second call boundary. The fixture click count stayed zero,
and the user's foreground, AX focus, cursor, Space, and HID invariants held.
No deliberate, Windows, stock-Chrome, tag, or Release step followed. Its
sanitized terminal record is retained at
[`withdrawn-a423ed2-macos-active-receiver-probe-timeout`](../../v0.12.46/computer/attempts/withdrawn-a423ed2-macos-active-receiver-probe-timeout/README.md).
Version 0.12.47 binds the runner request and native SystemProbe cap to one
sixteen-second deadline, one second beyond the server call boundary, and adds
a source contract that prevents those limits from diverging again.

This directory defines a fresh deterministic evidence harness for the packaged
macOS v0.12.68 server and helper candidate. It is protocol infrastructure, not
...
The exact v0.12.42 candidate passed source, CI, artifact, checksum, architecture,
and GitHub-attestation gates, then stopped before package extraction completed
or any candidate executable ran. Its archive correctly included the new
`Local Browser Bridge.app` desktop host, but the bounded acceptance extractor
still allowed only the earlier server/helper archive layout. The mismatch was
terminal for that candidate; no quiet-seat readiness, screenshot, app-share,
Windows, stock-Chrome, tag, or Release followed. Its sanitized record is
retained at
[`withdrawn-014800d-macos-package-inventory-mismatch`](../../v0.12.42/computer/attempts/withdrawn-014800d-macos-package-inventory-mismatch/README.md).
Version 0.12.68 expands the exact bounded inventory and its adversarial tests
to cover the shipped desktop-host bundle without relaxing path, type, mode,
size, link, PAX, traversal, duplicate, or candidate-execution protections.
...
Release. Version 0.12.68 retains the exact-attempt trust rules and completes the
...
is bound to an atomic kill-on-close guard Job and then a named lifetime Job
before Worker or Intent state is published. Both allow only explicit child
breakaway; each runner-owned child is created suspended and atomically bound to
its private kill-on-close Job before resume.
...
self-test names and fixtures; it does not launch candidate binaries. Windows
readiness automatically requires a stable foreground and focus root in the runner's current interactive session
outside the fixture process while the fixture remains nonactivating and in the
background. The legacy-named `operator/foreground-arm-request.json` and
`operator/foreground-arm-received.json` files retain the matching schema-v3
request and ready proof; the read-only watcher validates them without a click,
manual relay, or operator action. Every marker and `Follow` projection remains
notification-only with `uiActionAllowed: false` and grants no consent or action
authority. Neither v0.12.26, v0.12.27, v0.12.42, nor v0.12.67 supplies candidate
bytes or evidence for this fresh harness. The coordinator assumes the
independently verified GitHub-attested candidate is trusted; it is not a
hostile-code sandbox or sudden-power-loss journal. Fresh packaged-platform and
browser acceptance remain mandatory.
...
This v0.12.68 harness reuses none of those evidence bytes.
...
Version 0.12.33 restored and enforced the byte-exact one-line self-test result,
then passed both fresh macOS lanes. Its single Windows attempt failed closed at
`build-dedicated-fixture` before a fixture executable, candidate product
process, Chrome action, tag, or Release existed. The sanitized terminal record
is retained at
...
Version 0.12.34 added explicit breakaway to both coordinator Jobs, atomic
private Job-list child creation, and a guard-plus-lifetime runner self-test that
compiles and executes the real fixture. Its trust gate and both fresh macOS
lanes passed. The Windows session was then interrupted after the old coordinator
...
withdrawn without a retry, tag, or Release. Version 0.12.68 retains the exact
share, target, geometry, advancement, and no-one-shot-observation checks while
setting the evidence-only authority-age ceiling to 2,500 ms, 500 ms inside the
product refusal boundary. It also retains the larger abortable wait and
v0.12.34 Job topology. It moves the
schema-2 reservation to the worker's last pre-intent boundary after staging,
Job/guard ownership, and runner preparation, and binds it to an opaque
coordinator instance. It reuses no earlier candidate or evidence byte.

The sole v0.12.35 candidate passed its source, artifact, provenance, and package
gates, then stopped in the macOS quiet lane before any candidate executable,
fixture, server, or helper process started. Repeated shared-seat activity reset
the quiet epoch, and a later native monitor sample became unknown, so the gate
failed closed without screenshots; Windows, stock Chrome, tagging, and
...
Version 0.12.68 keeps the same thresholds and refusal behavior, adds only a
fixed diagnostic vocabulary and source-only readiness probe, and reuses none
of those candidate or evidence bytes.

The exact v0.12.36 candidate then passed both fresh macOS lanes and their
independent evidence audit. Its Windows trust verifier created a fresh empty
destination, but the required protected owner-only ACL did not replace the
directory's inherited ACL. The verifier failed closed before creating a trust
subdirectory, cloning source, reading GitHub artifact metadata, downloading or
executing candidate bytes, reserving a Windows attempt, starting Computer Use,
...
Version 0.12.68 creates the destination and protected owner-only DACL in one
...
canceled. Version 0.12.68 does not reuse or relabel any prior binary,
...
tag remains protected historical metadata. Version 0.12.68 does not reuse any
...
Before spending a one-shot candidate attempt, run this from the exact reviewed
...
bytes. It compiles only the checked-in `SystemProbe.swift`, applies the exact
30-second, 60-transition, 500 ms cadence and immutable 30-minute deadline used
by the packaged lanes, removes its private temporary probe, and prints one
canonical JSON line. Exit status `0` with `status: "ready"` means the sampled
source-only epoch completed; exit status `1` with `status: "refused"` means it
did not. The record is explicitly `acceptanceEvidence: false` with
`candidateInvocations: 0`. It retains only fixed reset/unknown/probe categories
and aggregate counts—never probe stdout, errors, paths, PIDs, PSNs, window IDs,
coordinates, or HID counters. A ready result is scheduling guidance only and
cannot replace either fresh candidate-bound macOS lane or any later invariant.
...
- The server and helper report exactly v0.12.68, are universal
...
- The supplied executables are byte-for-byte identical to the copies inside
  `local-browser-bridge-v0.12.68-macos-universal.tar.gz`. The archive must be
  bound by a canonical `SHA256SUMS.txt` containing exactly the four v0.12.68
...
open an 18-second `ACTION` interval. It reads the current public share state as
...
client-side age estimate of at most 2.5 seconds. Authority refresh may consume
at most five seconds while preserving at least eight seconds for action work
and three seconds for completion publication. The
...
Computer Use provider. The schema-9 result records only the bound receipt chain
...
ATTEMPT_ROOT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.68-macos.XXXXXX")"
SCRATCH_PARENT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.68-scratch.XXXXXX")"
PACKAGE_ROOT="$PRIVATE_PARENT/lbb-v0.12.68-package-$RUN_NONCE"
...
ARCHIVE="$RELEASE_CANDIDATE_DIR/local-browser-bridge-v0.12.68-macos-universal.tar.gz"
MANIFEST="$RELEASE_CANDIDATE_DIR/SHA256SUMS.txt"
[[ ! -e "$PACKAGE_ROOT" && ! -L "$PACKAGE_ROOT" ]]
node evidence/v0.12.68/computer/helper-evidence-rig.mjs \
  --prepare-package \
...
pixels. In order, verify: ready with empty/zero counters; the v0.12.68 semantic
...
the v0.12.68 semantic value; semantic action 1/complete; persistent stream;
...
AGGREGATE_CANONICAL="$(
  bash scripts/finalize-macos-acceptance.sh \
    "$VERSION" \
    "$QUIET_CANONICAL" \
...
jq -e \
  ...
  --arg deliberate_sha256 "$DELIBERATE_REVIEWED_RESULT_SHA256" '
    ...
    .aggregateChecks.passingResultSchemaVersion == 9 and
...
`pwd -P` spellings to the finalizer wrapper. The wrapper creates and returns
one fresh owner-private aggregate directory before invoking the JavaScript
finalizer, so callers never combine dependent shell assignments. Never replace
the wrapper call with parallel or same-command assignments. The finalizer
rejects linked, non-private,
...
`evidence/v0.12.68/release/run-RUN_ID-attempt-ATTEMPT/macos/` on
`evidence/v0.12.68-release-run-RUN_ID-attempt-ATTEMPT`; its single commit parent
...
created. These records are diagnostic history, not v0.12.68 evidence:
...
The v0.12.2 candidate run is preserved byte-for-byte and is not v0.12.68
evidence:
...
They are linked for diagnostic history only and are not v0.12.68 results:
