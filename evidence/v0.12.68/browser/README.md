# Windows v0.12.68 stock-Chrome acceptance

This directory defines the release gate for the v0.12.68 browser extension and
...
gate expired with no mouse-down, mouse-up, or acknowledgement. The run failed
closed before observation or any product action and is terminal. Version
0.12.68 removes that operator action from Windows readiness. The runner accepts
only consecutive native samples proving that the existing user-owned
foreground and focus root remains stable and outside the dedicated fixture
process while the fixture stays nonactivating in the background. It does not
raise or activate the fixture, synthesize input, or wait for an operator. It
publishes matching schema-v3 request and ready-proof records under the legacy
foreground-arm filenames, and the read-only watcher validates both
automatically. Those records remain notification-only and grant no consent or
action authority. The new version requires fresh source, candidate bytes,
reservation, and evidence; no v0.12.67 byte or result can satisfy this gate.
...
asynchronous response, and requires a fresh connector after
`COMPUTER_HELPER_WATCHDOG`. It uses fresh source, candidate bytes, reservations,
and evidence; no v0.12.66 byte or result can satisfy this gate.

The sole exact v0.12.65 candidate was withdrawn during its one-shot Windows
computer acceptance. The recovery-suite `computer.share.start` command returned
`COMMAND_OUTCOME_UNKNOWN` after the helper connection ended once the command had
been enqueued, so the coordinator failed closed and permanently disallowed a
retry. The sanitized terminal record is frozen at
...
0.12.68 requires fresh source, candidate bytes, reservation, and evidence; no
v0.12.65 byte or result can satisfy this gate.

Version 0.12.57 stopped at a local macOS diagnostic before any GitHub
candidate, Windows, or Chrome execution. A native source frame advanced while
the harness collected cancellation baselines, and the server correctly refused
the retired authority as `COMPUTER_STALE_FRAME`. Version 0.12.68 refreshes the
exact share frame after those baselines and reports any future early refusal
directly. No v0.12.57 local package or diagnostic byte can satisfy this fresh
gate.

Version 0.12.56 stopped at a local macOS diagnostic before any GitHub
candidate, Windows, or Chrome execution. Its exact-view tracking observer was
timing-dependent: the first run missed a target-routed move, while an unchanged
package later passed all 207 checks. Version 0.12.57 observes the exact
window-number-bound event at `NSWindow.sendEvent`. No v0.12.56 local package or
diagnostic byte can satisfy this fresh gate.

Version 0.12.55 stopped at a local macOS diagnostic before any GitHub
candidate, Windows, or Chrome execution. The fixture's movement observer had
no exact-view tracking area and depended on first-responder routing. Version
0.12.56 installed that tracking area before the remaining window-boundary race
was exposed. No v0.12.55 local package or diagnostic byte can satisfy this
fresh gate.

Version 0.12.54 stopped at a source-built macOS diagnostic before any GitHub
candidate, Windows, or Chrome execution. AX state alone did not commit the
requested AppKit event receiver for a trajectory. Version 0.12.68 adds the
bounded target-only make-key commit and post-commit phase proof. No v0.12.54
local package or diagnostic byte can satisfy this fresh gate.

The exact v0.12.53 candidate never reached Windows or Chrome. Its source and
artifact gates passed, but the packaged macOS quiet lane stopped when a
cancellation-bound `computer.move` raced the target receiver and did not
advance the exact fixture's instrumentation. Version 0.12.68 re-proves the
requested-active receiver immediately before every trajectory event. No
v0.12.53 candidate byte or diagnostic record can satisfy this fresh gate.

The exact v0.12.52 candidate never reached Windows or Chrome. Its packaged
macOS quiet lane passed 158 checks and retained six reviewed screenshots, then
failed closed when an inactive exact-window `computer.move` did not advance
the target fixture's mouse-move instrumentation within the bounded dispatch
proof. Version 0.12.53 added the bounded target-receiver lease for that path.
No v0.12.52 candidate byte or diagnostic record can satisfy this fresh gate.

The sole exact v0.12.50 candidate passed its fresh packaged quiet macOS lane
and mandatory six-image review, then was withdrawn when the coordinator gave
the deliberate runner a nonexistent scratch parent. The runner stopped before
any deliberate-lane candidate byte or app-share UI action. The remote Windows
task was immediately told to stop, so no retained v0.12.50 Windows or Chrome
result can satisfy this gate. Version 0.12.68 requires fresh candidate bytes
and evidence.

The exact v0.12.49 candidate passed both fresh packaged macOS lanes. Its
one-shot Windows run then stopped before any Computer Use or Chrome action:
the authenticated disposable helper worker was connected and protocol-bound,
but the readiness probe's redundant Toolhelp basename filter returned zero
exact-image children. Version 0.12.50 removed that advisory prefilter while
retaining the exact supervisor binding, live volume/file identity, worker PID,
protocol session, interactive session, and stable consecutive observations.
Version 0.12.68 retains the repair. No v0.12.49 or v0.12.50 binary or result is
reusable by this fresh gate.

The exact v0.12.42 candidate never reached Windows or Chrome. Its macOS
pre-execution package preparer rejected the intentionally added
`Local Browser Bridge.app` desktop-host bundle because the acceptance inventory
still described the earlier archive layout. No candidate executable, UI
action, screenshot, evidence aggregate, tag, or Release followed. Version
0.12.68 aligns the bounded acceptance inventory and tests with the shipped
archive; it reuses none of the withdrawn candidate's bytes or evidence.
...
approval, or public Release. Version 0.12.68 retains the exact-attempt trust
...
runner-owned child. Its trust gate and both fresh macOS lanes passed, but the
...
0.12.68 retains that Job topology and moves the schema-2 reservation to the
...
history. Recovery remains serialized by the stable admission mutex and one
monotonic deadline. A raced same-name Job is never adopted, configured,
terminated, retained, or retried.
...
candidate and performs no Chrome action. Windows readiness is automatic and
requires a stable foreground and focus root in the runner's current interactive session outside the fixture
process while the fixture remains nonactivating and in the background. The
legacy-named `operator/foreground-arm-request.json` and
`operator/foreground-arm-received.json` files retain the automatic matching
...
authority. No v0.12.26, v0.12.27, or v0.12.67 candidate byte, result,
...
product dispatch when the post-receipt fresh-frame gate timed out. No aggregate,
...
and cannot satisfy any v0.12.68 browser gate.
...
permission probes, quiet-seat stabilization, or any candidate process launch.
...
and cannot satisfy or be reused by any v0.12.68 browser gate.
...
No v0.12.33 byte, result, screenshot, or marker can satisfy v0.12.68.
...
messages, and evidence cannot be reused for v0.12.68. An interrupted or failed
v0.12.68 attempt is preserved only as sanitized negative metadata and is never
...
1. the installed v0.12.68 extension and Chrome's debugger-use indicator;
...
and pass absolute paths to ordinary `git.exe` and `gh.exe` installations. Live
mode is deliberately non-interactive: `GH_TOKEN` must already be present when
the one-shot wrapper is invoked. An absent token fails before the clean trust
child, destination, repository clone, candidate download, or candidate byte is
created or read. Never launch the wrapper under `-NonInteractive` while relying
on a prompt. The following launcher captures the current `gh` credential without
printing it, makes the token available only for the bounded wrapper call, and
then restores the caller's prior environment:

```powershell
$trustedGh = (Get-Command gh.exe).Source
$previousGhToken = [Environment]::GetEnvironmentVariable("GH_TOKEN", "Process")
$ghToken = [string](& $trustedGh auth token --hostname github.com)
if ($LASTEXITCODE -ne 0 -or [String]::IsNullOrWhiteSpace($ghToken)) {
  throw "A GitHub token is unavailable for the one-shot Windows trust gate."
}
...
try {
  ...
  & .\scripts\verify-windows-release-candidate.ps1 `
    ...
    -TrustedGh $trustedGh
  if ($LASTEXITCODE -ne 0) { throw "The Windows trust gate failed." }
}
finally {
...
$privateParent = Join-Path ([IO.Path]::GetTempPath()) ("lbb-1235-acceptance-" + [Guid]::NewGuid().ToString("N"))
...
3. perform only required conditional toggles and load the verified directory
   ...
   is changed;
4. verify exactly one new test-owned card, version 0.12.68, no load errors, and
