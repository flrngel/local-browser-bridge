# macOS v0.12.48 persistent-share evidence candidate
...
Version 0.12.48 binds the runner request and native SystemProbe cap to one
...
macOS v0.12.48 server and helper candidate. It is protocol infrastructure, not
passing evidence. Any passing packaged result must be read from the immutable
[v0.12.48 GitHub Release](https://github.com/flrngel/local-browser-bridge/releases/tag/v0.12.48)
...
Version 0.12.48 expands the exact bounded inventory and its adversarial tests
...
Release. Version 0.12.48 retains the exact-attempt trust rules and completes the
...
This v0.12.48 harness reuses none of those evidence bytes.
...
withdrawn without a retry, tag, or Release. Version 0.12.48 retains the 1,000 ms
...
Version 0.12.48 keeps the same thresholds and refusal behavior, adds only a
...
Version 0.12.48 creates the destination and protected owner-only DACL in one
...
canceled. Version 0.12.48 does not reuse or relabel any prior binary,
...
tag remains protected historical metadata. Version 0.12.48 does not reuse any
...
- The server and helper report exactly v0.12.48, are universal
...
- The supplied executables are byte-for-byte identical to the copies inside
  `local-browser-bridge-v0.12.48-macos-universal.tar.gz`. The archive must be
  bound by a canonical `SHA256SUMS.txt` containing exactly the four v0.12.48
...
ATTEMPT_ROOT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.48-macos.XXXXXX")"
SCRATCH_PARENT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.48-scratch.XXXXXX")"
PACKAGE_ROOT="$PRIVATE_PARENT/lbb-v0.12.48-package-$RUN_NONCE"
...
ARCHIVE="$RELEASE_CANDIDATE_DIR/local-browser-bridge-v0.12.48-macos-universal.tar.gz"
MANIFEST="$RELEASE_CANDIDATE_DIR/SHA256SUMS.txt"
[[ ! -e "$PACKAGE_ROOT" && ! -L "$PACKAGE_ROOT" ]]
node evidence/v0.12.48/computer/helper-evidence-rig.mjs \
  --prepare-package \
...
pixels. In order, verify: ready with empty/zero counters; the v0.12.48 semantic
...
the v0.12.48 semantic value; semantic action 1/complete; persistent stream;
...
`evidence/v0.12.48/release/run-RUN_ID-attempt-ATTEMPT/macos/` on
`evidence/v0.12.48-release-run-RUN_ID-attempt-ATTEMPT`; its single commit parent
...
created. These records are diagnostic history, not v0.12.48 evidence:
...
The v0.12.2 candidate run is preserved byte-for-byte and is not v0.12.48
evidence:
...
They are linked for diagnostic history only and are not v0.12.48 results:
