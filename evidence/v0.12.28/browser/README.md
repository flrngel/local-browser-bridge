# Windows v0.12.28 stock-Chrome acceptance

This directory defines the release gate for the v0.12.28 browser extension and
...
approval, or public Release. Version 0.12.28 retains the exact-attempt trust
rules and adds a checked-in computer-use coordinator with atomic owner-private
state, an allowlisted Job-owned worker tree, persistent stream files, and exact
predecessor-chain validation. It starts the
read-only watcher only after the atomic foreground-arm request marker exists;
every `Follow` projection remains notification-only with
`uiActionAllowed: false` and grants no consent or action authority. No v0.12.26
or v0.12.27 candidate byte, result, screenshot, approval, or receipt is reused.
The expanded non-product coordinator self-test covers ACL and atomic-state
boundaries, environment and token isolation, Job containment, concurrent
streams, exact process identity and liveness, repeated handoff and request-ID
deduplication, and terminal-failure precedence.
...
messages, and evidence cannot be reused for v0.12.28. An interrupted or failed
v0.12.28 attempt is preserved only as sanitized negative metadata and is never
...
1. the installed v0.12.28 extension and Chrome's debugger-use indicator;
...
Do not copy PowerShell fragments into a new wrapper. Run the tagged source blob
...
handoff variables, checks its tagged blob, and fails closed.

First run
[`scripts/verify-windows-release-candidate.ps1`](../../../scripts/verify-windows-release-candidate.ps1)
to bind the repository, annotated tag object and peel, workflow run and attempt,
...
```powershell
$trustDestination = Join-Path ([IO.Path]::GetTempPath()) ("lbb-1228-" + [Guid]::NewGuid().ToString("N"))
...
$privateParent = Join-Path ([IO.Path]::GetTempPath()) ("lbb-1228-acceptance-" + [Guid]::NewGuid().ToString("N"))
...
3. perform only required conditional toggles and load the verified directory
   ...
   is changed;
4. verify exactly one new test-owned card, version 0.12.28, no load errors, and
