# macOS v0.12.28 persistent-share evidence candidate

This directory defines a fresh deterministic evidence harness for the packaged
macOS v0.12.28 server and helper candidate. It is protocol infrastructure, not
passing evidence; no v0.12.28 packaged run exists yet.
...
Release. Version 0.12.28 retains the exact-attempt trust rules and checkpoints
provisional checked-in Windows coordinator source with atomic owner-private
state, an allowlisted Job-owned worker tree, persistent stream files, exact predecessor-chain
validation, and no-retry state classification. It starts the read-only watcher
...
no consent or action authority. Neither v0.12.26 nor v0.12.27 supplies candidate
bytes or evidence for this fresh harness. Source contracts cover ACL and
atomic-state boundaries, environment and token isolation, provisional Job
recovery and containment, concurrent streams, exact process identity and
liveness, impossible-state refusal, repeated handoff and request-ID
deduplication, and terminal-failure precedence. The coordinator assumes the
independently verified GitHub-attested candidate is trusted; it is not a
hostile-code sandbox or sudden-power-loss journal. The stronger named-Job
fresh-acquisition boundary remains release-blocked pending the Windows-local
handoff and native validation.
...
This v0.12.28 harness reuses none of those evidence bytes.
...
canceled. Version 0.12.28 does not reuse or relabel any prior binary,
...
tag remains protected historical metadata. Version 0.12.28 does not reuse any
...
- The server and helper report exactly v0.12.28, are universal
...
- The supplied executables are byte-for-byte identical to the copies inside
  `local-browser-bridge-v0.12.28-macos-universal.tar.gz`. The archive must be
  bound by a canonical `SHA256SUMS.txt` containing exactly the four v0.12.28
...
`v0.12.28` tag. The source checkout and every candidate/evidence directory
...
ATTEMPT_ROOT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.28-macos.XXXXXX")"
SCRATCH_PARENT="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.28-scratch.XXXXXX")"
PACKAGE_ROOT="$PRIVATE_PARENT/lbb-v0.12.28-package-$RUN_NONCE"
...
ARCHIVE="$RELEASE_CANDIDATE_DIR/local-browser-bridge-v0.12.28-macos-universal.tar.gz"
MANIFEST="$RELEASE_CANDIDATE_DIR/SHA256SUMS.txt"
[[ ! -e "$PACKAGE_ROOT" && ! -L "$PACKAGE_ROOT" ]]
node evidence/v0.12.28/computer/helper-evidence-rig.mjs \
  --prepare-package \
...
pixels. In order, verify: ready with empty/zero counters; the v0.12.28 semantic
...
the v0.12.28 semantic value; semantic action 1/complete; persistent stream;
...
AGGREGATE_DIR="$(mktemp -d "$PRIVATE_PARENT/lbb-v0.12.28-aggregate.XXXXXX")"
...
`evidence/v0.12.28/release/run-RUN_ID-attempt-ATTEMPT/macos/` on
`evidence/v0.12.28-release-run-RUN_ID-attempt-ATTEMPT`; its single commit parent
...
created. These records are diagnostic history, not v0.12.28 evidence:
...
The v0.12.2 candidate run is preserved byte-for-byte and is not v0.12.28
evidence:
...
They are linked for diagnostic history only and are not v0.12.28 results:
