# Windows v0.12.4 stock-Chrome acceptance protocol

This directory defines the release gate for the v0.12.4 browser extension and
...
coordinator independently supplies version `0.12.4`, lowercase `FINAL_SHA`,
...
if ($Version -cne "0.12.4" -or $FinalSha -cnotmatch '^[0-9a-f]{40}$' -or
...
4. Verify exactly one enabled v0.12.4 card with no load error. Open the
...
|---|---|---|
| `extension-loaded` | `browser-01-extension-loaded.png` | Stock `chrome://extensions` shows exactly one enabled unpacked Local Browser Bridge v0.12.4 card, no load error, and Chrome's native debugger-use indicator while the bridge lease is active. |
