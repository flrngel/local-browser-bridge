...
private let fixtureTitle = "LBB v0.12.62 Persistent SCStream Evidence"
...
private final class FixtureWindow: NSWindow {
    var recordExactMouseMoved: (() -> Void)?

    override func sendEvent(_ event: NSEvent) {
        if event.type == .mouseMoved, event.windowNumber == windowNumber {
...
private final class FixtureView: NSView, NSTextFieldDelegate {
    ...
    func recordExactWindowMoveDelivery() {
        // This is evidence-only exact-window instrumentation. Keep it bounded,
        // leave every functional fixture field untouched, and persist without
        // redrawing so cancellation waits for NSWindow delivery rather than a
        // later tracking-area or first-responder callback.
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        "Local Browser Bridge — v0.12.62 persistent SCStream".draw(
...
private final class FixtureDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        window.recordExactMouseMoved = { [weak view] in
            view?.recordExactWindowMoveDelivery()
        }
