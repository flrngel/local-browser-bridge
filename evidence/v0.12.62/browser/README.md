# Windows v0.12.62 stock-Chrome acceptance

This directory defines the release gate for the v0.12.62 browser extension and
...
the retired authority as `COMPUTER_STALE_FRAME`. Version 0.12.62 refreshes the
...
requested AppKit event receiver for a trajectory. Version 0.12.62 adds the
...
advance the exact fixture's instrumentation. Version 0.12.62 re-proves the
...
result can satisfy this gate. Version 0.12.62 requires fresh candidate bytes
and evidence.
...
Version 0.12.62 retains the repair. No v0.12.49 or v0.12.50 binary or result is
...
0.12.62 aligns the bounded acceptance inventory and tests with the shipped
...
approval, or public Release. Version 0.12.62 retains the exact-attempt trust
...
0.12.62 retains that Job topology and moves the schema-2 reservation to the
...
and cannot satisfy any v0.12.62 browser gate.
...
and cannot satisfy or be reused by any v0.12.62 browser gate.
...
No v0.12.33 byte, result, screenshot, or marker can satisfy v0.12.62.
...
messages, and evidence cannot be reused for v0.12.62. An interrupted or failed
v0.12.62 attempt is preserved only as sanitized negative metadata and is never
...
1. the installed v0.12.62 extension and Chrome's debugger-use indicator;
...
3. perform only required conditional toggles and load the verified directory
   ...
   is changed;
4. verify exactly one new test-owned card, version 0.12.62, no load errors, and
