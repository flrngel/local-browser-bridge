...
const EXPECTED_VERSION = "0.12.10";
...
const FIXTURE_TITLE = "LBB v0.12.10 Persistent SCStream Evidence";
const SIBLING_FIXTURE_TITLE = "LBB v0.12.10 Same-PID Sibling Receiver";
const SEMANTIC_VALUE = "v0.12.10-semantic-value";
...
async function waitForDeliberatePointerActivity(baseline) {
  ...
  log(
    "ACTION REQUIRED: continuously move the shared hardware pointer without clicking; keep moving until the post-resize action finishes.",
  );
  await waitFor(
    "separately authorized deliberate shared-pointer activity",
    async () => {
      const sample = processProbe(systemProbeBinary);
      return (
        sample.pointerBoundaryActivityObserved === true ||
        hidPointerCounterProgress(baseline, sample) === "advanced"
      );
    },
    DELIBERATE_CONCURRENCY_WAIT_MS,
  );
  log("Deliberate shared-pointer activity observed; continue moving for three seconds.");
}
...
async function main() {
  ...
  scratchDir = await mkdtemp(join(scratchParent, "lbb-v0.12.10-scstream-"));
  ...
  const result = {
    ...
    candidateNotice: "This becomes release evidence only after the supplied checksum manifest and archive are published immutably for v0.12.10.",
    ...
    limitations: [
      "This run proves the supplied v0.12.10 macOS release-candidate archive, not an immutable GitHub release until those exact checksums are published.",
...
try {
  ...
  if (scratchDir?.startsWith(`${scratchParent}/lbb-v0.12.10-scstream-`)) {
