# macOS v0.12.10 persistent-share evidence candidate

This directory defines a fresh deterministic evidence harness for the packaged
macOS v0.12.10 server and helper candidate. It is protocol infrastructure, not
passing evidence; no v0.12.10 packaged run exists yet.

The exact v0.12.9 candidate failed closed at the first semantic mutation after
a global cursor-position change could not be attributed to the helper or to
concurrent shared-seat activity. Windows and stock-Chrome acceptance never
started, publication was canceled, and no v0.12.9 Release exists. Version
0.12.10 does not reuse or relabel any prior binary, screenshot, result, marker,
notification, log, or other generated evidence byte.
...
- The server and helper report exactly v0.12.10, are universal
...
- The supplied executables are byte-for-byte identical to the copies inside
  `local-browser-bridge-v0.12.10-macos-universal.tar.gz`. The archive must be
  bound by a canonical `SHA256SUMS.txt` containing exactly the four v0.12.10
...
input. After the first quiet cells, the runner prints one `ACTION REQUIRED`
line before the post-resize action. Move the shared pointer continuously without
clicking, and keep moving until that action finishes. The runner waits up to 300
seconds for the first HID-system activity, then immediately runs a bounded
two-second target-routed product action so the independent and product
boundaries can both observe the deliberate concurrency. The same run must
contain at least one `quiet` cell and at least one `contaminated` cell, keep the
helper route and preservation claims valid, and contain no `unknown` cell.
HID-system, Raw Input, and injected-input booleans remain supplemental and are
not individually required for product success. Generated bytes from a different
attempt are never merged or reused.

The retained schema-v4 result contains only bounded booleans and state enums
for this boundary. Raw cursor coordinates and raw platform activity counters
exist only inside the ephemeral probe/runner comparison and are never written
to the result, log, or failure diagnostics.
...
PACKAGE_ROOT="$(mktemp -d "$SCRATCH_PARENT/lbb-v0.12.10-package.XXXXXX")"
trap 'rm -rf -- "$PACKAGE_ROOT"' EXIT
tar -xzf "$RELEASE_CANDIDATE_DIR/local-browser-bridge-v0.12.10-macos-universal.tar.gz" \
  -C "$PACKAGE_ROOT"

node evidence/v0.12.10/computer/helper-evidence-rig.mjs \
  "$PACKAGE_ROOT/local-browser-bridge" \
  "$PACKAGE_ROOT/Local Computer Helper.app/Contents/MacOS/local-computer-helper" \
  evidence/v0.12.10/computer \
  "$SCRATCH_PARENT" \
  "$RELEASE_CANDIDATE_DIR/local-browser-bridge-v0.12.10-macos-universal.tar.gz" \
...
created. These records are diagnostic history, not v0.12.10 evidence:
...
The v0.12.2 candidate run is preserved byte-for-byte and is not v0.12.10
evidence:
...
They are linked for diagnostic history only and are not v0.12.10 results:
