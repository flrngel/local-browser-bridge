# Windows v0.12.10 stock-Chrome acceptance protocol

This directory defines the release gate for the v0.12.10 browser extension and
...
No v0.12.10 stock-Chrome acceptance result exists yet. This protocol requires a
fresh v0.12.10 candidate and output directory; it cannot reuse any prior
operator record, notification, image, or generated evidence byte. The exact
v0.12.9 macOS candidate failed closed at its first semantic mutation after a
global cursor-position change could not be attributed to the helper or
concurrent shared-seat activity. Windows and stock-Chrome acceptance never
started, publication was canceled, and no v0.12.9 Release exists.
...
coordinator independently supplies version `0.12.10`, lowercase `FINAL_SHA`,
...
if ($Version -cne "0.12.10" -or $FinalSha -cnotmatch '^[0-9a-f]{40}$' -or
...
4. Verify exactly one enabled v0.12.10 card with no load error. Open the
...
|---|---|---|
| `extension-loaded` | `browser-01-extension-loaded.png` | Stock `chrome://extensions` shows exactly one enabled unpacked Local Browser Bridge v0.12.10 card, no load error, and Chrome's native debugger-use indicator while the bridge lease is active. |
