...
- The extension authenticates with a canonical URL-safe encoding of 32 random bytes stored outside the repository. Empty, weak, malformed, or permissive persisted tokens are rotated atomically. Unix stores the token under mode `0600` in a mode `0700` dedicated directory and replaces a symlink entry without following it. Windows uses a protected DACL containing one full-control ACE for the current user on the token and its dedicated parent directory, refuses to touch reparse points and multiply linked token files, and replaces a rotatable token with write-through `MoveFileExW` rather than a delete-then-rename gap.
