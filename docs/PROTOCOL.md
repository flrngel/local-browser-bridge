...
**What is and is not proven.** The ref grammar, the sanitizers, the coordinate translation, the merge and budget arithmetic, the fail-closed staleness paths, and the in-frame agent's proofs are covered by the Rust and Node contract suites. The five CDP behaviours the design depends on are, since the 0.11.0 live bundle, confirmed in practice as well — on one genuine out-of-process iframe at depth 1, in Google Chrome for Testing 152 against the published artifacts ([../evidence/v0.11.0/README.md](../evidence/v0.11.0/README.md)): `Target.attachedToTarget` reached the extension with a populated `sessionId`; `chrome.debugger.sendCommand({tabId, sessionId})` routed to the child session; `DOM.getBoxModel`'s content quad was in the parent frame's viewport space (the child button published `bounds {x:266, y:187}`, its frame-local position plus the frame's `{x:250, y:123}` offset); `Input.dispatchMouseEvent` on the page target hit-tested into the OOPIF renderer, and the child document recorded `event.isTrusted === true`; and the isolated world survived long enough to prove the target. That run is one frame at depth 1 in one Chrome build, so what remains proven only against the harnesses is the residue: a Chrome whose `chrome.debugger` ignores the `sessionId` key, a same-process frame skip and a live `FRAME_ACTION_UNSUPPORTED` refusal, nesting deeper than depth 1, and isolated-world survival across a frame's own same-document navigation. Each of those still degrades to a refusal rather than a wrong click. In particular, the first child command of every lease is a discriminating routing probe: the child session must return its own frame id, and a Chrome whose `chrome.debugger` ignores the `sessionId` key answers for the root frame instead, at which point frame support is disabled for the lease (`frameSummary.supported: false`, `reason: "session_routing_unverified"`), every attached record is dropped, and the observation is exactly what it would have been without frame support. `minimum_chrome_version` stays `118`.
