...
Protocol version: `1`. Package version examples below use `0.12.2`.
...
The extension also injects a separate page-owned pill, Stop button, and synthetic pointer. That content overlay is not Chrome's native warning. Its host identifier is freshly randomized for each document. Lease start, same-tab action reuse, and capture restoration require a new two-frame visible acknowledgement (capture begin instead acknowledges the deliberately hidden pill), and a 500 ms content watchdog requests exact-session revocation if the popover, host, pill, or Stop control becomes hidden or disconnected outside capture. The extension validates the sender tab and exact lease session before honoring that request. A JavaScript dialog is the sole pre-action exception because it freezes the renderer; `page.handleDialog` resolves it in the browser process, after which normal paint checks resume.

These checks narrow but cannot eliminate a page-owned UI race: a hostile document can discover or suppress the overlay immediately after an acknowledgement and before the following trusted CDP dispatch or watchdog tick. The paint acknowledgement is not atomically bound to input. Chrome's browser-owned warning and Cancel action remain the page-independent authority and handback surface throughout the lease, and popup **Release control** remains available.
...
The explicit browser freshness set is `browser.control.start`, `page.observe`, and every controlled `page.*` command except `page.waitFor`. `page.observe` is read-only but advances the turn before later capture awaits; `browser.control.start` can renew or establish a lease before returning; the remaining entries can cross a renderer or CDP side-effect boundary. `status`, `browser.control.status`, `tabs.list`, and `page.waitFor` neither consume nor advance controlled-page freshness. `tabs.activate`, `tabs.new`, and `tabs.close` are browser-process mutations but do not consume a lease turn, DOM generation, screenshot, or element ref, so they are not allowed to clear or advance unrelated controlled-page authority. Their canceled result is still outcome-unknown and must never be retried under a fresh `callId`; use `tabs.list` to reconcile tab state first. `tabs.new` commits every created tab's bridge provenance before group decoration, and a late creation holds the serialized extension queue until provenance persistence finishes; this also keeps an omitted-URL `about:blank` visible to Safe-mode reconciliation. The original `callId` remains non-dispatching, so no retry behavior is inferred from a stale observation.
