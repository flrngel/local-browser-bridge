...
An explicit browser-control lease attaches `chrome.debugger` to one tab. The held attachment supplies trusted CDP input and causes Chrome to show its browser-owned debugging warning. An isolated content script supplies structured DOM observation and the extension-owned page pill, Stop control, and synthetic pointer. The page-owned host identifier is randomized per document; control start, reuse, and capture restoration are paint-acknowledged, and a short exact-session watchdog revokes on detected loss outside capture. This does not atomically bind page paint to later CDP input, so Chrome's browser-owned warning and Cancel remain authoritative.
