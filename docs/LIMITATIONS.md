...
Capture dimensions are fixed when `SCStream` starts, so restart a share after a major resize, aspect-ratio change, or move between displays with different scale factors. macOS 14 and later can use the filter's point-to-pixel scale before the transport cap; macOS 13 falls back to the validated enumerated window size and can produce lower-resolution Retina frames. After startup, blank, protected, or other non-complete samples can produce no new observation without immediately ending the share; the three-second action-frame lease still expires normally.
