...
- The in-page pill and Stop control are defense in depth, not browser chrome. Their host is randomized per document, same-tab actions require a fresh paint acknowledgement, and a 500 ms watchdog revokes after detected loss outside an intentional capture. A hostile page can still suppress page-owned UI in the interval after acknowledgement and before input or the next watchdog tick. Chrome's native debugger warning/Cancel and popup release remain independent.
