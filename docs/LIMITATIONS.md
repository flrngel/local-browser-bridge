...
Setup requirements such as browser loading, matching versions, macOS permissions, and the Windows interactive session are documented separately in [Windows](INSTALL_WINDOWS.md) and [macOS](INSTALL_MACOS.md) install guides. This page describes constraints that remain after correct setup.
...
- Nothing is injected into the controlled page. The only visible, page-independent control indicators are Chrome's own "Local Browser Bridge started debugging this browser" warning and Cancel button, the named Local Browser Bridge tab group, and Release control/Resume in the extension popup — a hostile page cannot hide, spoof, or race any of the three, because none of them are page content.
...
The checked-in [evidence index](../evidence/) records what was actually run. A code path, unit test, or transport acknowledgement alone is not evidence that an application accepted the action or that the user's desktop remained unchanged. For the release-by-release history of what was attempted and why it did or did not ship, see [Release-attempt history](history/release-attempts.md).
...
- [Computer-use research](research/COMPUTER_USE_RESEARCH.md) — pinned implementation and community review
