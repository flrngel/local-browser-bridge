...
- API cancellation is cooperative and immediately removes server-side authority, but it is not rollback. A native call already inside an operating-system provider can finish cleanup after the HTTP 202; Windows retains the 12-second disposable-worker containment boundary.
