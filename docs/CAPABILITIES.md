...
- Browser control requires Chromium 140+. Cross-origin child-session routing first appeared in Chromium 125, but version 0.12.26 retains the overall floor so persisted extension storage can be restricted to trusted contexts.
...
- Version 0.12.26 is the fresh pending candidate. It retains the v0.12.23 classifier separation and, after the app-share `ACTION` receipt, requires a strictly newer frame from the same share, exact target, and unchanged geometry within the reserved deadline before deriving click authority. Before either macOS lane executes candidate bytes, its native SystemProbe gate requires a 30-second sampled quiet epoch with at least 60 stable 500 ms transitions; pointer, foreground/focus/front-window, cursor, or active-Space activity resets the epoch under one immutable 30-minute deadline, while unknown monitoring fails immediately. This is a preflight, not seat isolation or proof of continuous monitoring, and every per-action/whole-run boundary remains unchanged. Its action records separate sealed target-route provenance, operating-system API acceptance, and application-owned postconditions. `cursorPositionUnchanged` is diagnostic; `helperGlobalPointerPreservation`, `sharedPointerBoundaryCorroborated`/`sharedPointerBoundaryState`, `hidSystemPointerActivityObserved`, `pointerActivityMonitorHealthy`, and `sharedPointerActivityState` carry conservative pointer-attribution state. The deliberate macOS lane uses a separate nonactivating app with one exact bundle, window, and accessibility button. A separately authorized app-share controller presses that button once; the app writes create-once request/start/complete records around the real product action. Those records are orchestration evidence, not notification-only and not product-control authority. `appShareHandoff.sharedHidInputObserved` is `null` in the quiet lane because no app-share transaction occurs and `false` in the deliberate lane when its HID boundary observed no pointer or keyboard activity. The other narrowed fields report the acceptance-button action, app-share surface at product boundaries, and sampled shared-context equality—not provider identity or continuous custody. Endpoint samples and cumulative HID counters cannot prove zero transient programmatic changes, a continuous monitor, atomic provider identity, or zero transient focus/window manipulation. The former v0.12.20 physical-pointer lane remains historical, optional adversarial coverage and cannot replace this gate. These source capabilities still need version-specific packaged macOS, Windows, stock-Chrome, and immutable-release proof.
