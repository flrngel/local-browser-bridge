...
- Browser control requires Chromium 140+. Cross-origin child-session routing first appeared in Chromium 125, but version 0.12.28 retains the overall floor so persisted extension storage can be restricted to trusted contexts.
...
- Version 0.12.27 added strict exact-attempt attestation selection and atomic five-file local deployment. It passed both packaged macOS lanes, but its ad-hoc Windows launcher lost the runner's terminal streams and exit fact before an evidence directory appeared; candidate-byte execution could not be proved or excluded, so the version was withdrawn without a Windows action, Chrome run, or Release. Version 0.12.28 is a blocked source target, not a candidate. Product, protocol, and platform capabilities remain unchanged. Its provisional checked-in Windows coordinator uses flush-before-move create-once state, an owner-private allowlisted worker environment, kill-on-close process-tree containment, persistent separate stream files, exact worker/runner PID-plus-start-time binding, and complete predecessor-chain validation. Its stronger named-Job recovery and fresh-name refusal state machine remain pending in the [Windows acceptance handoff](WINDOWS_ACCEPTANCE_HANDOFF.md). It starts its read-only watcher only after the atomic foreground-arm request marker exists. Every `Follow` result remains notification-only with `uiActionAllowed: false` and cannot grant consent or action authority; every started or uncertain attempt is terminal with no retry. Source contracts cover ACL and atomic-state boundaries, environment and token isolation, provisional Job recovery and containment, concurrent streams, exact process identity and liveness, impossible-state refusal, repeated handoff and request-ID deduplication, and terminal-failure precedence, but native Windows validation is not complete. The coordinator assumes the independently verified GitHub-attested candidate is trusted and is not a hostile-code sandbox. All quiet-seat, per-action, whole-run, Windows, stock-Chrome, evidence, and immutable-release gates still require fresh proof.
