...
- Browser control requires Chromium 140+. Cross-origin child-session routing first appeared in Chromium 125, but version 0.12.19 retains the overall floor so persisted extension storage can be restricted to trusted contexts.
...
- Version 0.12.19 is the fresh pending candidate. Before either macOS lane executes candidate bytes, its native SystemProbe gate requires 30 continuous quiet seconds and at least 60 stable 500 ms transitions; pointer, foreground/focus/front-window, cursor, or active-Space activity resets the epoch under one immutable 30-minute deadline, while unknown monitoring fails immediately. This is a preflight, not seat isolation, and every per-action/whole-run boundary remains unchanged. Its action records separate sealed target-route provenance, operating-system API acceptance, and application-owned postconditions. `cursorPositionUnchanged` is diagnostic; `helperGlobalPointerPreservation`, `sharedPointerBoundaryCorroborated`/`sharedPointerBoundaryState`, `hidSystemPointerActivityObserved`, `pointerActivityMonitorHealthy`, and `sharedPointerActivityState` carry conservative pointer-attribution state. Its macOS acceptance handoff uses a runner-owned nonactivating prompt plus notification-only markers. Before deliberate product dispatch, contamination resets a clean cumulative-counter arm under the same absolute deadline; unknown monitoring and every post-dispatch contamination still fail closed. Only sustained independent HID activity and fresh product/independent action boundaries can satisfy that gate. These source capabilities still need version-specific packaged macOS, Windows, stock-Chrome, and immutable-release proof.
