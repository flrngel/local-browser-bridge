...
- Browser control requires Chromium 140+. Cross-origin child-session routing first appeared in Chromium 125, but version 0.12.68 retains the overall floor so persisted extension storage can be restricted to trusted contexts.
...
See [Windows](INSTALL_WINDOWS.md) or [macOS](INSTALL_MACOS.md) for the user flow.
...
Live acceptance evidence for a specific version lives in [`evidence/`](../evidence/)
and, for the release-by-release history of what was tried and why, in
...
See [Computer-use research](research/COMPUTER_USE_RESEARCH.md) for the complete source comparison and the distinction between capture, input routing, and isolation.
