...
- Browser control requires Chromium 140+. Cross-origin child-session routing first appeared in Chromium 125, but version 0.12.27 retains the overall floor so persisted extension storage can be restricted to trusted contexts.
...
- Version 0.12.27 is the fresh pending candidate. Product, protocol, and platform capabilities are unchanged from v0.12.26; release tooling changes in two places. Every returned attestation must be well formed and bind the same run, source, tag, workflow, GitHub-hosted runner, and exact subject, and exactly one must bind the current attempt. Local deployment verifies an exact five-file staging set and atomically replaces only a proven generated-artifact `dist/`, retaining rollback state on failure. Zero, duplicate, malformed, wrong-subject, or polluted-output results fail closed. The candidate retains the v0.12.23 classifier separation and same-share/same-target/same-geometry authority refresh, the source-bound Windows GUI fixture, and all quiet-seat, per-action, whole-run, Windows, stock-Chrome, and immutable-release gates. The former v0.12.20 physical-pointer lane remains historical optional coverage and cannot replace this gate.
