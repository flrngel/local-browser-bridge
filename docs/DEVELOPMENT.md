...
Version 0.12.45 retains the nested-Job repair and moves the persistent attempt
...
immutable sanitized negative record is linked from the v0.12.45 harness
README. Version 0.12.45 does not relax that gate. It adds fixed reset/unknown
...
exact v0.12.45/schema-2 marker field set and order, a fresh non-expired publication,
...
and caller-selected alternate parents. Version 0.12.45 resolves the prospective
...
bundle that the release package correctly contained. Version 0.12.45 adds that
bundle's exact seven-entry app layout to the acceptance inventory and exercises
the complete 17-entry archive in the package-preparer contract test.
...
For v0.12.45, the canonical evidence branch is
`evidence/v0.12.45-release-run-RUN_ID-attempt-RUN_ATTEMPT`, and its additions
live below
`evidence/v0.12.45/release/run-RUN_ID-attempt-RUN_ATTEMPT/`. The five primary
machine records are:
