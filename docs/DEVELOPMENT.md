...
Version 0.12.59 retains the nested-Job repair and moves the persistent attempt
...
immutable sanitized negative record is linked from the v0.12.59 harness
README. Version 0.12.59 does not relax that gate. It adds fixed reset/unknown
...
exact v0.12.59/schema-2 marker field set and order, a fresh non-expired publication,
...
and caller-selected alternate parents. Version 0.12.59 resolves the prospective
...
action, tag, or Release followed. Version 0.12.59 binds the authenticated
`controllerProcessId` to the exact launched supervisor, requires the reported
worker's live full image path and interactive session to match, and keeps
Toolhelp enumeration as a conflicting-child refusal rather than requiring it
to rediscover the authenticated worker. It requires fresh bytes and evidence.
...
0.12.59 preserves both changes and repairs the terminal v0.12.58 Windows
helper-readiness false negative with fresh release identity.
...
probe verifies every direct child by the exact live full image path instead of
prefiltering on Toolhelp's advisory executable basename. Version 0.12.59 also
binds the authenticated controller process to the exact launched supervisor
and the worker to its live full image path and interactive session. Toolhelp
enumeration remains a conflicting-child refusal but is no longer required to
rediscover that authenticated worker.
...
For v0.12.59, the canonical evidence branch is
`evidence/v0.12.59-release-run-RUN_ID-attempt-RUN_ATTEMPT`, and its additions
live below
`evidence/v0.12.59/release/run-RUN_ID-attempt-RUN_ATTEMPT/`. The five primary
machine records are:
