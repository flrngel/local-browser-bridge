...
Version 0.12.53 retains the nested-Job repair and moves the persistent attempt
...
immutable sanitized negative record is linked from the v0.12.53 harness
README. Version 0.12.53 does not relax that gate. It adds fixed reset/unknown
...
exact v0.12.53/schema-2 marker field set and order, a fresh non-expired publication,
...
and caller-selected alternate parents. Version 0.12.53 resolves the prospective
...
0.12.53 preserves both changes with fresh release identity after the terminal
v0.12.50 coordinator failure.
...
prefiltering on Toolhelp's advisory executable basename. Version 0.12.53 makes
no boundary change; it provides fresh immutable candidate identity after the
v0.12.50 coordinator failure.
...
For v0.12.53, the canonical evidence branch is
`evidence/v0.12.53-release-run-RUN_ID-attempt-RUN_ATTEMPT`, and its additions
live below
`evidence/v0.12.53/release/run-RUN_ID-attempt-RUN_ATTEMPT/`. The five primary
machine records are:
