...
Version 0.12.66 retains the nested-Job repair and moves the persistent attempt
...
or publication. Version 0.12.66 uses the same 18-second bound in every producer
and consumer and self-tests a valid receipt whose action exceeds ten seconds.
...
immutable sanitized negative record is linked from the v0.12.66 harness
README. Version 0.12.66 does not relax that gate. It adds fixed reset/unknown
...
exact v0.12.66/schema-2 marker field set and order, a fresh non-expired publication,
...
and caller-selected alternate parents. Version 0.12.66 resolves the prospective
...
Version 0.12.66 uses fresh source, candidate bytes, reservation, and evidence;
...
under `evidence/v0.12.62/computer/attempts/`. Version 0.12.66 requires fresh
bytes and evidence.
...
0.12.66 preserves all prior changes with fresh release identity.
...
0.12.66 retains the same checks with fresh candidate and reservation identity.
...
For v0.12.66, the canonical evidence branch is
`evidence/v0.12.66-release-run-RUN_ID-attempt-RUN_ATTEMPT`, and its additions
live below
`evidence/v0.12.66/release/run-RUN_ID-attempt-RUN_ATTEMPT/`. The five primary
machine records are:
