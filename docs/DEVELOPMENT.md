...
Version 0.12.60 retains the nested-Job repair and moves the persistent attempt
...
immutable sanitized negative record is linked from the v0.12.60 harness
README. Version 0.12.60 does not relax that gate. It adds fixed reset/unknown
...
exact v0.12.60/schema-2 marker field set and order, a fresh non-expired publication,
...
and caller-selected alternate parents. Version 0.12.60 resolves the prospective
...
copy, and keeps Toolhelp as a conflicting-child refusal. It requires fresh
bytes and evidence.
...
path-alias false negative above. Version 0.12.60 preserves the prior changes and
repairs that exact v0.12.59 boundary with fresh release identity.
...
For v0.12.60, the canonical evidence branch is
`evidence/v0.12.60-release-run-RUN_ID-attempt-RUN_ATTEMPT`, and its additions
live below
`evidence/v0.12.60/release/run-RUN_ID-attempt-RUN_ATTEMPT/`. The five primary
machine records are:
