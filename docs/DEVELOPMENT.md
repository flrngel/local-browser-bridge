...
Version 0.12.41 retains the nested-Job repair and moves the persistent attempt
...
immutable sanitized negative record is linked from the v0.12.41 harness
README. Version 0.12.41 does not relax that gate. It adds fixed reset/unknown
...
exact v0.12.41/schema-2 marker field set and order, a fresh non-expired publication,
...
and caller-selected alternate parents. Version 0.12.41 resolves the prospective
...
For v0.12.41, the canonical evidence branch is
`evidence/v0.12.41-release-run-RUN_ID-attempt-RUN_ATTEMPT`, and its additions
live below
`evidence/v0.12.41/release/run-RUN_ID-attempt-RUN_ATTEMPT/`. The five primary
machine records are:
