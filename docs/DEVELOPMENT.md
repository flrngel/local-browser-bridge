...
Version 0.12.62 retains the nested-Job repair and moves the persistent attempt
...
immutable sanitized negative record is linked from the v0.12.62 harness
README. Version 0.12.62 does not relax that gate. It adds fixed reset/unknown
...
exact v0.12.62/schema-2 marker field set and order, a fresh non-expired publication,
...
and caller-selected alternate parents. Version 0.12.62 resolves the prospective
...
saturates any future lead to zero elapsed age and requires fresh bytes and
evidence.
...
the valid future-lead boundary. Version 0.12.62 preserves all prior changes and
repairs the newly observed boundary with fresh release identity.
...
0.12.62 retains the same identity checks and saturates that future lead to zero
elapsed age while preserving positive age.
...
For v0.12.62, the canonical evidence branch is
`evidence/v0.12.62-release-run-RUN_ID-attempt-RUN_ATTEMPT`, and its additions
live below
`evidence/v0.12.62/release/run-RUN_ID-attempt-RUN_ATTEMPT/`. The five primary
machine records are:
