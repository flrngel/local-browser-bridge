...
Version 0.12.51 retains the nested-Job repair and moves the persistent attempt
...
immutable sanitized negative record is linked from the v0.12.51 harness
README. Version 0.12.51 does not relax that gate. It adds fixed reset/unknown
...
exact v0.12.51/schema-2 marker field set and order, a fresh non-expired publication,
...
and caller-selected alternate parents. Version 0.12.51 resolves the prospective
...
0.12.51 preserves both changes with fresh release identity after the terminal
...
prefiltering on Toolhelp's advisory executable basename. Version 0.12.51 makes
...
For v0.12.51, the canonical evidence branch is
`evidence/v0.12.51-release-run-RUN_ID-attempt-RUN_ATTEMPT`, and its additions
live below
`evidence/v0.12.51/release/run-RUN_ID-attempt-RUN_ATTEMPT/`. The five primary
machine records are:
