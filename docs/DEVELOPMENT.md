...
Version 0.12.44 retains the nested-Job repair and moves the persistent attempt
...
immutable sanitized negative record is linked from the v0.12.44 harness
README. Version 0.12.44 does not relax that gate. It adds fixed reset/unknown
...
exact v0.12.44/schema-2 marker field set and order, a fresh non-expired publication,
...
and caller-selected alternate parents. Version 0.12.44 resolves the prospective
...
bundle that the release package correctly contained. Version 0.12.44 adds that
...
For v0.12.44, the canonical evidence branch is
`evidence/v0.12.44-release-run-RUN_ID-attempt-RUN_ATTEMPT`, and its additions
live below
`evidence/v0.12.44/release/run-RUN_ID-attempt-RUN_ATTEMPT/`. The five primary
machine records are:
