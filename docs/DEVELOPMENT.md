...
exact v0.12.17/schema-2 field set and order, a fresh non-expired publication,
...
`evidence/v0.12.17-release-run-RUN_ID-attempt-RUN_ATTEMPT`; its tip must be one
...
`evidence/v0.12.17/release/run-RUN_ID-attempt-RUN_ATTEMPT/`. The five required
machine records are:
...
receipt="$(jq -cn \
  ...
  --arg evidence_ref "refs/heads/evidence/v0.12.17-release-run-EXACT_GITHUB_RUN_ID-attempt-EXACT_GITHUB_RUN_ATTEMPT" \
