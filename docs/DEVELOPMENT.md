...
Version 0.12.61 retains the nested-Job repair and moves the persistent attempt
...
immutable sanitized negative record is linked from the v0.12.61 harness
README. Version 0.12.61 does not relax that gate. It adds fixed reset/unknown
...
exact v0.12.61/schema-2 marker field set and order, a fresh non-expired publication,
...
and caller-selected alternate parents. Version 0.12.61 resolves the prospective
...
rational QPC domain, maps only one QPC tick plus one 100 ns quantum of future
uncertainty to zero age, rounds positive age upward, and requires fresh bytes
and evidence.
...
but exposed the WGC/QPC conversion failure above. Version 0.12.61 preserves the
prior changes and repairs that new boundary with fresh release identity.
...
authenticated worker. Version 0.12.61 retains those checks and corrects only
the WGC compositor-age conversion described above.
...
For v0.12.61, the canonical evidence branch is
`evidence/v0.12.61-release-run-RUN_ID-attempt-RUN_ATTEMPT`, and its additions
live below
`evidence/v0.12.61/release/run-RUN_ID-attempt-RUN_ATTEMPT/`. The five primary
machine records are:
