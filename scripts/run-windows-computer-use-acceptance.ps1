    ...
    try {
        ...
        if (-not $acceptanceCompleted) {
            Write-TerminalFailure $files "runner-finished" "runner-started-terminal" $(
                if (-not $watcherAccepted -or -not $handoffPublished) { "watcher-handoff-missing" }
...
'@
        ...
        $inputServer = [IO.Path]::Combine($selfTestInputs, "local-browser-bridge-v0.12.51-windows-x86_64.exe")
        $inputHelper = [IO.Path]::Combine($selfTestInputs, "local-computer-helper-v0.12.51-windows-x86_64.exe")
