...
function Get-CandidateBinding {
    ...
    if ($Preflight.phase -cne "preflight" -or $Preflight.passed -ne $true -or
        ...
        throw "PreflightRecord is not a passing v0.12.42 preflight."
    }
...
function Read-RollbackCandidateCardState {
    ...
    $exchange = Invoke-OperatorExchange "ui-state" "rollback-candidate-card" `
        "Interpret exact v0.12.42 test-owned candidate-card presence from this fresh owned frame." `
        ([ordered]@{ type = "enum"; values = @("present", "absent") }) $fresh.Observation $null
                ...
                if ($cardState -ceq "present") {
                    ...
                    Invoke-UnrecordedNativeSteps `
                        "Use only the exact bound test-owned chrome://extensions window." @(
                            [ordered]@{ kind="click"; label="Remove on the exact v0.12.42 test-owned candidate card" },
...
function Invoke-Run {
    ...
    if ($Port -ne 17373) {
        throw "The v0.12.42 acceptance recorder requires the canonical 127.0.0.1:17373 endpoint."
    }
    ...
    try {
        ...
        $initialCandidateCard = Read-LiveCandidateCardState `
            $epoch "absent" "Exact v0.12.42 test-owned candidate card before installation"
        ...
        Invoke-RecordedAction $epoch $script:ActionNames[5] @() "none" "Verify exactly one enabled unpacked Local Browser Bridge v0.12.42 card, no duplicate, and no load error."
        ...
        ) "acceptanceTokenSave" "Verify v0.12.42 is connected and the credential field is empty."
        ...
        Invoke-RecordedAction $epoch $script:ActionNames[9] @([ordered]@{ kind="click"; label="the exact chrome://extensions tab" }) "none" "Verify exactly one enabled unpacked Local Browser Bridge v0.12.42 card, no error, and Chrome's native debugger-use indicator while the exact bridge lease is active."
        ...
        Invoke-RecordedAction $epoch $script:ActionNames[24] @(
            ...
            [ordered]@{ kind="click"; label="Remove on the exact v0.12.42 test-owned candidate card" },
            [ordered]@{ kind="click"; label="confirm removal of that exact candidate card" }
        ) "extensionDisposition" "Verify the helper switched to the protected chrome://extensions tab before removing only the new test-owned v0.12.42 card."
        $finalCandidateCard = Read-LiveCandidateCardState `
            $epoch "absent" "Exact v0.12.42 test-owned candidate card after removal"
