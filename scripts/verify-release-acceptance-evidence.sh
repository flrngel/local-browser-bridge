...
readonly EVIDENCE_PRODUCT_VERSION="0.12.28"
...
PY
  jq -e '
      ...
      and .version == "0.12.28" and .target == "loopback-demo" and .passed == true
  ...
  jq -e '
      ...
      and .version == "0.12.28" and .passed == true
  ...
  jq -e '
      ...
      and .extension.cardCount == 1 and .extension.version == "0.12.28" and .extension.enabled == true
...
self_test() {
  ...
  self_test_attestation_selection
  printf '%s' '{"schemaVersion":2,"tag":"v0.12.28","sourceSha":"1111111111111111111111111111111111111111","tagObjectSha":"2222222222222222222222222222222222222222","workflowRunId":"123","workflowRunAttempt":"1","releaseCandidateArtifactId":"456","releaseCandidateArtifactZipSha256":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb","checksumManifestSha256":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","evidenceRef":"refs/heads/evidence/v0.12.28-release-run-123-attempt-1","evidenceCommitSha":"3333333333333333333333333333333333333333","macosPassed":true,"macosAcceptanceSha256":"cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc","macosQuietResultSha256":"dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd","macosDeliberateConcurrencyResultSha256":"eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee","windowsPassed":true,"windowsResultSha256":"ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff","stockChromePassed":true,"stockChrome":true,"stockChromeResultSha256":"9999999999999999999999999999999999999999999999999999999999999999"}' > "$receipt"
  validate_receipt "$receipt" v0.12.28 "$sha1_a" "$sha1_b" 123 1 "$sha256_a" \
    || die "self-test rejected a valid canonical schema-2 receipt"
  jq -cS . "$receipt" > "$scratch/noncanonical.json"
  if validate_receipt "$scratch/noncanonical.json" v0.12.28 "$sha1_a" "$sha1_b" 123 1 "$sha256_a"; then
    die "self-test accepted reordered receipt keys"
  fi
  jq -c '.schemaVersion = 1' "$receipt" > "$scratch/stale.json"
  if validate_receipt "$scratch/stale.json" v0.12.28 "$sha1_a" "$sha1_b" 123 1 "$sha256_a"; then
