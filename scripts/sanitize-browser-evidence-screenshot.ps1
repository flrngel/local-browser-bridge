...
function Get-CandidateBindingFromPreflight {
    ...
    try {
        ...
        if ([string]$record.candidate.version -ceq "0.12.60") {
            $expected = @(
        ...
        if (($actual -join "`n") -cne ($expected -join "`n") -or
            ...
            @("0.12.2", "0.12.60") -cnotcontains [string]$record.candidate.version) {
        ...
        foreach ($value in @(
            ...
            $(if ([string]$record.candidate.version -ceq "0.12.60") { [string]$record.candidate.computerHelper.sha256 } else { [string]$record.candidate.server.sha256 }),
        ...
        if ($script:CandidateVersionFromPreflight -ceq "0.12.60") {
...
function Invoke-Sanitize {
    ...
    if (-not $legacyOnePhase -and $LegacyV0122ReviewConfirmed) {
        throw "LegacyV0122ReviewConfirmed is forbidden for the v0.12.60 independent-review protocol."
    }
            ...
            try {
                ...
                if (-not $legacyOnePhase) {
                    ...
                    if ([IO.Path]::GetFileName($inputPath) -cne $expectedRawName) {
                        throw "v0.12.60 InputImage must use the canonical raw helper-capture filename."
                    }
...
function Invoke-BindReview {
    ...
    $denyValues = @(Read-DenyValues $DenyValuesFile)
    $candidateBinding = Get-CandidateBindingFromPreflight $PreflightRecord
    if ($script:CandidateVersionFromPreflight -cne "0.12.60") {
        throw "BindReview is available only for the v0.12.60 independent-review protocol."
    }
    ...
    try {
        ...
        $preflight = [ordered]@{
            ...
            releaseCandidateBinding = [ordered]@{
                schemaVersion = 3; version = "0.12.60"; releaseTag = "v0.12.60"
