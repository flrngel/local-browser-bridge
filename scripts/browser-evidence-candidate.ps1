...
function Initialize-TrustedGitExecutable {
    if ($Mode -cne "SelfTest" -and $Version -ceq "0.12.65") {
        ...
        if (-not [IO.Path]::IsPathRooted($TrustedEmptyHooksDirectory)) {
            throw "TrustedEmptyHooksDirectory must be an absolute path for v0.12.65."
        }
...
function Invoke-GitText {
    ...
    if ($script:UseHardenedGit) {
        # v0.12.65 accepts only a fresh isolated clone. These global switches
...
function Assert-HardenedGitEnvironment {
    if ($env:GIT_CONFIG_NOSYSTEM -cne "1" -or $env:GIT_CONFIG_GLOBAL -cne "NUL" -or
        ...
        throw "v0.12.65 requires the isolated Git environment declared by the acceptance protocol."
    }
    ...
    foreach ($entry in [Environment]::GetEnvironmentVariables("Process").GetEnumerator()) {
        ...
        if ($name -cmatch '^GIT_' -and $allowedGitEnvironment -cnotcontains $name) {
            throw "v0.12.65 refuses unexpected Git process variables."
        }
    }
    if (-not [String]::IsNullOrEmpty($env:SSH_ASKPASS)) {
        throw "v0.12.65 refuses SSH_ASKPASS in the isolated HTTPS Git environment."
    }
...
function Assert-HardenedRepositoryMetadata {
    ...
    if (-not [IO.Directory]::Exists($gitDirectory)) {
        throw "v0.12.65 requires a fresh clone with an ordinary .git directory."
    }
    ...
    if (($gitDirectoryInfo.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
        throw "v0.12.65 refuses a reparse-point Git directory."
    }
    ...
    if ([regex]::IsMatch($configText, $dangerousConfig)) {
        throw "v0.12.65 refuses executable, included, rewritten, or promisor local Git configuration."
    }
    ...
    if (-not [regex]::IsMatch($configText, '^\s*\[remote\s+"origin"\]\s*$', [Text.RegularExpressions.RegexOptions]::Multiline) -or
        ...
        throw "v0.12.65 repository origin is not the fixed HTTPS release repository."
    }
    foreach ($forbiddenPath in @(
        ...
        if ([IO.File]::Exists($forbiddenPath) -or [IO.Directory]::Exists($forbiddenPath)) {
            throw "v0.12.65 repository contains replacement, alternate, or shallow object state."
        }
    }
    ...
    if ([IO.Directory]::Exists($packDirectory)) {
        ...
        if (($packInfo.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0 -or
            ...
            throw "v0.12.65 repository contains promisor object state."
        }
    }
}
...
function Assert-CleanExactCheckout {
    ...
    if ($script:UseHardenedGit) {
        ...
        if (@($indexFlags | Where-Object { $_ -cnotmatch '^H ' }).Count -ne 0) {
            throw "v0.12.65 refuses assume-unchanged or skip-worktree index entries."
        }
    }
}
...
function Get-CandidateSnapshot {
    ...
    $helperBinding = $null
    if ($ExpectedVersion -ceq "0.12.65") {
        if ([String]::IsNullOrWhiteSpace($ComputerHelperPath)) {
            throw "Computer helper executable is required for v0.12.65."
        }
...
function Get-CandidateBindingDomain {
    ...
    if ($Candidate.version -ceq "0.12.65") {
...
function Assert-BindingRecord {
    ...
    if ($Record.candidate.version -ceq "0.12.65") {
        $expectedKeys = @(
...
function Invoke-Preflight {
    ...
    if ($Version -ceq "0.12.65") { $record.releaseCandidateBinding = $releaseBinding }
...
function Invoke-SelfTest {
    ...
    try {
        ...
        # Exercise the v0.12.65 metadata gate without enabling hardened dispatch
        ...
        foreach ($adversarialConfig in @(
            ...
            if (-not $refused) {
                throw "v0.12.65 metadata gate accepted adversarial $($adversarialConfig.Label) config."
            }
        ...
        if (-not $replaceRefused) {
            throw "v0.12.65 metadata gate accepted a replacement-object ref."
        }
        ...
        if (-not $promisorRefused) {
            throw "v0.12.65 metadata gate accepted a promisor object marker."
        }
        ...
        $v3Names = @(
            "local-browser-bridge-v0.12.65-windows-x86_64.exe",
            "local-computer-helper-v0.12.65-windows-x86_64.exe",
            "local-browser-bridge-v0.12.65-macos-universal.tar.gz",
            "local-browser-bridge-extension-v0.12.65.zip"
        )
        $v3Candidate = [ordered]@{
            version = "0.12.65"
            finalSha = $v3Source
