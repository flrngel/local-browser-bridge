...
$script:OperatorV2Version = "0.12.9"
...
$script:RequiredVisibleStatesV2 = [ordered]@{
    "extension-loaded" = "stock Chrome chrome://extensions shows exactly one enabled unpacked Local Browser Bridge v0.12.9 card with no load errors and Chrome's debugger-use indicator during the active bridge lease"
...
function Assert-OperatorResultsV2 {
    ...
    Assert-ExactKeys $Operator $operatorFields "v0.12.9 operator results"
    Assert-ExactPropertyOrder $Operator $operatorFields "v0.12.9 operator results"
    Assert-IntegerRange $Operator.schemaVersion 2 2 "v0.12.9 operator schemaVersion"
    if ($ExpectedVersion -cne $script:OperatorV2Version -or
        $Operator.evidenceType -cne "stock-user-chrome-operator-observations") {
        throw "The v0.12.9 operator schema is bound only to a v0.12.9 candidate."
    }
    Assert-CandidateBindingDomain $Operator.candidateBinding $ExpectedBinding "v0.12.9 operator candidateBinding"
    ...
    Assert-ExactKeys $environment $environmentFields "v0.12.9 operator environment"
    Assert-ExactPropertyOrder $environment $environmentFields "v0.12.9 operator environment"
    if ($environment.platform -cne "windows-x86_64" -or $environment.browserProduct -cne "Google Chrome" -or
        -not [regex]::IsMatch([string]$environment.browserVersion, '^[0-9]{1,3}\.[0-9]{1,5}\.[0-9]{1,5}\.[0-9]{1,5}$') -or
        [int]([string]$environment.browserVersion).Split('.')[0] -lt 140) {
        throw "The v0.12.9 operator browser identity is invalid."
    }
    foreach ($name in @("stockUserChrome", "existingUserSession", "dedicatedTemporaryWindow", "localDemoOnly")) {
        Assert-ExactBoolean $environment.$name $true "v0.12.9 operator environment $name"
    }
    foreach ($name in @("browserLaunchFlagsUsed", "directCdpUsed", "automationTestProfileUsed")) {
        Assert-ExactBoolean $environment.$name $false "v0.12.9 operator environment $name"
    }
    ...
    Assert-ExactKeys $surfaces $surfaceFields "v0.12.9 action surfaces"
    Assert-ExactPropertyOrder $surfaces $surfaceFields "v0.12.9 action surfaces"
    ...
    if ($script:ExternalOrchestrationSurfaces -cnotcontains $surfaces.orchestrationAndConsent) {
        throw "v0.12.9 requires an explicit external human-consent orchestration surface."
    }
    foreach ($name in @(
        ...
        Assert-BrowserChromeSurfaceV2 $surfaces.$name "v0.12.9 action surface $name"
    }
    ...
    Assert-ExactKeys $helper $helperFields "v0.12.9 computer helper chain"
    Assert-ExactPropertyOrder $helper $helperFields "v0.12.9 computer helper chain"
    foreach ($name in @(
        "candidateBoundExecutableStarted", "connectedThroughLoopbackServer", "serverApiOnly",
        "exactChromeWindowSelected", "chromeExtensionsLoadCompleted", "browserApiActionCompleted",
        ...
        Assert-ExactBoolean $helper.$name $true "v0.12.9 computer helper chain $name"
    }
    if ($helper.screenshotEndpoint -cne "/api/computer/screenshot") {
        throw "v0.12.9 computer helper screenshots must use /api/computer/screenshot."
    }
    Assert-IntegerRange $helper.rawScreenshotCount 3 3 "v0.12.9 computer helper raw screenshot count"
    ...
    ) "v0.12.9 extension proof"
    Assert-IntegerRange $extension.cardCount 1 1 "v0.12.9 extension cardCount"
    Assert-IntegerRange $extension.loadErrors 0 0 "v0.12.9 extension loadErrors"
    if ($extension.version -cne $ExpectedVersion -or
        $extension.loadedVia -cne "chrome://extensions-load-unpacked") {
        throw "The v0.12.9 extension card identity is invalid."
    }
    foreach ($name in @(
        "enabled", "loadedDirectoryByteMatchesCandidateZip", "popupConnected",
        "debuggerLeaseActiveAtFirstCapture", "nativeDebuggerUseIndicatorSeen"
    )) {
        Assert-ExactBoolean $extension.$name $true "v0.12.9 extension $name"
    }

    if ($Operator.screenshotCaptures.Count -ne $script:ExpectedScreenshotsV2.Count) {
        throw "v0.12.9 must bind exactly three machine-helper screenshot capture surfaces."
    }
    for ($index = 0; $index -lt $script:ExpectedScreenshotsV2.Count; $index += 1) {
        ...
        if ($capture.purpose -cne $purpose -or
            ...
            throw "v0.12.9 screenshot identity, helper surface, or visible-state criterion is invalid."
        }
    }
    ...
    Assert-ExactKeys $review $reviewFields "v0.12.9 human visual review"
    foreach ($name in @(
        ...
        Assert-ExactBoolean $review.$name $true "v0.12.9 human visual review $name"
    }
    Assert-IntegerRange $review.reviewedCropCount 3 3 "v0.12.9 human-reviewed crop count"
    ...
    Assert-ExactKeys $cleanup $cleanupFields "v0.12.9 cleanup"
    foreach ($name in @(
        "controlReleased", "testTabsClosed", "testWindowClosed", "popupDisconnected", "serverStopped",
        ...
        Assert-ExactBoolean $cleanup.$name $true "v0.12.9 cleanup $name"
    }
    Assert-ExactKeys $cleanup.savedTokenClear @(
    ...
    foreach ($name in @(
        ...
        Assert-ExactBoolean $retained.$name $false "v0.12.9 retained evidence $name"
    }
}
...
function Assert-ScreenshotRecords {
    ...
    if ($Paths.Count -ne $expectedScreenshots.Count) {
        if ($ExpectedVersion -ceq $script:OperatorV2Version) {
            throw "Exactly three v0.12.9 screenshot sidecars are required."
        }
...
function Assert-ComputerHelperRecordV2 {
    ...
    ) "v0.12.9 computer-helper record"
...
function Assert-RetainedEvidenceDirectoryV2 {
    ...
    if ([IO.Path]::GetFileName($OutputPath) -cne "browser-acceptance.json") {
        throw "v0.12.9 final evidence must use the canonical browser-acceptance.json filename."
    }
    $fixedInputs = [ordered]@{
    ...
    foreach ($entry in $fixedInputs.GetEnumerator()) {
        if ([IO.Path]::GetFileName($entry.Value) -cne $entry.Key -or
            ...
            throw "v0.12.9 retained evidence inputs must use canonical names in one directory."
        }
    }
    foreach ($sidecarPath in $SidecarPaths) {
        if (-not [String]::Equals(
            ...
            throw "v0.12.9 screenshot sidecars must be in the retained evidence directory."
        }
        ...
        if (-not [IO.File]::Exists($imagePath)) {
            throw "v0.12.9 retained screenshot image is missing."
        }
        $image = [IO.FileInfo]::new($imagePath)
        if (($image.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
            throw "v0.12.9 retained screenshot image must not be a reparse point."
        }
    ...
    if ($expectedNames.Count -ne 11 -or ($expectedNames | Select-Object -Unique).Count -ne 11) {
        throw "v0.12.9 retained evidence allowlist must contain exactly 11 unique inputs."
    }
    $actualNames = @()
    foreach ($item in [IO.DirectoryInfo]::new($directory).GetFileSystemInfos()) {
        if ($item -isnot [IO.FileInfo] -or
            ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
            throw "v0.12.9 retained evidence directory contains a directory, link, or unsupported entry."
        }
    ...
    if (($expectedSorted -join "`n") -cne ($actualSorted -join "`n")) {
        throw "v0.12.9 retained evidence directory contains a missing or unexpected input."
    }
}
...
function Invoke-InitializeOperator {
    ...
    if ($templateVersion -ceq $script:OperatorV2Version) {
        ...
        ) "v0.12.9 operator template"
        Assert-IntegerRange $operator.schemaVersion 2 2 "v0.12.9 operator template schemaVersion"
    }
    else {
        Assert-ExactKeys $operator @(
            "schemaVersion", "evidenceType", "candidateBinding", "environment", "extension", "handback", "cleanup"
...
function New-PassingOperatorV2SelfTest {
    ...
    $templatePath = [IO.Path]::GetFullPath([IO.Path]::Combine(
        $PSScriptRoot, "..", "evidence", "v0.12.9", "browser", "operator-results.template.json"
    ))
    $operator = Read-Json $templatePath "v0.12.9 operator self-test template"
    $operator.candidateBinding = [pscustomobject]$Binding
...
function Invoke-SelfTest {
    ...
    try {
        ...
        $initializedV2 = Read-Json $initializedV2Path "initialized v0.12.9 operator checklist"
        if ($initializedV2.schemaVersion -ne 2 -or $initializedV2.extension.version -cne $script:OperatorV2Version) {
            throw "Evidence initializer did not select the v0.12.9 operator schema."
        }
        ...
        # Exercise the complete v0.12.9 Finalize path with its exact 11-file
        ...
        $persistedV2 = Read-Json $finalizeV2OutputPath "finalized v0.12.9 self-test record"
        if ($persistedV2.schemaVersion -ne 2 -or $persistedV2.candidate.version -cne $script:OperatorV2Version -or
            ...
            throw "Evidence finalizer complete v0.12.9 self-test failed."
        }
