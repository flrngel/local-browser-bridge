...
$script:OperatorV2Version = "0.12.49"
...
function Assert-OperatorResultsV2 {
    ...
    Assert-ExactKeys $Operator $operatorFields "v0.12.49 operator results"
    Assert-ExactPropertyOrder $Operator $operatorFields "v0.12.49 operator results"
    Assert-IntegerRange $Operator.schemaVersion 3 3 "v0.12.49 operator schemaVersion"
    if ($ExpectedVersion -cne $script:OperatorV2Version -or
        $Operator.evidenceType -cne "stock-user-chrome-operator-observations") {
        throw "The v0.12.49 operator schema is bound only to a v0.12.49 candidate."
    }
    [void](Assert-ReleaseCandidateBinding $Operator.releaseCandidateBinding $Candidate "v0.12.49 operator releaseCandidateBinding")
    ...
    Assert-CandidateBindingDomain $Operator.candidateBinding $ExpectedBinding "v0.12.49 operator candidateBinding"
    ...
    Assert-ExactKeys $environment $environmentFields "v0.12.49 operator environment"
    Assert-ExactPropertyOrder $environment $environmentFields "v0.12.49 operator environment"
    if ($environment.platform -cne "windows-x86_64" -or $environment.browserProduct -cne "Google Chrome" -or
        -not [regex]::IsMatch([string]$environment.browserVersion, '^[0-9]{1,3}\.[0-9]{1,5}\.[0-9]{1,5}\.[0-9]{1,5}$') -or
        [int]([string]$environment.browserVersion).Split('.')[0] -lt 140) {
        throw "The v0.12.49 operator browser identity is invalid."
    }
    foreach ($name in @("stockUserChrome", "existingUserSession", "dedicatedTemporaryWindow", "localDemoOnly")) {
        Assert-ExactBoolean $environment.$name $true "v0.12.49 operator environment $name"
    }
    foreach ($name in @("browserLaunchFlagsUsed", "directCdpUsed", "automationTestProfileUsed")) {
        Assert-ExactBoolean $environment.$name $false "v0.12.49 operator environment $name"
    }
    ...
    Assert-ExactKeys $surfaces $surfaceFields "v0.12.49 action surfaces"
    Assert-ExactPropertyOrder $surfaces $surfaceFields "v0.12.49 action surfaces"
    ...
    if ($surfaces.orchestrationAndConsent -cne `
        "user-orchestrator-secured-ssh-exported-file-review") {
        throw "v0.12.49 requires the secured-SSH exported-file review orchestration surface."
    }
    foreach ($name in @(
        ...
        Assert-BrowserChromeSurfaceV2 $surfaces.$name "v0.12.49 action surface $name"
    }
    ...
    Assert-ExactKeys $helper $helperFields "v0.12.49 computer helper chain"
    Assert-ExactPropertyOrder $helper $helperFields "v0.12.49 computer helper chain"
    foreach ($name in @(
        "candidateBoundExecutableStarted", "connectedThroughLoopbackServer", "serverApiOnly",
        "exactChromeWindowSelected", "chromeExtensionsLoadCompleted", "browserApiActionCompleted",
        ...
        Assert-ExactBoolean $helper.$name $true "v0.12.49 computer helper chain $name"
    }
    if ($helper.screenshotEndpoint -cne "/api/computer/screenshot") {
        throw "v0.12.49 computer helper screenshots must use /api/computer/screenshot."
    }
    Assert-IntegerRange $helper.rawScreenshotCount 6 6 "v0.12.49 computer helper raw screenshot count"
    ...
    ) "v0.12.49 extension proof"
    Assert-IntegerRange $extension.cardCount 1 1 "v0.12.49 extension cardCount"
    Assert-IntegerRange $extension.loadErrors 0 0 "v0.12.49 extension loadErrors"
    if ($extension.version -cne $ExpectedVersion -or
        $extension.loadedVia -cne "chrome://extensions-load-unpacked") {
        throw "The v0.12.49 extension card identity is invalid."
    }
    foreach ($name in @(
        "enabled", "loadedDirectoryByteMatchesCandidateZip", "popupConnected",
        "debuggerLeaseActiveAtFirstCapture", "nativeDebuggerUseIndicatorSeen"
    )) {
        Assert-ExactBoolean $extension.$name $true "v0.12.49 extension $name"
    }

    Assert-ExactKeys $Operator.handback @("stop", "cancel") "v0.12.49 handback matrix"
    Assert-ExactPropertyOrder $Operator.handback @("stop", "cancel") "v0.12.49 handback matrix"
    ...
    if ($Operator.screenshotCaptures.Count -ne $script:ExpectedScreenshotsV2.Count) {
        throw "v0.12.49 must bind exactly six machine-helper screenshot capture surfaces."
    }
    for ($index = 0; $index -lt $script:ExpectedScreenshotsV2.Count; $index += 1) {
        ...
        if ($capture.purpose -cne $purpose -or
            ...
            throw "v0.12.49 screenshot identity, helper surface, or visible-state criterion is invalid."
        }
    }
    ...
    Assert-ExactKeys $review $reviewFields "v0.12.49 independent visual review"
    Assert-ExactPropertyOrder $review $reviewFields "v0.12.49 independent visual review"
    foreach ($name in @("recordSha256", "executorSessionRef", "reviewerSessionRef")) {
        Assert-Hex $review.$name 64 "v0.12.49 independent visual review $name"
    }
    ...
    foreach ($name in @(
        ...
        Assert-ExactBoolean $review.$name $true "v0.12.49 independent visual review $name"
    }
    Assert-IntegerRange $review.reviewedCropCount 6 6 "v0.12.49 independently reviewed crop count"
    ...
    Assert-ExactKeys $cleanup $cleanupFields "v0.12.49 cleanup"
    foreach ($name in @(
        "controlReleased", "testTabsClosed", "testWindowClosed", "popupDisconnected", "serverStopped",
        ...
        Assert-ExactBoolean $cleanup.$name $true "v0.12.49 cleanup $name"
    }
    ...
    foreach ($name in @(
        ...
        Assert-ExactBoolean $retained.$name $false "v0.12.49 retained evidence $name"
    }
}
...
function Assert-ScreenshotRecords {
    ...
    if ($Paths.Count -ne $expectedScreenshots.Count) {
        if ($ExpectedVersion -ceq $script:OperatorV2Version) {
            throw "Exactly six v0.12.49 screenshot sidecars are required."
        }
...
function Assert-ComputerHelperRecordV2 {
    ...
    ) "v0.12.49 computer-helper record"
...
function Assert-RetainedEvidenceDirectoryV2 {
    ...
    if ([IO.Path]::GetFileName($OutputPath) -cne "browser-acceptance.json") {
        throw "v0.12.49 final evidence must use the canonical browser-acceptance.json filename."
    }
    $fixedInputs = [ordered]@{
    ...
    foreach ($entry in $fixedInputs.GetEnumerator()) {
        if ([IO.Path]::GetFileName($entry.Value) -cne $entry.Key -or
            ...
            throw "v0.12.49 retained evidence inputs must use canonical names in one directory."
        }
    }
    foreach ($sidecarPath in $SidecarPaths) {
        if (-not [String]::Equals(
            ...
            throw "v0.12.49 screenshot sidecars must be in the retained evidence directory."
        }
        ...
        if (-not [IO.File]::Exists($imagePath)) {
            throw "v0.12.49 retained screenshot image is missing."
        }
        $image = [IO.FileInfo]::new($imagePath)
        if (($image.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
            throw "v0.12.49 retained screenshot image must not be a reparse point."
        }
    ...
    if ($expectedNames.Count -ne 21 -or ($expectedNames | Select-Object -Unique).Count -ne 21) {
        throw "v0.12.49 retained evidence allowlist must contain exactly 21 unique inputs."
    }
    $actualNames = @()
    foreach ($item in [IO.DirectoryInfo]::new($directory).GetFileSystemInfos()) {
        if ($item -isnot [IO.FileInfo] -or
            ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
            throw "v0.12.49 retained evidence directory contains a directory, link, or unsupported entry."
        }
    ...
    if (($expectedSorted -join "`n") -cne ($actualSorted -join "`n")) {
        throw "v0.12.49 retained evidence directory contains a missing or unexpected input."
    }
}
...
function Invoke-BuildOperator {
    ...
    if ($candidate.version -cne $script:OperatorV2Version) {
        throw "BuildOperator is registered only for the v0.12.49 autonomous evidence protocol."
    }
...
function Invoke-InitializeOperator {
    ...
    if ($templateVersion -ceq $script:OperatorV2Version) {
        throw "v0.12.49 operator results must be created with BuildOperator; editable checklists are not accepted."
    }
...
function New-PassingOperatorV2SelfTest {
    ...
    $templatePath = [IO.Path]::GetFullPath([IO.Path]::Combine(
        $PSScriptRoot, "..", "evidence", "v0.12.49", "browser", "operator-results.template.json"
    ))
    $operator = Read-Json $templatePath "v0.12.49 operator self-test template"
...
function Invoke-SelfTest {
    ...
    try {
        ...
        # Exercise the complete v0.12.49 Finalize path with its exact 21-file
        ...
        $persistedV2 = Read-Json $finalizeV2OutputPath "finalized v0.12.49 self-test record"
        if ($persistedV2.schemaVersion -ne 3 -or $persistedV2.candidate.version -cne $script:OperatorV2Version -or
            ...
            throw "Evidence finalizer complete v0.12.49 self-test failed."
        }
